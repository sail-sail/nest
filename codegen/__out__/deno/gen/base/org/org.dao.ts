...
// MARK: syncUsrLblByUsrIdOrg
/** 根据 usr_id 同步创建人/更新人标签 */
export async function syncUsrLblByUsrIdOrg(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameOrg();
  const method = "syncUsrLblByUsrIdOrg";
  ...
  const sql = `update base_org set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
