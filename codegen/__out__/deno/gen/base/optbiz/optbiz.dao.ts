...
// MARK: syncUsrLblByUsrIdOptbiz
/** 根据 usr_id 同步创建人/更新人标签 */
export async function syncUsrLblByUsrIdOptbiz(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameOptbiz();
  const method = "syncUsrLblByUsrIdOptbiz";
  ...
  const sql = `update base_optbiz set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
