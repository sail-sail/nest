...
// MARK: syncUsrLblByUsrIdLoginLog
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdLoginLog(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameLoginLog();
  const method = "syncUsrLblByUsrIdLoginLog";
  ...
  const sql = `update base_login_log set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function deleteByIdsLoginLog(
  ...
  const oldModels = await findByIdsOkLoginLog(ids, options);
  let affectedRows = 0;
