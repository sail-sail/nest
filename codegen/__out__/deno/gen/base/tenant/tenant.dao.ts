...
// MARK: syncUsrLblByUsrIdTenant
/** 根据 usr_id 同步创建人/更新人标签 */
export async function syncUsrLblByUsrIdTenant(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameTenant();
  const method = "syncUsrLblByUsrIdTenant";
  ...
  const sql = `update base_tenant set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function deleteByIdsTenant(
  ...
  const oldModels = await findByIdsOkTenant(ids, options);
  let affectedRows = 0;
