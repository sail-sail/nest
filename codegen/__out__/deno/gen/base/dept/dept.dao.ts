...
// MARK: syncUsrLblByUsrIdDept
/** 根据 usr_id 同步创建人/更新人标签 */
export async function syncUsrLblByUsrIdDept(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameDept();
  const method = "syncUsrLblByUsrIdDept";
  ...
  const sql = `update base_dept set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function deleteByIdsDept(
  ...
  const oldModels = await findByIdsOkDept(ids, options);
  let affectedRows = 0;
