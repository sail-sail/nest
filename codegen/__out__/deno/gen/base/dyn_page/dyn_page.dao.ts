...
// MARK: syncUsrLblByUsrIdDynPage
/** 根据 usr_id 同步创建人/更新人标签 */
export async function syncUsrLblByUsrIdDynPage(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameDynPage();
  const method = "syncUsrLblByUsrIdDynPage";
  ...
  const sql = `update base_dyn_page set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
