...
async function getWhereQuery(
  ...
  if (search?.dict_id != null && !Array.isArray(search?.dict_id)) {
...
async function _creates(
  ...
  const args = new QueryArgs();
  let sql = `insert into base_dict_detail(id,create_time,create_usr_id,create_usr_id_lbl,dict_id,lbl,val,is_locked,is_enabled,order_by,rem,is_sys)values`;
...
export async function deleteByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    const args = new QueryArgs();
    const sql = `update base_dict_detail set is_deleted=1,delete_time=${ args.push(reqDate()) } where id=${ args.push(id) } limit 1`;
...
export async function lockByIds(
  ...
  const args = new QueryArgs();
  let sql = `update base_dict_detail set is_locked=${ args.push(is_locked) } where id in ${ args.push(ids) }`;
  const result = await execute(sql, args);
  const num = result.affectedRows;
...
export async function forceDeleteByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    {
      const args = new QueryArgs();
      const sql = `select * from base_dict_detail where id = ${ args.push(id) }`;
