...
// MARK: syncUsrLblByUsrIdDynPageField
/** 根据 usr_id 同步创建人/更新人标签 */
export async function syncUsrLblByUsrIdDynPageField(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameDynPageField();
  const method = "syncUsrLblByUsrIdDynPageField";
  ...
  const sql = `update base_dyn_page_field set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function deleteByIdsDynPageField(
  ...
  const oldModels = await findByIdsOkDynPageField(ids, options);
  let affectedRows = 0;
