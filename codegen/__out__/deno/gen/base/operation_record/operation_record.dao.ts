...
// MARK: syncUsrLblByUsrIdOperationRecord
/** 根据 usr_id 同步创建人/更新人标签 */
export async function syncUsrLblByUsrIdOperationRecord(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameOperationRecord();
  const method = "syncUsrLblByUsrIdOperationRecord";
  ...
  const sql = `update base_operation_record set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function deleteByIdsOperationRecord(
  ...
  const oldModels = await findByIdsOkOperationRecord(ids, options);
  let affectedRows = 0;
