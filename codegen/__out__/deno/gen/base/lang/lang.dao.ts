...
export async function findAll(
  ...
  const result = await query<LangModel>(
    sql,
    args,
    {
      cacheKey1,
 ...
 * @return {Promise<LangId | undefined>}
 */
...
): Promise<LangId | undefined> {
  const isEquals = equalsByUnique(oldModel, input);
  if (isEquals) {
    if (uniqueType === UniqueType.Throw) {
      throw new UniqueException(await ns("此 {0} 已经存在", await ns("语言")));
    }
...
async function _creates(
  ...
  const args = new QueryArgs();
  let sql = `insert into base_lang(id,create_time,create_usr_id,code,lbl,is_enabled,order_by,rem,is_sys)values`;
...
export async function updateById(
  ...
  
  {
    ...
    if (models.length > 0) {
      if (!options || options.uniqueType === UniqueType.Throw) {
        throw await ns("此 {0} 已经存在", await ns("语言"));
  ...
  if (!oldModel) {
    throw await ns("编辑失败, 此 {0} 已被删除", await ns("语言"));
  }
  
  const args = new QueryArgs();
  let sql = `
    update base_lang set
  `;
  let updateFldNum = 0;
  if (input.code != null) {
...
export async function deleteByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    const args = new QueryArgs();
    const sql = `update base_lang set is_deleted=1,delete_time=${ args.push(reqDate()) } where id = ${ args.push(id) } limit 1`;
...
export async function enableByIds(
  ...
  const args = new QueryArgs();
  const sql = `update base_lang set is_enabled=${ args.push(is_enabled) } where id in ${ args.push(ids) }`;
  const result = await execute(sql, args);
...
export async function revertByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    const args = new QueryArgs();
    const sql = `update base_lang set is_deleted = 0 where id = ${ args.push(id) } limit 1`;
    const result = await execute(sql, args);
    num += result.affectedRows;
    // 检查数据的唯一索引
    {
...
export async function forceDeleteByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    {
      const args = new QueryArgs();
      const sql = `select * from base_lang where id = ${ args.push(id) }`;
      const model = await queryOne(sql, args);
      log("forceDeleteByIds:", model);
    }
    const args = new QueryArgs();
    const sql = `delete from base_lang where id = ${ args.push(id) } and is_deleted = 1 limit 1`;
    const result = await execute(sql, args);
    num += result.affectedRows;
  }
