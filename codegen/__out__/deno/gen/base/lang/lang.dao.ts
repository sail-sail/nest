...
// MARK: syncUsrLblByUsrIdLang
/** 根据 usr_id 同步创建人/更新人标签 */
export async function syncUsrLblByUsrIdLang(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameLang();
  const method = "syncUsrLblByUsrIdLang";
  ...
  const sql = `update base_lang set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
