...
export async function findAll(
  ...
  if (search && search.state != null) {
    ...
    if (len > FIND_ALL_IDS_LIMIT) {
      throw new Error(`search.state.length > ${ FIND_ALL_IDS_LIMIT }`);
    }
  }
  // 类型
...
async function _creates(
  ...
  const args = new QueryArgs();
  let sql = `insert into base_background_task(id,create_time,tenant_id,create_usr_id,lbl,state,type,result,err_msg,begin_time,end_time,rem)values`;
