...
/**
 * 获得表的唯一字段名列表
 * @return {{ uniqueKeys: (keyof Dict_DetailModel)[]; uniqueComments: { [key: string]: string }; }}
 */
...
/**
 * 通过唯一约束获得一行数据
 * @param {Dict_DetailSearch & { $extra?: SearchExtra[] } | PartialNull<Dict_DetailModel>} search0
 */
async function findByUnique(
  search0: Dict_DetailSearch & { $extra?: SearchExtra[] } | PartialNull<Dict_DetailModel>,
  options?: {
  },
) {
  if (search0.id) {
  ...
  const search: Dict_DetailSearch & { $extra?: SearchExtra[] } = { };
...
/**
 * 根据条件查找第一条数据
 * @param {Dict_DetailSearch & { $extra?: SearchExtra[] }} search?
 */
async function findOne(
  ...
  const model = result[0] as Dict_DetailModel | undefined;
  return model;
}
...
async function updateById(
  ...
  const args = new QueryArgs();
  let sql = /*sql*/ `
    update dict_detail set update_time = ${ args.push(reqDate()) }
  `;
  let updateFldNum = 0;
  if (model.dict_id !== undefined) {
