...
async function _creates(
  ...
  const args = new QueryArgs();
  let sql = `insert into wx_wxo_app(id,create_time,tenant_id,create_usr_id,code,lbl,appid,appsecret,token,encoding_aes_key,domain_id,is_locked,is_enabled,order_by,rem)values`;
...
export async function deleteByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    const args = new QueryArgs();
    const sql = `update wx_wxo_app set is_deleted=1,delete_time=${ args.push(reqDate()) } where id=${ args.push(id) } limit 1`;
    const result = await execute(sql, args);
...
export async function forceDeleteByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    {
      const args = new QueryArgs();
      const sql = `select * from wx_wxo_app where id = ${ args.push(id) }`;
