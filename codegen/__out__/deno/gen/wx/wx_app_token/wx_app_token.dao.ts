...
// MARK: syncUsrLblByUsrIdWxAppToken
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdWxAppToken(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameWxAppToken();
  const method = "syncUsrLblByUsrIdWxAppToken";
  ...
  const sql = `update wx_wx_app_token set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function updateByIdReturnWxAppToken(
  id: WxAppTokenId,
  input: WxAppTokenInput,
  options?: {
    is_debug?: boolean;
  ...
  const model = await findByIdWxAppToken(
    id,
    options,
  );
