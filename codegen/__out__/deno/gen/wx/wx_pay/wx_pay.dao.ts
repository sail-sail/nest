...
async function _creates(
  ...
  const args = new QueryArgs();
  let sql = `insert into wx_wx_pay(id,create_time,tenant_id,create_usr_id,lbl,appid,mchid,public_key,private_key,v3_key,payer_client_ip,notify_url,is_locked,is_enabled,order_by,rem)values`;
...
export async function deleteByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    const args = new QueryArgs();
    const sql = `update wx_wx_pay set is_deleted=1,delete_time=${ args.push(reqDate()) } where id=${ args.push(id) } limit 1`;
    const result = await execute(sql, args);
...
export async function forceDeleteByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    {
      const args = new QueryArgs();
      const sql = `select * from wx_wx_pay where id = ${ args.push(id) }`;
