...
async function getWhereQuery(
  ...
  if (search?.wxo_app_id && !Array.isArray(search?.wxo_app_id)) {
    search.wxo_app_id = [ search.wxo_app_id ];
  }
  if (search?.wxo_app_id && search?.wxo_app_id.length > 0) {
...
export async function findByUnique(
  ...
  const models: WxoAppTokenModel[] = [ ];
  {
    ...
    if (!Array.isArray(search0.wxo_app_id)) {
      wxo_app_id.push(search0.wxo_app_id, search0.wxo_app_id);
    } else {
