...
async function _creates(
  ...
  const args = new QueryArgs();
  let sql = `insert into wxwork_wxw_app(id,create_time,tenant_id,create_usr_id,lbl,corpid,agentid,domain_id,corpsecret,contactsecret,is_locked,is_enabled,order_by,rem)values`;
...
export async function deleteByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    const args = new QueryArgs();
    const sql = `update wxwork_wxw_app set is_deleted=1,delete_time=${ args.push(reqDate()) } where id = ${ args.push(id) } limit 1`;
    const result = await execute(sql, args);
