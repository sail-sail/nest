...
export async function findCountWxwAppToken(
  ...
  const table = getTableNameWxwAppToken();
...
export async function getFieldCommentsWxwAppToken(): Promise<WxwAppTokenFieldComment> {
  const field_comments: WxwAppTokenFieldComment = {
    id: "ID",
    wxw_app_id: "企微应用",
    wxw_app_id_lbl: "企微应用",
    type: "类型corp和contact",
...
export async function checkByUniqueWxwAppToken(
  ...
  if (isEquals) {
    if (uniqueType === UniqueType.Throw) {
      throw new UniqueException("企微应用接口凭据 重复");
    }
...
export async function findOneWxwAppToken(
  ...
  const wxw_app_token_models = await findAllWxwAppToken(
    search,
    page,
    sort,
    options,
  ...
  const wxw_app_token_model = wxw_app_token_models[0];
  
  return wxw_app_token_model;
}

// MARK: findOneOkWxwAppToken
/** 根据条件查找第一企微应用接口凭据, 如果不存在则抛错 */
...
export async function findByIdWxwAppToken(
  ...
  const wxw_app_token_model = await findOneWxwAppToken(
    {
      id,
    },
    undefined,
...
export async function findByIdOkWxwAppToken(
  ...
  const wxw_app_token_model = await findByIdWxwAppToken(
    id,
    options,
  );
...
export async function findByIdsOkWxwAppToken(
  ...
  const table = getTableNameWxwAppToken();
  const method = "findByIdsOkWxwAppToken";
  ...
  const models = await findByIdsWxwAppToken(
    ids,
    options,
  );
...
// MARK: syncUsrLblByUsrIdWxwAppToken
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdWxwAppToken(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameWxwAppToken();
  const method = "syncUsrLblByUsrIdWxwAppToken";
  ...
  const sql = `update wxwork_wxw_app_token set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function updateByIdReturnWxwAppToken(
  id: WxwAppTokenId,
  input: WxwAppTokenInput,
  options?: {
    is_debug?: boolean;
  ...
  const model = await findByIdWxwAppToken(
    id,
    options,
  );
  
  if (!model) {
    throw new Error(`企微应用接口凭据 不存在`);
  }
