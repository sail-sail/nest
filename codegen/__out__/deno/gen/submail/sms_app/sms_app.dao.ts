...
async function getWhereQuery(
  ...
  if (search?.appkey != null) {
    whereQuery += ` and t.appkey=${ args.push(search.appkey) }`;
  }
  if (isNotEmpty(search?.appkey_like)) {
    whereQuery += ` and t.appkey like ${ args.push("%" + sqlLike(search?.appkey_like) + "%") }`;
  }
...
async function getFromQuery(
  ...
  let fromQuery = `submail_sms_app t`;
  return fromQuery;
}
...
export async function findAll(
  ...
  const result = await query<SmsAppModel>(
    sql,
    args,
    {
      cacheKey1,
...
export async function checkByUnique(
  ...
  if (isEquals) {
    if (uniqueType === UniqueType.Throw) {
      throw new UniqueException(await ns("此 {0} 已经存在", await ns("短信应用")));
    }
...
export async function existById(
  ...
  const args = new QueryArgs();
  const sql = `select 1 e from submail_sms_app t where t.id=${ args.push(id) } and t.is_deleted = 0 limit 1`;
...
export async function validateIsEnabled(
  ...
  if (model.is_enabled == 0) {
    throw `${ await ns("短信应用") } ${ await ns("已禁用") }`;
  }
}
...
export async function validateOption(
  ...
  if (!model) {
    const err_msg = `${ await ns("短信应用") } ${ await ns("不存在") }`;
...
async function _creates(
  ...
  for (const inputs2 of inputs2Arr) {
    for (let i = 0; i < inputs2.length; i++) {
      ...
      if (input.appkey != null) {
        sql += `,${ args.push(input.appkey) }`;
      } else {
        sql += ",default";
      }
...
export async function updateTenantById(
  ...
  const args = new QueryArgs();
  const sql = `update submail_sms_app set tenant_id=${ args.push(tenant_id) } where id=${ args.push(id) }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function updateById(
  ...
  
  {
    ...
    if (models.length > 0) {
      if (!options || !options.uniqueType || options.uniqueType === UniqueType.Throw) {
        throw await ns("此 {0} 已经存在", await ns("短信应用"));
  ...
  if (!oldModel) {
    throw await ns("编辑失败, 此 {0} 已被删除", await ns("短信应用"));
  }
  
  const args = new QueryArgs();
  let sql = `update submail_sms_app set `;
  let updateFldNum = 0;
  if (input.lbl != null) {
  ...
  if (input.appkey != null) {
    if (input.appkey != oldModel.appkey) {
      sql += `appkey=${ args.push(input.appkey) },`;
      updateFldNum++;
    }
  }
  if (input.is_locked != null) {
...
export async function deleteByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    const args = new QueryArgs();
    let sql = `update submail_sms_app set is_deleted=1`;
    if (!is_silent_mode && !is_creating) {
      let usr_id = await get_usr_id();
      if (usr_id != null) {
...
export async function enableByIds(
  ...
  const args = new QueryArgs();
  const sql = `update submail_sms_app set is_enabled=${ args.push(is_enabled) } where id in (${ args.push(ids) })`;
...
export async function revertByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    const args = new QueryArgs();
    const sql = `update submail_sms_app set is_deleted=0 where id=${ args.push(id) } limit 1`;
    const result = await execute(sql, args);
...
export async function forceDeleteByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    const args = new QueryArgs();
    const sql = `delete from submail_sms_app where id=${ args.push(id) } and is_deleted = 1 limit 1`;
    const result = await execute(sql, args);
    num += result.affectedRows;
  }
