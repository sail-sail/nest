...
async function getWhereQuery(
  ...
  if (search?.company_id != null) {
    whereQuery += ` and company_id_lbl.id in ${ args.push(search.company_id) }`;
  }
  if (search?.company_id_is_null) {
    whereQuery += ` and company_id_lbl.id is null`;
  }
...
async function getFromQuery(
  ...
  let fromQuery = `eams_archive t
    left join eams_company company_id_lbl
      on company_id_lbl.id = t.company_id
...
export async function findAll(
  ...
  // 关联单位
  if (search && search.company_id != null && search.company_id.length === 0) {
    return [ ];
  }
  // 创建人
...
export async function setIdByLbl(
  ...
  if (isNotEmpty(input.company_id_lbl) && input.company_id == null) {
    ...
    const companyModel = await findOneCompany({ lbl: input.company_id_lbl });
    if (companyModel) {
...
export async function getFieldComments(): Promise<ArchiveFieldComment> {
  ...
  const fieldComments: ArchiveFieldComment = {
    ...
    code: await n("编号"),
    lbl: await n("名称"),
    company_id: await n("关联单位"),
    company_id_lbl: await n("关联单位"),
    order_by: await n("排序"),
    rem: await n("备注"),
...
export async function checkByUnique(
  ...
  if (isEquals) {
    if (uniqueType === UniqueType.Throw) {
      throw new UniqueException(await ns("此 {0} 已经存在", await ns("全宗设置")));
    }
...
export async function create(
  ...
  while (true) {
    input.id = shortUuidV4<ArchiveId>();
...
export async function updateById(
  ...
  if (!oldModel) {
    throw await ns("编辑失败, 此 {0} 已被删除", await ns("全宗设置"));
  }
