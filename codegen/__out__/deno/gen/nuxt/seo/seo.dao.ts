...
// MARK: syncUsrLblByUsrIdSeo
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdSeo(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameSeo();
  const method = "syncUsrLblByUsrIdSeo";
  ...
  const sql = `update nuxt_seo set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
