...
async fn get_where_query(
  ...
  {
    let ids: Vec<FieldPermitId> = match &search {
  ...
  {
    let r#type: Option<Vec<FieldPermitType>> = match &search {
...
pub async fn revert_by_ids(
  ...
  for id in ids.clone() {
    ...
    // 检查数据的唯一索引
    {
      ...
      let mut input: FieldPermitInput = old_model.into();
      input.id = None;
      ...
      let models: Vec<FieldPermitModel> = models.into_iter()
        .filter(|item| 
          item.id != id
        )
        .collect();
