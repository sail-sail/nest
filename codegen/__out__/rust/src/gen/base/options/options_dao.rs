...
fn get_where_query<'a>(
  ...
  }
  {
    ...
    if let Some(ky) = ky {
      where_query += &format!(" and t.ky = {}", args.push(ky.into()));
    }
    ...
    if let Some(ky_like) = ky_like {
      where_query += &format!(" and t.ky like {}", args.push((sql_like(&ky_like) + "%").into()));
    }
  }
  {
  ...
  {
    let version: Vec<i64> = match &search {
      Some(item) => item.version.clone().unwrap(),
      None => vec![],
    };
    let version_gt: Option<i64> = match &version.len() {
      0 => None,
      _ => version[0].clone().into(),
    };
    let version_lt: Option<i64> = match &version.len() {
      0 => None,
      1 => None,
      _ => version[1].clone().into(),
    };
...
fn get_from_query() -> &'static str {
  let from_query = r#"base_options t
...
pub async fn find_all<'a>(
  ...
  let mut res: Vec<OptionsModel> = ctx.query(
    sql,
    args,
    options,
  ).await?;
