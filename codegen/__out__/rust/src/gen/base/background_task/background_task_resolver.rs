...
impl BackgroundTaskGenQuery {
  ...
  pub async fn find_one_background_task<'a>(
    ...
    let res = background_task_service::find_one(
      &mut ctx,
      search,
      sort,
      None,
  ...
  pub async fn find_by_id_background_task<'a>(
    ...
    let res = background_task_service::find_by_id(
      &mut ctx,
      id,
      None,
    ).await;
  ...
  /// 获取字段对应的名称
  pub async fn get_field_comments_background_task<'a>(
    &self,
    ctx: &Context<'a>,
  ) -> Result<BackgroundTaskFieldComment> {
    let mut ctx = CtxImpl::new(&ctx).auth()?;
    
    let res = background_task_service::get_field_comments(
      &mut ctx,
      None,
    ).await;
