...
impl MenuGenQuery {
  ...
  /// 根据 ID 查找是否已启用
  /// 记录不存在则返回 false
  async fn get_is_enabled_by_id_menu<'a>(
    &self,
    ctx: &Context<'a>,
    id: String,
  ) -> Result<bool> {
  ...
  /// 已锁定的记录不能修改和删除
  /// 记录不存在则返回 false
  async fn get_is_locked_by_id_menu<'a>(
    &self,
    ctx: &Context<'a>,
    id: String,
  ) -> Result<bool> {
    ...
    let res = menu_resolver::get_is_locked_by_id(
      &mut ctx,
      id,
      None,
    ).await;
  ...
  /// 获取字段对应的名称
  async fn get_field_comments_menu<'a>(
    &self,
    ctx: &Context<'a>,
  ...
  /// 查找 order_by 字段的最大值
  async fn find_last_order_by_menu<'a>(
    &self,
    ctx: &Context<'a>,
  ) -> Result<u32> {
    let mut ctx = CtxImpl::new(&ctx).auth()?;
...
impl MenuGenMutation {
  ...
  async fn lock_by_ids_menu<'a>(
    ...
    let res = menu_resolver::lock_by_ids(
      &mut ctx,
      ids,
      is_locked,
      None,
  ...
  /// 根据 ids 彻底删除数据
  async fn force_delete_by_ids_menu<'a>(
    &self,
    ctx: &Context<'a>,
    ids: Vec<String>,
  ) -> Result<u64> {
