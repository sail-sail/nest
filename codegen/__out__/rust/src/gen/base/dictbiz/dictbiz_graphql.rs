...
impl DictbizGenQuery {
  ...
  /// 根据 ID 查找是否已启用
  /// 记录不存在则返回 false
  pub async fn get_is_enabled_by_id_dictbiz<'a>(
    &self,
    ctx: &Context<'a>,
    id: String,
  ) -> Result<bool> {
  ...
  /// 获取字段对应的名称
  pub async fn get_field_comments_dictbiz<'a>(
    &self,
    ctx: &Context<'a>,
  ...
  /// 查找 order_by 字段的最大值
  pub async fn find_last_order_by_dictbiz<'a>(
    &self,
    ctx: &Context<'a>,
  ) -> Result<u32> {
    let mut ctx = CtxImpl::new(&ctx).auth()?;
...
impl DictbizGenMutation {
  ...
  /// 根据 ids 启用或禁用数据
  pub async fn enable_by_ids_dictbiz<'a>(
    &self,
    ctx: &Context<'a>,
    ids: Vec<String>,
    is_enabled: u8,
