...
fn get_from_query() -> &'static str {
  let from_query = r#"base_dictbiz t
...
pub async fn find_all<'a>(
  ...
  let mut res: Vec<DictbizModel> = ctx.query(
    sql,
    args,
    options,
  ).await?;
...
pub async fn get_field_comments<'a>(
  ...
  let n_route = i18n_dao::NRoute {
    route_path: "/base/dictbiz".to_owned().into(),
  };
  
  let field_comments = DictbizFieldComment {
...
pub async fn update_by_id<'a>(
  ...
  
  {
    ...
    let models: Vec<DictbizModel> = models.into_iter()
      .filter(|item| 
        &item.id != &id
      )
      .collect();
...
pub async fn revert_by_ids<'a>(
  ...
  for id in ids {
    ...
    // 检查数据的唯一索引
    {
      ...
      let mut input: DictbizInput = old_model.into();
      input.id = None;
