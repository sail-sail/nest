...
async fn get_where_query<'a>(
  ...
  }
  {
    ...
    if dictbiz_id_is_null {
      where_query += &format!(" and dictbiz_id_lbl.id is null");
    }
  }
  {
...
async fn get_from_query() -> Result<String> {
  let from_query = r#"base_dictbiz_detail t
    left join base_dictbiz dictbiz_id_lbl
      on dictbiz_id_lbl.id = t.dictbiz_id"#.to_owned();
  Ok(from_query)
}
...
pub async fn find_all<'a>(
  ...
  let mut res: Vec<DictbizDetailModel> = ctx.query(
    sql,
    args,
    options,
  ).await?;
...
pub async fn validate_is_enabled<'a>(
  ...
  if model.is_enabled == 0 {
    ...
    map.insert("0".to_owned(), "业务字典明细".to_owned());
    let err_msg = i18n_dao::ns(
      ctx,
      "{0} 已禁用".to_owned(),
      map.into(),
