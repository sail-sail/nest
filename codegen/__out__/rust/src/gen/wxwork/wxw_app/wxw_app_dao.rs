...
async fn get_where_query(
  ...
  // 可信域名
  {
    ...
    if let Some(domain_id) = domain_id {
      ...
      where_query.push_str(" and domain_id_lbl.id in (");
      where_query.push_str(&arg);
      where_query.push(')');
    }
  }
  {
    ...
    if domain_id_is_null {
      where_query.push_str(" and domain_id_lbl.id is null");
    }
  }
  // 锁定
  {
...
pub async fn find_all(
  ...
  // 可信域名
  if let Some(search) = &search {
    if search.domain_id.is_some() && search.domain_id.as_ref().unwrap().is_empty() {
      return Ok(vec![]);
    }
  }
  // 锁定
...
pub async fn create(
  ...
  let mut sql_values = String::with_capacity(2 * 17 + 2);
