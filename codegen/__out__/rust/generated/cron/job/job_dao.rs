...
async fn get_where_query(
  ...
  {
    let ids: Option<Vec<JobId>> = match search {
...
async fn get_from_query(
  ...
  let from_query = r#"cron_job t"#.to_owned();
  Ok(from_query)
}
...
pub async fn find_all_job(
  ...
  let mut res: Vec<JobModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
pub async fn get_field_comments_job(
  ...
  let mut field_comments = JobFieldComment {
    id: "ID".into(),
    code: "编码".into(),
    lbl: "名称".into(),
    is_locked: "锁定".into(),
...
pub async fn find_one_job(
  ...
  let model: Option<JobModel> = res.into_iter().next();
...
async fn _creates(
  ...
  let mut ids2: Vec<JobId> = vec![];
  let mut inputs2: Vec<JobInput> = vec![];
    ...
    .enumerate()
  {
    ...
    let id: JobId = get_short_uuid().into();
    ids2.push(id);
...
// MARK: sync_usr_lbl_by_usr_id_job
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_job(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_job();
  let method = "sync_usr_lbl_by_usr_id_job";
...
pub async fn delete_by_ids_job(
  ...
  let old_models = find_by_ids_ok_job(
    ids.clone(),
    options,
  ).await?;
