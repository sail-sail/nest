...
static CAN_SORT_IN_API_DEPT: [&str; 3] = [
  "order_by",
  "create_time",
  "update_time",
];
...
impl FromRow<'_, MySqlRow> for DeptModel {
  fn from_row(row: &MySqlRow) -> sqlx::Result<Self> {
    ...
    let usr_ids = {
      ...
      keys.into_iter()
        .map(|x| 
          usr_ids.get(x.to_string().as_str())
            .unwrap_or(&UsrId::default())
            .to_owned()
        )
        .collect::<Vec<UsrId>>()
    };
    let usr_ids_lbl: Option<sqlx::types::Json<HashMap<&str, &str>>> = row.try_get("usr_ids_lbl")?;
...
impl std::fmt::Debug for DeptInput {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let mut item = &mut f.debug_struct("DeptInput");
    if let Some(ref id) = self.id {
      item = item.field("id", id);
    }
    if let Some(ref is_deleted) = self.is_deleted {
    ...
    if let Some(ref usr_ids_lbl) = self.usr_ids_lbl {
      item = item.field("usr_ids_lbl", usr_ids_lbl);
    }
    if let Some(ref is_locked) = self.is_locked {
      item = item.field("is_locked", is_locked);
    }
