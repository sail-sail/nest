...
impl std::fmt::Debug for I18nSearch {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let mut item = &mut f.debug_struct("I18nSearch");
...
pub struct I18nId([u8; 22]);

impl Serialize for I18nId {
...
impl<'de> Deserialize<'de> for I18nId {
...
impl fmt::Debug for I18nId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "I18nId({s})"),
      Err(_) => write!(f, "I18nId()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("I18nId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl<'r> sqlx::Decode<'r, MySql> for I18nId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("I18nId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}

impl PartialEq<str> for I18nId {
...
pub fn check_sort_i18n(
  ...
  for item in sort {
    ...
    if !get_can_sort_in_api_i18n.contains(&prop) {
      return Err(eyre!("check_sort_i18n: {}", serde_json::to_string(item)?));
    }
  }
