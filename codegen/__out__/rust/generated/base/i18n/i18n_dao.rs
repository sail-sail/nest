...
pub async fn find_all_i18n(
  ...
  let res = {
    ...
    if let Some(str) = str {
      let res2: Vec<I18nModel>;
      let res = serde_json::from_str::<Vec<I18nModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
  ...
  let mut res: Vec<I18nModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
pub async fn create_return_i18n(
  ...
  let model_i18n = match model_i18n {
    Some(model) => model,
    None => {
...
pub async fn revert_by_ids_i18n(
  ...
  for id in ids.clone() {
    ...
    
    {
      ...
      let models: Vec<I18nModel> = models
        .into_iter()
        .filter(|item| 
          item.id != id
        )
