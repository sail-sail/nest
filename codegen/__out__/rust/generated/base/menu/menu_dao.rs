...
pub async fn find_all_menu(
  ...
  let mut res: Vec<MenuModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
// MARK: sync_usr_lbl_by_usr_id_menu
/// 根据 usr_id 同步创建人/更新人标签
pub async fn sync_usr_lbl_by_usr_id_menu(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_menu();
  let method = "sync_usr_lbl_by_usr_id_menu";
