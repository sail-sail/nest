...
static CAN_SORT_IN_API_DICT: [&str; 3] = [
  "order_by",
  "create_time",
  "update_time",
];
...
impl FromRow<'_, MySqlRow> for DictModel {
  fn from_row(row: &MySqlRow) -> sqlx::Result<Self> {
    ...
    let r#type: DictType = type_lbl.try_into()?;
...
#[graphql(rename_fields = "snake_case", name = "DictFieldComment")]
#[allow(dead_code)]
...
pub struct DictInput {
  ...
  /// 系统字典明细
  #[graphql(name = "dict_detail")]
  pub dict_detail: Option<Vec<DictDetailInput>>,
}

impl std::fmt::Debug for DictInput {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let mut item = &mut f.debug_struct("DictInput");
    if let Some(ref id) = self.id {
      item = item.field("id", id);
    }
    if let Some(ref is_deleted) = self.is_deleted {
...
impl FromStr for DictType {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    match s {
      ...
      _ => Err(eyre!("{s} 无法转换到 数据类型")),
    }
  }
}

impl TryFrom<&str> for DictType {
...
impl TryFrom<SmolStr> for DictType {
  type Error = sqlx::Error;
  ...
  for item in sort {
    ...
    if !get_can_sort_in_api_dict.contains(&prop) {
      return Err(eyre!(ServiceException {
        message: format!("check_sort_dict: {}", serde_json::to_string(item)?).into(),
        trace: true,
        ..Default::default()
      }));
    }
