...
// 系统字典明细
use crate::base::dict_detail::dict_detail_dao::{
...
async fn get_where_query(
  ...
  {
    let ids: Option<Vec<DictId>> = match search {
  ...
  // 数据类型
  {
    let r#type: Option<Vec<DictType>> = match search {
...
pub async fn find_all_dict(
  ...
  let res = {
    ...
    if let Some(str) = str {
      let res2: Vec<DictModel>;
      let res = serde_json::from_str::<Vec<DictModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
  ...
  let mut res: Vec<DictModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
pub async fn find_by_id_ok_dict(
  ...
  let dict_model = find_by_id_dict(
    id,
    options,
  ).await?;
...
pub async fn find_by_ids_ok_dict(
  ...
  let dict_models = find_by_ids_dict(
    ids.clone(),
    options,
  ).await?;
...
// MARK: exists_by_id_dict
/// 根据 id 判断系统字典是否存在
#[allow(dead_code)]
pub async fn exists_by_id_dict(
  id: DictId,
  options: Option<Options>,
) -> Result<bool> {
...
// MARK: find_by_unique_dict
/// 通过唯一约束获得数据列表
#[allow(unused_variables)]
pub async fn find_by_unique_dict(
  search: DictSearch,
  sort: Option<Vec<SortInput>>,
  options: Option<Options>,
) -> Result<Vec<DictModel>> {
  ...
  let mut models: Vec<DictModel> = vec![];
...
// MARK: check_by_unique_dict
/// 通过唯一约束检查数据是否已经存在
#[allow(unused_variables)]
pub async fn check_by_unique_dict(
  input: DictInput,
  model: DictModel,
  options: Option<Options>,
...
// MARK: creates_return_dict
/// 批量创建系统字典并返回
#[allow(dead_code)]
pub async fn creates_return_dict(
  ...
  let models_dict = find_by_ids_dict(
    ids,
    options,
  ).await?;
...
async fn _creates(
  ...
  for input in inputs {
    ...
    let old_models = find_by_unique_dict(
      input.clone().into(),
      None,
      options,
    ).await?;
    ...
    .enumerate()
  {
    ...
    let id: DictId = get_short_uuid().into();
    ids2.push(id);
...
// MARK: create_return_dict
/// 创建系统字典并返回
#[allow(dead_code)]
pub async fn create_return_dict(
  #[allow(unused_mut)]
  mut input: DictInput,
  options: Option<Options>,
) -> Result<DictModel> {
  ...
  let model_dict = find_by_id_dict(
    id,
    options,
  ).await?;
  
  let model_dict = match model_dict {
    Some(model) => model,
    None => {
      let err_msg = "create_return_dict: model_dict.is_none()";
      return Err(eyre!(
        ServiceException {
...
// MARK: sync_usr_lbl_by_usr_id_dict
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_dict(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_dict();
  let method = "sync_usr_lbl_by_usr_id_dict";
...
pub async fn update_by_id_dict(
  ...
  
  {
    ...
    let models = find_by_unique_dict(
      input.into(),
      None,
      options,
    ).await?;
  ...
  if let Some(dict_detail_input) = input.dict_detail {
    ...
    for mut input2 in dict_detail_input {
...
// MARK: update_by_id_return_dict
/// 根据 id 更新系统字典, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_dict(
  id: DictId,
  input: DictInput,
  options: Option<Options>,
) -> Result<DictModel> {
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "系统字典 update_by_id_return_dict id: {id}",
    )),
  }
}
...
/// 清空缓存
#[allow(dead_code)]
pub async fn del_cache_dict() -> Result<()> {
...
// MARK: get_is_enabled_by_id_dict
/// 根据 id 查找系统字典是否已启用
/// 记录不存在则返回 false
...
pub async fn revert_by_ids_dict(
  ...
  for id in ids.clone() {
    ...
    let mut old_model = find_one_dict(
      DictSearch {
        id: Some(id),
        is_deleted: Some(1),
        ..Default::default()
...
// MARK: validate_is_enabled_dict
/// 校验系统字典是否启用
#[allow(dead_code)]
pub async fn validate_is_enabled_dict(
  model: &DictModel,
) -> Result<()> {
