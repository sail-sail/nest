...
pub async fn find_all_usr(
  ...
  let mut res: Vec<UsrModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
// MARK: sync_usr_lbl_by_usr_id_usr
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_usr(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_usr();
  let method = "sync_usr_lbl_by_usr_id_usr";
