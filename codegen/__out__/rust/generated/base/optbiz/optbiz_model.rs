...
static CAN_SORT_IN_API_OPTBIZ: OnceLock<[&'static str; 3]> = OnceLock::new();

/// 业务选项 前端允许排序的字段
fn get_can_sort_in_api_optbiz() -> &'static [&'static str; 3] {
  CAN_SORT_IN_API_OPTBIZ.get_or_init(|| [
    "order_by",
    "create_time",
    "update_time",
  ])
...
#[graphql(rename_fields = "snake_case", name = "OptbizFieldComment")]
#[allow(dead_code)]
...
pub struct OptbizId([u8; 22]);

impl Serialize for OptbizId {
...
impl<'de> Deserialize<'de> for OptbizId {
...
impl fmt::Debug for OptbizId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "OptbizId({s})"),
      Err(_) => write!(f, "OptbizId()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("OptbizId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl<'r> sqlx::Decode<'r, MySql> for OptbizId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("OptbizId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}
...
pub fn check_sort_optbiz(
  ...
  let get_can_sort_in_api_optbiz = get_can_sort_in_api_optbiz();
