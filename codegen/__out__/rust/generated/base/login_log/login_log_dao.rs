...
pub async fn find_all_login_log(
  ...
  // 登录成功
  if let Some(search) = &search && let Some(is_succ) = &search.is_succ {
    let len = is_succ.len();
    if len == 0 {
      return Ok(vec![]);
    }
...
pub async fn find_count_login_log(
  ...
  // 登录成功
  if let Some(search) = &search && search.is_succ.is_some() {
...
// MARK: sync_usr_lbl_by_usr_id_login_log
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_login_log(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_login_log();
  let method = "sync_usr_lbl_by_usr_id_login_log";
