...
pub async fn find_all_dict_detail(
  ...
  let res = {
    ...
    if let Some(str) = str {
      let res2: Vec<DictDetailModel>;
      let res = serde_json::from_str::<Vec<DictDetailModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
  ...
  let mut res: Vec<DictDetailModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
// MARK: sync_usr_lbl_by_usr_id_dict_detail
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_dict_detail(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_dict_detail();
  let method = "sync_usr_lbl_by_usr_id_dict_detail";
...
// MARK: update_by_id_return_dict_detail
/// 根据 id 更新系统字典明细, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_dict_detail(
  id: DictDetailId,
  input: DictDetailInput,
  options: Option<Options>,
) -> Result<DictDetailModel> {
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "系统字典明细 update_by_id_return_dict_detail id: {id}",
    )),
  }
}
