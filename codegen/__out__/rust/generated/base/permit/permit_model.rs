...
static CAN_SORT_IN_API_PERMIT: [&str; 1] = [
  "order_by",
];

/// 按钮权限 前端允许排序的字段
fn get_can_sort_in_api_permit() -> &'static [&'static str; 1] {
  &CAN_SORT_IN_API_PERMIT
}
...
impl std::fmt::Debug for PermitInput {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let mut item = &mut f.debug_struct("PermitInput");
    if let Some(ref id) = self.id {
      item = item.field("id", id);
    }
  ...
  for item in sort {
    ...
    if !get_can_sort_in_api_permit.contains(&prop) {
      return Err(eyre!(ServiceException {
        message: format!("check_sort_permit: {}", serde_json::to_string(item)?).into(),
        trace: true,
        ..Default::default()
      }));
    }
