...
pub struct FieldPermitId([u8; 22]);

impl Serialize for FieldPermitId {
...
impl<'de> Deserialize<'de> for FieldPermitId {
...
impl fmt::Debug for FieldPermitId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "FieldPermitId({s})"),
      Err(_) => write!(f, "FieldPermitId()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("FieldPermitId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl From<[u8; 22]> for FieldPermitId {
...
impl From<FieldPermitId> for [u8; 22] {
  fn from(id: FieldPermitId) -> Self {
    id.0
  }
}
...
impl<'r> sqlx::Decode<'r, MySql> for FieldPermitId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("FieldPermitId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}
