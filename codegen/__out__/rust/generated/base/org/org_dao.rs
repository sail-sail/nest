...
async fn get_where_query(
  ...
  {
    let ids: Option<Vec<OrgId>> = match search {
...
async fn get_from_query(
  ...
  let from_query = r#"base_org t"#.to_owned();
  Ok(from_query)
}
...
pub async fn find_all_org(
  ...
  let res = {
    ...
    if let Some(str) = str {
      let res2: Vec<OrgModel>;
      let res = serde_json::from_str::<Vec<OrgModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
  ...
  let mut res: Vec<OrgModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
pub async fn find_by_id_ok_org(
  ...
  let Some(org_model) = org_model else {
    let err_msg = SmolStr::new("此 组织 已被删除");
    error!(
...
pub async fn find_by_ids_ok_org(
  ...
  let org_models = ids
    ...
    .map(|id| {
      ...
      let err_msg = SmolStr::new("此 组织 已经被删除");
      Err(eyre!(err_msg))
    })
...
// MARK: exists_by_id_org
/// 根据 id 判断组织是否存在
#[allow(dead_code)]
pub async fn exists_by_id_org(
  id: OrgId,
  options: Option<Options>,
) -> Result<bool> {
...
// MARK: find_by_unique_org
/// 通过唯一约束获得数据列表
#[allow(unused_variables)]
pub async fn find_by_unique_org(
  search: OrgSearch,
  sort: Option<Vec<SortInput>>,
  options: Option<Options>,
) -> Result<Vec<OrgModel>> {
...
// MARK: check_by_unique_org
/// 通过唯一约束检查数据是否已经存在
#[allow(unused_variables)]
pub async fn check_by_unique_org(
  input: OrgInput,
  model: OrgModel,
  options: Option<Options>,
) -> Result<Option<OrgId>> {
...
// MARK: creates_return_org
/// 批量创建组织并返回
#[allow(dead_code)]
pub async fn creates_return_org(
  inputs: Vec<OrgInput>,
  options: Option<Options>,
) -> Result<Vec<OrgModel>> {
  ...
  let models_org = find_by_ids_org(
    ids,
    options,
  ).await?;
...
async fn _creates(
  ...
  let mut ids2: Vec<OrgId> = vec![];
  let mut inputs2: Vec<OrgInput> = vec![];
    ...
    .enumerate()
  {
    ...
    let id: OrgId = get_short_uuid().into();
    ids2.push(id);
...
// MARK: create_return_org
/// 创建组织并返回
#[allow(dead_code)]
pub async fn create_return_org(
  #[allow(unused_mut)]
  mut input: OrgInput,
  options: Option<Options>,
) -> Result<OrgModel> {
  ...
  let model_org = find_by_id_org(
    id,
    options,
  ).await?;
  
  let model_org = match model_org {
    Some(model) => model,
    None => {
...
// MARK: sync_usr_lbl_by_usr_id_org
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_org(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_org();
  let method = "sync_usr_lbl_by_usr_id_org";
...
/// 清空缓存
#[allow(dead_code)]
pub async fn del_cache_org() -> Result<()> {
...
pub async fn revert_by_ids_org(
  ...
  for id in ids.clone() {
    ...
    let mut old_model = find_one_org(
      OrgSearch {
        id: Some(id),
        is_deleted: Some(1),
        ..Default::default()
    ...
    
    {
      ...
      let models: Vec<OrgModel> = models
        .into_iter()
        .filter(|item| 
          item.id != id
        )
...
// MARK: validate_is_enabled_org
/// 校验组织是否启用
#[allow(dead_code)]
pub async fn validate_is_enabled_org(
  model: &OrgModel,
) -> Result<()> {
  if model.is_enabled == 0 {
    let err_msg = SmolStr::new("组织已禁用");
    return Err(eyre!(err_msg));
  }
  Ok(())
}

// MARK: validate_option_org
/// 校验组织是否存在
#[allow(dead_code)]
