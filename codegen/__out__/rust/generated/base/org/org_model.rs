...
static CAN_SORT_IN_API_ORG: OnceLock<[&'static str; 3]> = OnceLock::new();

/// 组织 前端允许排序的字段
fn get_can_sort_in_api_org() -> &'static [&'static str; 3] {
  CAN_SORT_IN_API_ORG.get_or_init(|| [
    "order_by",
    "create_time",
    "update_time",
  ])
...
#[graphql(rename_fields = "snake_case", name = "OrgFieldComment")]
#[allow(dead_code)]
...
#[graphql(rename_fields = "snake_case", name = "OrgSearch")]
#[allow(dead_code)]
...
impl std::fmt::Debug for OrgSearch {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let mut item = &mut f.debug_struct("OrgSearch");
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("OrgId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl<'r> sqlx::Decode<'r, MySql> for OrgId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("OrgId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}
...
pub fn check_sort_org(
  ...
  let get_can_sort_in_api_org = get_can_sort_in_api_org();
  ...
  for item in sort {
    ...
    if !get_can_sort_in_api_org.contains(&prop) {
      return Err(eyre!("check_sort_org: {}", serde_json::to_string(item)?));
    }
  }
