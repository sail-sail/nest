...
static CAN_SORT_IN_API_ICON: OnceLock<[&'static str; 3]> = OnceLock::new();

/// 图标库 前端允许排序的字段
fn get_can_sort_in_api_icon() -> &'static [&'static str; 3] {
  CAN_SORT_IN_API_ICON.get_or_init(|| [
    "order_by",
    "create_time",
    "update_time",
  ])
...
#[graphql(rename_fields = "snake_case", name = "IconModel")]
#[allow(dead_code)]
...
impl FromRow<'_, MySqlRow> for IconModel {
...
pub struct IconSearch {
  /// ID
  pub id: Option<IconId>,
  /// ID列表
  pub ids: Option<Vec<IconId>>,
...
impl std::fmt::Debug for IconSearch {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let mut item = &mut f.debug_struct("IconSearch");
...
#[graphql(rename_fields = "snake_case", name = "IconInput")]
#[allow(dead_code)]
...
impl From<IconModel> for IconInput {
  fn from(model: IconModel) -> Self {
    Self {
      id: model.id.into(),
...
impl From<IconInput> for IconSearch {
  fn from(input: IconInput) -> Self {
    Self {
      id: input.id,
      ids: None,
      is_deleted: None,
...
impl<'de> Deserialize<'de> for IconId {
...
impl fmt::Debug for IconId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "IconId({s})"),
      Err(_) => write!(f, "IconId()")
    }
  }
}
...
#[async_graphql::Scalar(name = "IconId")]
impl async_graphql::ScalarType for IconId {
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("IconId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl Encode<'_, MySql> for IconId {
...
impl sqlx::Type<MySql> for IconId {
...
impl<'r> sqlx::Decode<'r, MySql> for IconId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("IconId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}

impl PartialEq<str> for IconId {
...
pub fn check_sort_icon(
  ...
  let get_can_sort_in_api_icon = get_can_sort_in_api_icon();
  ...
  for item in sort {
    ...
    if !get_can_sort_in_api_icon.contains(&prop) {
      return Err(eyre!("check_sort_icon: {}", serde_json::to_string(item)?));
    }
  }
