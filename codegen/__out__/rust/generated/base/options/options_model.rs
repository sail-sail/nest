...
pub struct OptionsId([u8; 22]);

impl Serialize for OptionsId {
...
impl<'de> Deserialize<'de> for OptionsId {
...
impl fmt::Debug for OptionsId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "OptionsId({s})"),
      Err(_) => write!(f, "OptionsId()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("OptionsId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl<'r> sqlx::Decode<'r, MySql> for OptionsId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("OptionsId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}

impl PartialEq<str> for OptionsId {
