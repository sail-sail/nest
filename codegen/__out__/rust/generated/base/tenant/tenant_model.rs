...
impl fmt::Debug for TenantId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "TenantId({s})"),
      Err(_) => write!(f, "TenantId()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("TenantId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl<'r> sqlx::Decode<'r, MySql> for TenantId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("TenantId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}
