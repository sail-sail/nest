...
static CAN_SORT_IN_API_LANG: OnceLock<[&'static str; 3]> = OnceLock::new();

/// 语言 前端允许排序的字段
fn get_can_sort_in_api_lang() -> &'static [&'static str; 3] {
  CAN_SORT_IN_API_LANG.get_or_init(|| [
    "order_by",
    "create_time",
    "update_time",
  ])
...
#[graphql(rename_fields = "snake_case", name = "LangFieldComment")]
#[allow(dead_code)]
...
impl std::fmt::Debug for LangSearch {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let mut item = &mut f.debug_struct("LangSearch");
...
impl fmt::Debug for LangId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "LangId({s})"),
      Err(_) => write!(f, "LangId()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("LangId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl<'r> sqlx::Decode<'r, MySql> for LangId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("LangId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}
...
pub fn check_sort_lang(
  ...
  let get_can_sort_in_api_lang = get_can_sort_in_api_lang();
  ...
  for item in sort {
    ...
    if !get_can_sort_in_api_lang.contains(&prop) {
      return Err(eyre!("check_sort_lang: {}", serde_json::to_string(item)?));
    }
  }
