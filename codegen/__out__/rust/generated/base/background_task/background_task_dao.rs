...
pub async fn find_all_background_task(
  ...
  // 状态
  if let Some(search) = &search && let Some(state) = &search.state {
    let len = state.len();
    if len == 0 {
      return Ok(vec![]);
    }
...
// MARK: update_by_id_return_background_task
/// 根据 id 更新后台任务, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_background_task(
  id: BackgroundTaskId,
  input: BackgroundTaskInput,
  options: Option<Options>,
) -> Result<BackgroundTaskModel> {
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "后台任务 update_by_id_return_background_task id: {id}",
    )),
  }
}
