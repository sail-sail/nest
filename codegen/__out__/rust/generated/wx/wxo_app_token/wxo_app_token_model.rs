...
pub struct WxoAppTokenId([u8; 22]);

impl Serialize for WxoAppTokenId {
...
impl<'de> Deserialize<'de> for WxoAppTokenId {
...
impl fmt::Debug for WxoAppTokenId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "WxoAppTokenId({s})"),
      Err(_) => write!(f, "WxoAppTokenId()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("WxoAppTokenId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl From<[u8; 22]> for WxoAppTokenId {
...
impl From<WxoAppTokenId> for [u8; 22] {
  fn from(id: WxoAppTokenId) -> Self {
    id.0
  }
}
...
impl<'r> sqlx::Decode<'r, MySql> for WxoAppTokenId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("WxoAppTokenId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}
