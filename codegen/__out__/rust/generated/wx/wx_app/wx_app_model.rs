...
fn get_can_sort_in_api_wx_app() -> &'static [&'static str; 3] {
  CAN_SORT_IN_API_WX_APP.get_or_init(|| [
    "order_by",
    "create_time",
    "update_time",
  ])
...
pub struct WxAppId([u8; 22]);

impl Serialize for WxAppId {
...
impl<'de> Deserialize<'de> for WxAppId {
...
impl fmt::Debug for WxAppId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "WxAppId({s})"),
      Err(_) => write!(f, "WxAppId()")
    }
  }
}

impl fmt::Display for WxAppId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "{s}"),
      Err(_) => write!(f, "")
...
#[async_graphql::Scalar(name = "WxAppId")]
impl async_graphql::ScalarType for WxAppId {
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("WxAppId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl From<WxAppId> for ArgType {
  fn from(value: WxAppId) -> Self {
...
impl From<WxAppId> for SmolStr {
  fn from(id: WxAppId) -> Self {
    std::str::from_utf8(&id.0).unwrap_or("").into()
  }
}

impl From<SmolStr> for WxAppId {
...
impl From<String> for WxAppId {
  fn from(s: String) -> Self {
    s.as_str().into()
  }
}

impl From<[u8; 22]> for WxAppId {
...
impl From<WxAppId> for [u8; 22] {
  fn from(id: WxAppId) -> Self {
    id.0
  }
}

impl From<&str> for WxAppId {
  fn from(s: &str) -> Self {
    let bytes = s.as_bytes();
    let mut arr = [0u8; 22];
    if bytes.len() == 22 {
...
impl Encode<'_, MySql> for WxAppId {
...
impl sqlx::Type<MySql> for WxAppId {
...
impl<'r> sqlx::Decode<'r, MySql> for WxAppId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("WxAppId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}

impl PartialEq<str> for WxAppId {
...
pub fn check_sort_wx_app(
  ...
  for item in sort {
    ...
    if !get_can_sort_in_api_wx_app.contains(&prop) {
      return Err(eyre!("check_sort_wx_app: {}", serde_json::to_string(item)?));
    }
  }
