...
pub async fn find_all_wxo_app(
  ...
  // 消息加解密方式
  if let Some(search) = &search && let Some(encoding_type) = &search.encoding_type {
    let len = encoding_type.len();
    if len == 0 {
      return Ok(vec![]);
    }
    if len > ids_limit {
  ...
  let res = {
    ...
    if let Some(str) = str {
      ...
      let res = serde_json::from_str::<Vec<WxoAppModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
...
// MARK: sync_usr_lbl_by_usr_id_wxo_app
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_wxo_app(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_wxo_app();
  let method = "sync_usr_lbl_by_usr_id_wxo_app";
...
// MARK: update_by_id_return_wxo_app
/// 根据 id 更新公众号设置, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_wxo_app(
  id: WxoAppId,
  input: WxoAppInput,
  options: Option<Options>,
) -> Result<WxoAppModel> {
  
  update_by_id_wxo_app(
    id,
    input,
    options,
  ).await?;
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "公众号设置 update_by_id_return_wxo_app id: {id}",
    )),
  }
}
