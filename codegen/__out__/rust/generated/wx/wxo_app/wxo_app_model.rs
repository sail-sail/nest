...
pub struct WxoAppId([u8; 22]);

impl Serialize for WxoAppId {
...
impl<'de> Deserialize<'de> for WxoAppId {
...
impl fmt::Debug for WxoAppId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "WxoAppId({s})"),
      Err(_) => write!(f, "WxoAppId()")
    }
  }
}

impl fmt::Display for WxoAppId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "{s}"),
      Err(_) => write!(f, "")
...
#[async_graphql::Scalar(name = "WxoAppId")]
impl async_graphql::ScalarType for WxoAppId {
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("WxoAppId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl From<WxoAppId> for ArgType {
  fn from(value: WxoAppId) -> Self {
...
impl From<WxoAppId> for SmolStr {
  fn from(id: WxoAppId) -> Self {
    std::str::from_utf8(&id.0).unwrap_or("").into()
  }
}

impl From<SmolStr> for WxoAppId {
...
impl From<String> for WxoAppId {
  fn from(s: String) -> Self {
    s.as_str().into()
  }
}

impl From<[u8; 22]> for WxoAppId {
...
impl From<WxoAppId> for [u8; 22] {
  fn from(id: WxoAppId) -> Self {
    id.0
  }
}

impl From<&str> for WxoAppId {
  fn from(s: &str) -> Self {
    let bytes = s.as_bytes();
    let mut arr = [0u8; 22];
    if bytes.len() == 22 {
...
impl Encode<'_, MySql> for WxoAppId {
...
impl sqlx::Type<MySql> for WxoAppId {
...
impl<'r> sqlx::Decode<'r, MySql> for WxoAppId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("WxoAppId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}

impl PartialEq<str> for WxoAppId {
