...
pub async fn get_field_comments_pay_transactions_jsapi(
  ...
  let mut field_comments = PayTransactionsJsapiFieldComment {
    id: "ID".into(),
    appid: "开发者ID".into(),
    mchid: "商户号".into(),
...
pub async fn find_by_id_ok_pay_transactions_jsapi(
  ...
  let Some(pay_transactions_jsapi_model) = pay_transactions_jsapi_model else {
    let err_msg = SmolStr::new("此 微信JSAPI下单 已被删除");
    error!(
...
pub async fn find_by_ids_ok_pay_transactions_jsapi(
  ...
  let pay_transactions_jsapi_models = ids
    ...
    .map(|id| {
      ...
      let err_msg = SmolStr::new("此 微信JSAPI下单 已经被删除");
      Err(eyre!(err_msg))
    })
...
// MARK: sync_usr_lbl_by_usr_id_pay_transactions_jsapi
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_pay_transactions_jsapi(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_pay_transactions_jsapi();
  let method = "sync_usr_lbl_by_usr_id_pay_transactions_jsapi";
...
pub async fn update_by_id_pay_transactions_jsapi(
  ...
  // 交易限制时间
  if let Some(time_expire) = input.time_expire.clone() {
    field_num += 1;
    sql_fields += "time_expire=?,";
    args.push(time_expire.into());
  }
  ...
  // 分账
  if let Some(profit_sharing) = input.profit_sharing.clone() {
    field_num += 1;
    sql_fields += "profit_sharing=?,";
    args.push(profit_sharing.into());
  }
  ...
  // 预支付交易会话标识
  if let Some(prepay_id) = input.prepay_id.clone() {
    field_num += 1;
    sql_fields += "prepay_id=?,";
    args.push(prepay_id.into());
  }
...
// MARK: update_by_id_return_pay_transactions_jsapi
/// 根据 id 更新微信JSAPI下单, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_pay_transactions_jsapi(
  id: PayTransactionsJsapiId,
  input: PayTransactionsJsapiInput,
  options: Option<Options>,
) -> Result<PayTransactionsJsapiModel> {
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "微信JSAPI下单 update_by_id_return_pay_transactions_jsapi id: {id}",
    )),
  }
}
...
pub async fn force_delete_by_ids_pay_transactions_jsapi(
  ...
  for id in ids.clone() {
    ...
    let old_model = find_one_pay_transactions_jsapi(
...
pub async fn validate_option_pay_transactions_jsapi(
  ...
  let model = match model {
    Some(model) => model,
    None => {
      let err_msg = SmolStr::new("微信JSAPI下单不存在");
      error!(
        "{req_id} {err_msg}",
        req_id = get_req_id(),
      );
