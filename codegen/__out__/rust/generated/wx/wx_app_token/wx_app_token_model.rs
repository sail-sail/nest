...
#[graphql(rename_fields = "snake_case", name = "WxAppTokenFieldComment")]
#[allow(dead_code)]
...
impl std::fmt::Debug for WxAppTokenSearch {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    ...
    if let Some(ref wx_app_id_lbl) = self.wx_app_id_lbl {
      item = item.field("wx_app_id_lbl", wx_app_id_lbl);
    }
    if let Some(ref wx_app_id_lbl_like) = self.wx_app_id_lbl_like {
      item = item.field("wx_app_id_lbl_like", wx_app_id_lbl_like);
    }
...
pub struct WxAppTokenId([u8; 22]);

impl Serialize for WxAppTokenId {
...
impl<'de> Deserialize<'de> for WxAppTokenId {
...
impl fmt::Debug for WxAppTokenId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "WxAppTokenId({s})"),
      Err(_) => write!(f, "WxAppTokenId()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("WxAppTokenId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl From<[u8; 22]> for WxAppTokenId {
...
impl From<WxAppTokenId> for [u8; 22] {
  fn from(id: WxAppTokenId) -> Self {
    id.0
  }
}
...
impl<'r> sqlx::Decode<'r, MySql> for WxAppTokenId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("WxAppTokenId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}
