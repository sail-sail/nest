...
pub async fn find_all_wxo_usr(
  ...
  let table = get_table_name_wxo_usr();
  ...
  // 性别
  if let Some(search) = &search && let Some(sex) = &search.sex {
    let len = sex.len();
    if len == 0 {
      return Ok(vec![]);
    }
  ...
  let mut res: Vec<WxoUsrModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
pub async fn find_count_wxo_usr(
  ...
  // 性别
  if let Some(search) = &search && search.sex.is_some() {
...
pub async fn get_field_comments_wxo_usr(
  ...
  let mut field_comments = WxoUsrFieldComment {
    id: "ID".into(),
    lbl: "昵称".into(),
    head_img: "头像".into(),
    usr_id: "绑定用户".into(),
...
pub async fn find_by_ids_ok_wxo_usr(
  ...
  let table = get_table_name_wxo_usr();
  let method = "find_by_ids_ok_wxo_usr";
  ...
  let wxo_usr_models = find_by_ids_wxo_usr(
    ids.clone(),
    options,
  ).await?;
...
pub async fn find_by_ids_wxo_usr(
  ...
  let wxo_usr_models = find_all_wxo_usr(
    search,
    None,
    None,
    options,
...
pub async fn create_return_wxo_usr(
  ...
  let model_wxo_usr = match model_wxo_usr {
    Some(model) => model,
    None => {
...
// MARK: sync_usr_lbl_by_usr_id_wxo_usr
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_wxo_usr(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_wxo_usr();
  let method = "sync_usr_lbl_by_usr_id_wxo_usr";
...
pub async fn update_by_id_wxo_usr(
  ...
  // 头像
  if let Some(head_img) = input.head_img.clone() {
    field_num += 1;
    sql_fields += "head_img=?,";
    args.push(head_img.into());
  }
  ...
  // 特权
  if let Some(privilege) = input.privilege.clone() {
    field_num += 1;
    sql_fields += "privilege=?,";
    args.push(privilege.into());
  }
...
// MARK: update_by_id_return_wxo_usr
/// 根据 id 更新公众号用户, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_wxo_usr(
  id: WxoUsrId,
  input: WxoUsrInput,
  options: Option<Options>,
) -> Result<WxoUsrModel> {
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "公众号用户 update_by_id_return_wxo_usr id: {id}",
    )),
  }
}
...
pub async fn force_delete_by_ids_wxo_usr(
  ...
  for id in ids.clone() {
    ...
    let old_model = find_one_wxo_usr(
      Some(WxoUsrSearch {
        id: Some(id),
        is_deleted: Some(1),
        ..Default::default()
