...
pub struct WxoUsrId([u8; 22]);

impl Serialize for WxoUsrId {
...
impl<'de> Deserialize<'de> for WxoUsrId {
...
impl fmt::Debug for WxoUsrId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "WxoUsrId({s})"),
      Err(_) => write!(f, "WxoUsrId()")
    }
  }
}

impl fmt::Display for WxoUsrId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "{s}"),
      Err(_) => write!(f, "")
...
#[async_graphql::Scalar(name = "WxoUsrId")]
impl async_graphql::ScalarType for WxoUsrId {
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("WxoUsrId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl From<WxoUsrId> for ArgType {
  fn from(value: WxoUsrId) -> Self {
...
impl From<WxoUsrId> for SmolStr {
  fn from(id: WxoUsrId) -> Self {
    std::str::from_utf8(&id.0).unwrap_or("").into()
  }
}

impl From<SmolStr> for WxoUsrId {
...
impl From<String> for WxoUsrId {
  fn from(s: String) -> Self {
    s.as_str().into()
  }
}

impl From<[u8; 22]> for WxoUsrId {
...
impl From<WxoUsrId> for [u8; 22] {
  fn from(id: WxoUsrId) -> Self {
    id.0
  }
}

impl From<&str> for WxoUsrId {
  fn from(s: &str) -> Self {
    let bytes = s.as_bytes();
    let mut arr = [0u8; 22];
    if bytes.len() == 22 {
...
impl Encode<'_, MySql> for WxoUsrId {
...
impl sqlx::Type<MySql> for WxoUsrId {
...
impl<'r> sqlx::Decode<'r, MySql> for WxoUsrId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("WxoUsrId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}

impl PartialEq<str> for WxoUsrId {
