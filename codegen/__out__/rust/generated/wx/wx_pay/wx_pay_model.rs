...
pub struct WxPayId([u8; 22]);

impl Serialize for WxPayId {
...
impl<'de> Deserialize<'de> for WxPayId {
...
impl fmt::Debug for WxPayId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "WxPayId({s})"),
      Err(_) => write!(f, "WxPayId()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("WxPayId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl<'r> sqlx::Decode<'r, MySql> for WxPayId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("WxPayId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}
