...
async fn _creates(
  ...
  for input in inputs {
    ...
    // 处理人
    if (input.assignee_usr_id_lbl.is_none() || input.assignee_usr_id_lbl.as_ref().unwrap().is_empty())
      && input.assignee_usr_id.is_some()
      && !input.assignee_usr_id.as_ref().unwrap().is_empty()
    {
      let usr_model = crate::base::usr::usr_dao::find_by_id_usr(
        input.assignee_usr_id.clone().unwrap(),
        Some(Options::new().set_is_debug(Some(false))),
      ).await?;
      if let Some(usr_model) = usr_model {
        input.assignee_usr_id_lbl = usr_model.lbl.into();
...
// MARK: sync_usr_lbl_by_usr_id_task
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_task(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_task();
  let method = "sync_usr_lbl_by_usr_id_task";
