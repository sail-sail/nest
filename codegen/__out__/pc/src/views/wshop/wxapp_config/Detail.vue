...
type OnCloseResolveType = {
  type: "ok" | "cancel";
  changedIds: WxappConfigId[];
};

let onCloseResolve = function(_value: OnCloseResolveType) { };

/** 内置变量 */
let builtInModel = $ref<WxappConfigInput>();
...
async function onPageUp(e?: KeyboardEvent) {
  ...
  if (!isSucc) {
    ElMessage.warning(await nsAsync("已经是第一个 {0} 了", await nsAsync("小程序配置")));
  }
}
