            ...
            >
              <template #default="{ row }">
                <CustomSwitch
                  ...
                  v-model="row.is_home"
                  @change="onIs_home(row.id, row.is_home)"
                ></CustomSwitch>
              </template>
            </el-table-column>
          </template>
            ...
            >
              <template #default="{ row }">
                <CustomSwitch
                  ...
                  @change="onIs_recommend(row.id, row.is_recommend)"
                ></CustomSwitch>
              </template>
            </el-table-column>
          </template>
...
import {
  ...
  enableByIdsPtType,
  lockByIdsPtType,
  useExportExcelPtType,
  updateByIdPtType,
  importModelsPtType,
  useDownloadImportTemplatePtType,
} from "./Api.ts";
...
const pagePath = getPagePathPtType();
...
/** 内置查询条件 */
const builtInSearch: PtTypeSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: PtTypeModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: PtTypeSearch) {
  if (!searchStaging) {
    return;
  }
...
/** 表格选择功能 */
const tableSelected = useSelect<PtTypeModel, PtTypeId>(
  $$(tableRef),
  {
    multiple: $$(multiple),
    isListSelectDialog,
...
/** 当前表格数据对应的搜索条件 */
let currentSearch = $ref<PtTypeSearch>({ });
...
async function useFindCount(
  ...
  page.total = await findCountPtType(
    search2,
    opt,
  );
}
...
async function onSortChange(
  { prop, order, column }: { column: TableColumnCtx<PtTypeModel> } & Sort,
) {
  if (!order) {
    sort = {
      ..._defaultSort,
...
const exportExcel = $ref(useExportExcelPtType());
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 复制 的 产品类别");
    return;
  }
...
const downloadImportTemplate = $ref(useDownloadImportTemplatePtType());
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<PtTypeInput>(
      file,
      header,
      {
        key_types: {
    ...
    const res = await importModelsPtType(
      models,
      $$(importPercentage),
      $$(isStopImport),
    );
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要编辑的 产品类别");
    return;
  }
...
async function onRowDblclick(
  row: PtTypeModel,
  column: TableColumnCtx<PtTypeModel>,
) {
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要查看的 产品类别");
    return;
  }
  const search = getDataSearch();
  const is_deleted = search.is_deleted;
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要删除的 产品类别");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定删除已选择的 ${ selectedIds.length } 产品类别?`, {
  ...
  const num = await deleteByIdsPtType(selectedIds);
  tableData = tableData.filter((item) => !selectedIds.includes(item.id));
  selectedIds = [ ];
  dirtyStore.fireDirty(pageName);
  await dataGrid(true);
  ElMessage.success(`删除 ${ num } 产品类别 成功`);
  emit("remove", num);
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 彻底删除 的 产品类别");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定 彻底删除 已选择的 ${ selectedIds.length } 产品类别?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await forceDeleteByIdsPtType(selectedIds);
  if (num) {
    selectedIds = [ ];
    ElMessage.success(`彻底删除 ${ num } 产品类别 成功`);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onEnableByIds(is_enabled: 0 | 1) {
  ...
  const num = await enableByIdsPtType(selectedIds, is_enabled);
  if (num > 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = `启用 ${ num } 产品类别 成功`;
...
async function onLockByIds(is_locked: 0 | 1) {
  ...
  const num = await lockByIdsPtType(selectedIds, is_locked);
  if (num > 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = `锁定 ${ num } 产品类别 成功`;
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要还原的 产品类别");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定还原已选择的 ${ selectedIds.length } 产品类别?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await revertByIdsPtType(selectedIds);
  if (num) {
    search.is_deleted = 0;
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(`还原 ${ num } 产品类别 成功`);
    emit("revert", num);
  }
}
