...
watchEffect(async () => {
  ...
  form_rules = {
    ...
    amt: [
      {
        required: true,
        message: `${ await nsAsync("请输入") } ${ n("充值金额") }`,
      },
    ],
    // 赠送金额
    give_amt: [
      {
        required: true,
        message: `${ await nsAsync("请输入") } ${ n("赠送金额") }`,
      },
    ],
  };
});
...
/** 内置变量 */
let builtInModel = $ref<RechargeRuleInput>();
...
async function showDialog(
  arg?: {
    title?: string;
    builtInModel?: RechargeRuleInput;
