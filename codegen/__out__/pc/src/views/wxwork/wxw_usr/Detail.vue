  ...
  form_rules = {
    ...
    lbl: [
      ...
      },
      {
        type: "string",
        max: 44,
        message: `${ n("姓名") } ${ await nsAsync("长度不能超过 {0}", 44) }`,
      },
    ],
  };
});
...
/** 内置变量 */
let builtInModel = $ref<WxwUsrInput>();
...
async function onPageUp(e?: KeyboardEvent) {
  ...
  if (!isSucc) {
    ElMessage.warning(await nsAsync("已经是第一个 {0} 了", await nsAsync("企微用户")));
  }
}
