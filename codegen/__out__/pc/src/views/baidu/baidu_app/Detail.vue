    ...
    >
      ...
      >
        ...
        <template v-if="(showBuildIn || builtInModel?.api_key == null)">
          ...
          >
            <CustomInput
              v-model="dialogModel.api_key"
              :placeholder="`${ ns('请输入') } ${ n('API Key') }`"
              :readonly="isLocked || isReadonly"
            ></CustomInput>
          </el-form-item>
        </template>
        ...
        <template v-if="(showBuildIn || builtInModel?.secret_key == null)">
          ...
          >
            <CustomInput
              v-model="dialogModel.secret_key"
              :placeholder="`${ ns('请输入') } ${ n('Secret Key') }`"
              :readonly="isLocked || isReadonly"
            ></CustomInput>
          </el-form-item>
        </template>
        ...
        <template v-if="(showBuildIn || builtInModel?.aes_key == null)">
          ...
          >
            <CustomInput
              v-model="dialogModel.aes_key"
              :placeholder="`${ ns('请输入') } ${ n('AES Key') }`"
              :readonly="isLocked || isReadonly"
            ></CustomInput>
          </el-form-item>
        </template>
...
watchEffect(async () => {
  ...
  form_rules = {
    ...
    lbl: [
      {
        required: true,
        message: `${ await nsAsync("请输入") } ${ n("应用名称") }`,
      },
      {
        type: "string",
        max: 40,
        message: `${ n("应用名称") } ${ await nsAsync("长度不能超过 {0}", 40) }`,
      },
    ],
    // AppID
    appid: [
      ...
      },
      {
        type: "string",
        max: 20,
        message: `${ n("AppID") } ${ await nsAsync("长度不能超过 {0}", 20) }`,
      },
    ],
    // API Key
    api_key: [
      {
        required: true,
        message: `${ await nsAsync("请输入") } ${ n("API Key") }`,
      },
      {
        type: "string",
        max: 40,
        message: `${ n("API Key") } ${ await nsAsync("长度不能超过 {0}", 40) }`,
      },
    ],
    // Secret Key
    secret_key: [
      {
        required: true,
        message: `${ await nsAsync("请输入") } ${ n("Secret Key") }`,
      },
      {
        type: "string",
        max: 40,
        message: `${ n("Secret Key") } ${ await nsAsync("长度不能超过 {0}", 40) }`,
      },
    ],
    // 排序
    order_by: [
...
type OnCloseResolveType = {
  type: "ok" | "cancel";
  changedIds: BaiduAppId[];
};

let onCloseResolve = function(_value: OnCloseResolveType) { };

/** 内置变量 */
let builtInModel = $ref<BaiduAppInput>();
