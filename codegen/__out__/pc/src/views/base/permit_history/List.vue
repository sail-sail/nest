...
>
  ...
  >
    ...
    >
      ...
      <template v-if="showBuildIn || builtInSearch?.permit_id == null">
        <el-form-item
          label="权限"
          prop="permit_id"
        >
          <CustomSelect
            :set="search.permit_id = search.permit_id || [ ]"
            un-w="full"
            v-model="search.permit_id"
            :method="getPermitList"
            :options-map="((item: PermitModel) => {
              return {
                label: item.lbl,
                value: item.id,
              };
            })"
            :placeholder="`${ ns('请选择') } ${ n('权限') }`"
            multiple
            @change="onSearch"
          ></CustomSelect>
        </el-form-item>
    ...
    >
      ...
      >
        ...
        >
          ...
          <!-- 权限 -->
          <template v-if="'permit_id_lbl' === col.prop && (showBuildIn || builtInSearch?.permit_id == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
          ...
          <!-- 角色 -->
          <template v-else-if="'role_id_lbl' === col.prop && (showBuildIn || builtInSearch?.role_id == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
...
const {
  ...
  initI18ns,
  initSysI18ns
} = useI18n("/base/permit_history");

const usrStore = useUsrStore();
const permitStore = usePermitStore();

const permit = permitStore.getPermit("/base/permit_history");
...
const props = defineProps<{
  ...
  id?: string; // ID
  permit_id?: string|string[]; // 权限
  permit_id_lbl?: string|string[]; // 权限
...
/** 内置搜索条件 */
const builtInSearch: PermitHistorySearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: PermitHistoryModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 表格数据 */
let tableData = $ref<PermitHistoryModel[]>([ ]);
...
let {
  ...
  resetColumns,
  storeColumns,
} = $(useTableColumns<PermitHistoryModel>(
  $$(tableColumns),
  {
...
async function onSortChange(
  { prop, order, column }: { column: TableColumnCtx<PermitHistoryModel> } & Sort,
) {
  sort.prop = prop || "";
...
let exportExcel = $ref(useExportExcel("/base/permit_history"));
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<PermitHistoryInput>(
      file,
      header,
      {
        date_keys: [
