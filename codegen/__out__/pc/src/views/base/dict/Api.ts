  ...
  async function workerFn2() {
    ...
    try {
      ...
      const buffer = await workerFn(
        `${ location.origin }${ location.pathname }/import_template/base/dict.xlsx`,
        {
          sheetName,
          data,
        },
  ...
  async function workerFn2(
    ...
    try {
      ...
      try {
        const sheetName = "系统字典";
        const buffer = await workerFn(
          `${ location.origin }${ location.pathname }/excel_template/base/dict.xlsx`,
          {
            sheetName,
            columns,
            data,
...
export async function findLastOrderByDict(
  ...
  const data: {
    ...
    query: /* GraphQL */ `
      query($search: DictSearch) {
        findLastOrderByDict(search: $search)
      }
    `,
    variables: {
      search,
  ...
  const order_by = data.findLastOrderByDict;
