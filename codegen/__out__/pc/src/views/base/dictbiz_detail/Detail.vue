  ...
  >
    ...
    >
      ...
      >
        ...
        <template v-if="(showBuildIn || builtInModel?.dictbiz_id == null)">
          <el-form-item
            label="业务字典"
            prop="dictbiz_id"
          >
...
type OnCloseResolveType = {
  type: "ok" | "cancel";
  changedIds: DictbizDetailId[];
};

let onCloseResolve = function(_value: OnCloseResolveType) { };

/** 内置变量 */
let builtInModel = $ref<DictbizDetailInput>();
...
async function showDialog(
  arg?: {
    title?: string;
    notice?: string;
    builtInModel?: DictbizDetailInput;
    ...
    findOne?: typeof findOneDictbizDetail;
    action: DialogAction;
  },
) {
  inited = false;
  ...
  if (arg?.findOne) {
    ...
    findOneModel = findOneDictbizDetail;
  }
  ...
  dictbiz_detail_model = undefined;
...
async function onRefresh() {
  ...
  if (data) {
    dialogModel = intoInputDictbizDetail({
      ...data,
    });
    dialogTitle = `${ oldDialogTitle } - ${ dialogModel.lbl }`;
  }
  dictbiz_detail_model = data;
}
...
async function save() {
  ...
  if (dialogAction === "add" || dialogAction === "copy") {
    ...
    id = await createDictbizDetail(dialogModel2);
    dialogModel.id = id;
    msg = "新增成功";
  } else if (dialogAction === "edit" || dialogAction === "view") {
    if (!dialogModel.id) {
    ...
    Object.assign(dialogModel2, { is_deleted: undefined });
    id = await updateByIdDictbizDetail(
      dialogModel.id,
      dialogModel2,
    );
    msg = "编辑成功";
