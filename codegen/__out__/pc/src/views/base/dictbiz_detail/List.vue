...
const emit = defineEmits<{
  selectedIdsChg: [ DictbizDetailId[] ],
  add: [ DictbizDetailId[] ],
  edit: [ DictbizDetailId[] ],
  remove: [ number ],
  revert: [ number ],
  refresh: [ ],
  beforeSearchReset: [ ],
  rowEnter: [ KeyboardEvent? ],
  rowDblclick: [ DictbizDetailModel ],
}>();
...
/** 内置查询条件 */
const builtInSearch: DictbizDetailSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: DictbizDetailModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: DictbizDetailSearch) {
  if (!searchStaging) {
    return;
  }
...
/** 表格选择功能 */
const tableSelected = useSelect<DictbizDetailModel, DictbizDetailId>(
  $$(tableRef),
  {
    multiple: $$(multiple),
    isListSelectDialog,
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要 复制 的 {0}", await nsAsync("业务字典明细")));
    return;
  }
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<DictbizDetailInput>(
      file,
      header,
      {
        key_types: {
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要编辑的 {0}", await nsAsync("业务字典明细")));
    return;
  }
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要查看的 {0}", await nsAsync("业务字典明细")));
    return;
  }
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要删除的 {0}", await nsAsync("业务字典明细")));
    return;
  }
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定删除已选择的 {0} {1}", selectedIds.length, await nsAsync("业务字典明细")) }?`, {
  ...
  if (num) {
    ...
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(await nsAsync("删除 {0} {1} 成功", num, await nsAsync("业务字典明细")));
    emit("remove", num);
  }
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要 彻底删除 的 {0}", await nsAsync("业务字典明细")));
    return;
  }
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定 彻底删除 已选择的 {0} {1}", selectedIds.length, await nsAsync("业务字典明细")) }?`, {
      confirmButtonText: await nsAsync("确定"),
      cancelButtonText: await nsAsync("取消"),
      type: "warning",
    });
  ...
  if (num) {
    selectedIds = [ ];
    ElMessage.success(await nsAsync("彻底删除 {0} {1} 成功", num, await nsAsync("业务字典明细")));
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onEnableByIds(is_enabled: 0 | 1) {
  ...
  if (selectedIds.length === 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = await nsAsync("请选择需要 启用 的 {0}", await nsAsync("业务字典明细"));
    } else {
      msg = await nsAsync("请选择需要 禁用 的 {0}", await nsAsync("业务字典明细"));
    }
  ...
  if (num > 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = await nsAsync("启用 {0} {1} 成功", num, await nsAsync("业务字典明细"));
    } else {
      msg = await nsAsync("禁用 {0} {1} 成功", num, await nsAsync("业务字典明细"));
    }
...
async function onLockByIds(is_locked: 0 | 1) {
  ...
  if (selectedIds.length === 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = await nsAsync("请选择需要 锁定 的 {0}", await nsAsync("业务字典明细"));
    } else {
      msg = await nsAsync("请选择需要 解锁 的 {0}", await nsAsync("业务字典明细"));
    }
    ElMessage.warning(msg);
    return;
  }
  ...
  if (num > 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = await nsAsync("锁定 {0} {1} 成功", num, await nsAsync("业务字典明细"));
    } else {
      msg = await nsAsync("解锁 {0} {1} 成功", num, await nsAsync("业务字典明细"));
    }
    ElMessage.success(msg);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要还原的 {0}", await nsAsync("业务字典明细")));
    return;
  }
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定还原已选择的 {0} {1}", selectedIds.length, await nsAsync("业务字典明细")) }?`, {
      confirmButtonText: await nsAsync("确定"),
      cancelButtonText: await nsAsync("取消"),
      type: "warning",
    });
  ...
  if (num) {
    ...
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(await nsAsync("还原 {0} {1} 成功", num, await nsAsync("业务字典明细")));
    emit("revert", num);
  }
}
