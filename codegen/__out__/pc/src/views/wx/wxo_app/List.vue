...
async function onDeleteByIds() {
  ...
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定删除已选择的 {0} 个 {1}", selectedIds.length, await nsAsync("公众号设置")) }?`, {
  ...
  if (num) {
    ...
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(await nsAsync("删除 {0} 个 {1} 成功", num, await nsAsync("公众号设置")));
    emit("remove", num);
  }
}
...
async function onForceDeleteByIds() {
  ...
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定 彻底删除 已选择的 {0} 个 {1}", selectedIds.length, await nsAsync("公众号设置")) }?`, {
      confirmButtonText: await nsAsync("确定"),
      cancelButtonText: await nsAsync("取消"),
      type: "warning",
    });
  ...
  if (num) {
    selectedIds = [ ];
    ElMessage.success(await nsAsync("彻底删除 {0} 个 {1} 成功", num, await nsAsync("公众号设置")));
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onEnableByIds(is_enabled: 0 | 1) {
  ...
  if (num > 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = await nsAsync("启用 {0} 个 {1} 成功", num, await nsAsync("公众号设置"));
    } else {
      msg = await nsAsync("禁用 {0} 个 {1} 成功", num, await nsAsync("公众号设置"));
    }
...
async function onLockByIds(is_locked: 0 | 1) {
  ...
  if (num > 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = await nsAsync("锁定 {0} 个 {1} 成功", num, await nsAsync("公众号设置"));
    } else {
      msg = await nsAsync("解锁 {0} 个 {1} 成功", num, await nsAsync("公众号设置"));
    }
    ElMessage.success(msg);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
...
async function onRevertByIds() {
  ...
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定还原已选择的 {0} 个 {1}", selectedIds.length, await nsAsync("公众号设置")) }?`, {
      confirmButtonText: await nsAsync("确定"),
      cancelButtonText: await nsAsync("取消"),
      type: "warning",
    });
  ...
  if (num) {
    ...
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(await nsAsync("还原 {0} 个 {1} 成功", num, await nsAsync("公众号设置")));
    emit("revert", num);
  }
}
