    ...
    >
      ...
      >
        ...
        >
          ...
          <!-- 当前生效版本 -->
          <template v-else-if="'current_revision_id_lbl' === col.prop && (showBuildIn || builtInSearch?.current_revision_id == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
...
import {
  ...
  findAllProcessDef,
  findCountProcessDef,
  revertByIdsProcessDef,
  deleteByIdsProcessDef,
  forceDeleteByIdsProcessDef,
  enableByIdsProcessDef,
  useExportExcelProcessDef,
  updateByIdProcessDef,
  importModelsProcessDef,
  useDownloadImportTemplateProcessDef,
} from "./Api.ts";
...
const props = defineProps<{
  ...
  ids?: string[]; //ids
  selectedIds?: ProcessDefId[]; //已选择行的id列表
...
/** 内置查询条件 */
const builtInSearch: ProcessDefSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: ProcessDefModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
let search = $ref<ProcessDefSearch>(initSearch());
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: ProcessDefSearch) {
  if (!searchStaging) {
    return;
  }
...
let selectedIds = $(tableSelected.selectedIds as unknown as ProcessDefId[]);
...
/** 表格数据 */
let tableData = $ref<ProcessDefModel[]>([ ]);
...
function getTableColumns(): ColumnType[] {
  return [
    ...
    {
      label: "当前生效版本",
      prop: "current_revision_id_lbl",
      sortBy: "current_revision_id_lbl",
      width: 200,
      align: "center",
      headerAlign: "center",
      showOverflowTooltip: true,
...
const {
  ...
  storeColumns,
  initColumns,
} = $(useTableColumns<ProcessDefModel>(
  $$(tableColumns),
  {
...
async function useFindAll(
  ...
  if (isPagination) {
    ...
    tableData = await findAllProcessDef(
      search,
      {
        pgSize,
        pgOffset,
...
async function useFindCount(
  ...
  page.total = await findCountProcessDef(
    search,
    opt,
  );
}
...
async function onSortChange(
  { prop, order, column }: { column: TableColumnCtx<ProcessDefModel> } & Sort,
) {
  if (!order) {
    sort = {
      ..._defaultSort,
...
const exportExcel = $ref(useExportExcelProcessDef());
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 复制 的 流程定义");
    return;
  }
...
const downloadImportTemplate = $ref(useDownloadImportTemplateProcessDef());
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<ProcessDefInput>(
      file,
      header,
      {
        key_types: {
    ...
    const res = await importModelsProcessDef(
      models,
      $$(importPercentage),
      $$(isStopImport),
    );
...
/** 启用 */
async function onIs_enabled(id: ProcessDefId, is_enabled: 0 | 1) {
  if (isLocked) {
    return;
  }
  const notLoading = true;
  await enableByIdsProcessDef(
    [ id ],
    is_enabled,
    {
      notLoading,
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要编辑的 流程定义");
    return;
  }
...
async function onRowDblclick(
  row: ProcessDefModel,
  column: TableColumnCtx<ProcessDefModel>,
) {
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要查看的 流程定义");
    return;
  }
  const search = getDataSearch();
  const is_deleted = search.is_deleted;
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要删除的 流程定义");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定删除已选择的 ${ selectedIds.length } 流程定义?`, {
  ...
  const num = await deleteByIdsProcessDef(selectedIds);
  tableData = tableData.filter((item) => !selectedIds.includes(item.id));
  selectedIds = [ ];
  dirtyStore.fireDirty(pageName);
  await dataGrid(true);
  ElMessage.success(`删除 ${ num } 流程定义 成功`);
  emit("remove", num);
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 彻底删除 的 流程定义");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定 彻底删除 已选择的 ${ selectedIds.length } 流程定义?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await forceDeleteByIdsProcessDef(selectedIds);
  if (num) {
    selectedIds = [ ];
    ElMessage.success(`彻底删除 ${ num } 流程定义 成功`);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onEnableByIds(is_enabled: 0 | 1) {
  ...
  const num = await enableByIdsProcessDef(selectedIds, is_enabled);
  if (num > 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = `启用 ${ num } 流程定义 成功`;
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要还原的 流程定义");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定还原已选择的 ${ selectedIds.length } 流程定义?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await revertByIdsProcessDef(selectedIds);
  if (num) {
    search.is_deleted = 0;
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(`还原 ${ num } 流程定义 成功`);
    emit("revert", num);
  }
}
