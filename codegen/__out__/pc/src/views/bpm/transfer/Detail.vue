  ...
  >
    ...
    >
      ...
      >
        ...
        <template v-if="(showBuildIn || builtInModel?.from_usr_id == null)">
          <el-form-item
            label="转出人"
            prop="from_usr_id"
          >
            <CustomSelect
              v-model="dialogModel.from_usr_id"
              v-model:model-label="dialogModel.from_usr_id_lbl"
              :method="getListUsr"
        ...
        <template v-if="(showBuildIn || builtInModel?.to_usr_id == null)">
          <el-form-item
            label="接收人"
            prop="to_usr_id"
          >
            <CustomSelect
              v-model="dialogModel.to_usr_id"
              v-model:model-label="dialogModel.to_usr_id_lbl"
              :method="getListUsr"
              :find-by-values="findByIdsUsr"
              :options-map="((item: UsrModel) => {
                return {
...
let dialogModel = $ref<TransferInput>({
} as TransferInput);

let transfer_model = $ref<TransferModel>();

let ids = $ref<TransferId[]>([ ]);
let is_deleted = $ref<0 | 1>(0);
let changedIds = $ref<TransferId[]>([ ]);
...
type OnCloseResolveType = {
  type: "ok" | "cancel";
  changedIds: TransferId[];
};

let onCloseResolve = function(_value: OnCloseResolveType) { };

/** 内置变量 */
let builtInModel = $ref<TransferInput>();
...
let findOneModel = findOneTransfer;
...
async function showDialog(
  arg?: {
    ...
    findOne?: typeof findOneTransfer;
    action: DialogAction;
  },
) {
  inited = false;
...
watch(
  () => [
    dialogModel.task_id,
    dialogModel.from_usr_id,
    dialogModel.to_usr_id,
  ],
  () => {
    if (!inited) {
      return;
    ...
    if (!dialogModel.from_usr_id) {
      dialogModel.from_usr_id_lbl = "";
    }
    if (!dialogModel.to_usr_id) {
      dialogModel.to_usr_id_lbl = "";
    }
  },
);
...
async function save() {
  ...
  let id: TransferId | undefined = undefined;
  let msg = "";
  if (dialogAction === "add" || dialogAction === "copy") {
    ...
    id = await createTransfer(dialogModel2);
    dialogModel.id = id;
    msg = "新增成功";
