  ...
  >
    ...
    >
      ...
      <template v-if="(showBuildIn || builtInSearch?.process_def_id == null)">
        ...
        >
          <CustomSelect
            v-model="process_def_id_search"
            :method="getListProcessDef"
            :options-map="((item: ProcessDefModel) => {
              return {
                label: item.lbl,
                value: item.id,
              };
      ...
      <template v-if="(showBuildIn || builtInSearch?.start_usr_id == null)">
        <el-form-item
          label="发起人"
          prop="start_usr_id"
        >
          <CustomSelect
            v-model="start_usr_id_search"
            :method="getListUsr"
      ...
      <template v-if="(showBuildIn || builtInSearch?.start_dept_id == null)">
        <el-form-item
          label="发起人部门"
          prop="start_dept_id"
        >
          <CustomTreeSelect
            v-model="start_dept_id_search"
            :method="getTreeDept"
    ...
    >
      ...
      >
        ...
        >
          ...
          <!-- 流程定义 -->
          <template v-else-if="'process_def_id_lbl' === col.prop && (showBuildIn || builtInSearch?.process_def_id == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
          ...
          <!-- 发起人 -->
          <template v-else-if="'start_usr_id_lbl' === col.prop && (showBuildIn || builtInSearch?.start_usr_id == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
          ...
          <!-- 发起人部门 -->
          <template v-else-if="'start_dept_id_lbl' === col.prop && (showBuildIn || builtInSearch?.start_dept_id == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
          ...
          <!-- 当前节点名称 -->
          <template v-else-if="'current_node_lbls' === col.prop">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
...
import {
  getPagePathProcessInst,
  findAllProcessInst,
  findCountProcessInst,
  revertByIdsProcessInst,
  deleteByIdsProcessInst,
  forceDeleteByIdsProcessInst,
  useExportExcelProcessInst,
  importModelsProcessInst,
  useDownloadImportTemplateProcessInst,
} from "./Api.ts";

import {
  getListProcessDef, // 流程定义
  getListUsr, // 发起人
} from "./Api.ts";
...
const pagePath = getPagePathProcessInst();
// @ts-ignore
...
const props = defineProps<{
  ...
  ids?: string[]; //ids
  selectedIds?: ProcessInstId[]; //已选择行的id列表
  isMultiple?: string; //是否多选
  id?: ProcessInstId; // ID
  lbl?: string; // 实例标题
  lbl_like?: string; // 实例标题
  ...
  start_dept_id_lbl?: string; // 发起人部门
}>();
...
/** 内置查询条件 */
const builtInSearch: ProcessInstSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: ProcessInstModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
let search = $ref<ProcessInstSearch>(initSearch());
...
// 发起人
const start_usr_id_search = $computed({
  get() {
    return search.start_usr_id || [ ];
  },
  set(val) {
    if (!val || val.length === 0) {
      search.start_usr_id = undefined;
    } else {
      search.start_usr_id = val;
    }
  },
});

// 发起人部门
const start_dept_id_search = $computed({
  get() {
    return search.start_dept_id || [ ];
  },
  set(val) {
    if (!val || val.length === 0) {
      search.start_dept_id = undefined;
    } else {
      search.start_dept_id = val;
    }
  },
});
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: ProcessInstSearch) {
  if (!searchStaging) {
    return;
  }
...
let selectedIds = $(tableSelected.selectedIds as unknown as ProcessInstId[]);
...
/** 表格数据 */
let tableData = $ref<ProcessInstModel[]>([ ]);
...
function getTableColumns(): ColumnType[] {
  return [
    ...
    {
      label: "发起人",
      prop: "start_usr_id_lbl",
      sortBy: "start_usr_id_lbl",
      width: 200,
      align: "center",
      headerAlign: "center",
      showOverflowTooltip: true,
    ...
    {
      label: "发起人部门",
      prop: "start_dept_id_lbl",
      sortBy: "start_dept_id_lbl",
      width: 180,
      align: "left",
      headerAlign: "center",
      showOverflowTooltip: true,
...
const {
  ...
  storeColumns,
  initColumns,
} = $(useTableColumns<ProcessInstModel>(
  $$(tableColumns),
  {
...
async function useFindAll(
  ...
  if (isPagination) {
    ...
    tableData = await findAllProcessInst(
      search,
      {
        pgSize,
        pgOffset,
...
async function useFindCount(
  ...
  page.total = await findCountProcessInst(
    search,
    opt,
  );
}
...
async function onSortChange(
  { prop, order, column }: { column: TableColumnCtx<ProcessInstModel> } & Sort,
) {
  if (!order) {
    sort = {
      ..._defaultSort,
...
const exportExcel = $ref(useExportExcelProcessInst());
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 复制 的 流程实例");
    return;
  }
...
const downloadImportTemplate = $ref(useDownloadImportTemplateProcessInst());
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<ProcessInstInput>(
      file,
      header,
      {
        key_types: {
    ...
    const res = await importModelsProcessInst(
      models,
      $$(importPercentage),
      $$(isStopImport),
    );
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要编辑的 流程实例");
    return;
  }
...
async function onRowDblclick(
  row: ProcessInstModel,
  column: TableColumnCtx<ProcessInstModel>,
) {
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要查看的 流程实例");
    return;
  }
  const search = getDataSearch();
  const is_deleted = search.is_deleted;
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要删除的 流程实例");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定删除已选择的 ${ selectedIds.length } 流程实例?`, {
  ...
  const num = await deleteByIdsProcessInst(selectedIds);
  tableData = tableData.filter((item) => !selectedIds.includes(item.id));
  selectedIds = [ ];
  dirtyStore.fireDirty(pageName);
  await dataGrid(true);
  ElMessage.success(`删除 ${ num } 流程实例 成功`);
  emit("remove", num);
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 彻底删除 的 流程实例");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定 彻底删除 已选择的 ${ selectedIds.length } 流程实例?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await forceDeleteByIdsProcessInst(selectedIds);
  if (num) {
    selectedIds = [ ];
    ElMessage.success(`彻底删除 ${ num } 流程实例 成功`);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要还原的 流程实例");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定还原已选择的 ${ selectedIds.length } 流程实例?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await revertByIdsProcessInst(selectedIds);
  if (num) {
    search.is_deleted = 0;
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(`还原 ${ num } 流程实例 成功`);
    emit("revert", num);
  }
}
