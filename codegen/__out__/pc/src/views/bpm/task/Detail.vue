  ...
  >
    ...
    >
      ...
      >
        ...
        <template v-if="(showBuildIn || builtInModel?.assignee_usr_id == null)">
          <el-form-item
            label="处理人"
            prop="assignee_usr_id"
          >
            <CustomSelect
              v-model="dialogModel.assignee_usr_id"
              v-model:model-label="dialogModel.assignee_usr_id_lbl"
              :method="getListUsr"
        ...
        <template v-if="(showBuildIn || builtInModel?.action == null)">
          ...
          >
            <DictSelect
              ...
              code="bpm_task_action"
              placeholder="请选择 审批动作"
              :readonly="isLocked || isReadonly"
            ></DictSelect>
          </el-form-item>
        </template>
...
let dialogModel = $ref<TaskInput>({
} as TaskInput);

let task_model = $ref<TaskModel>();

let ids = $ref<TaskId[]>([ ]);
let is_deleted = $ref<0 | 1>(0);
let changedIds = $ref<TaskId[]>([ ]);
...
/** 内置变量 */
let builtInModel = $ref<TaskInput>();
...
watch(
  ...
  () => {
    ...
    if (!dialogModel.assignee_usr_id) {
      dialogModel.assignee_usr_id_lbl = "";
    }
...
async function save() {
  ...
  let id: TaskId | undefined = undefined;
  let msg = "";
  if (dialogAction === "add" || dialogAction === "copy") {
    ...
    id = await createTask(dialogModel2);
    dialogModel.id = id;
    msg = "新增成功";
