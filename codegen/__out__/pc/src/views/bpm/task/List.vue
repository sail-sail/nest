...
>
  ...
  >
    ...
    >
      ...
      <template v-if="(builtInSearch?.lbl == null && (showBuildIn || builtInSearch?.lbl_like == null))">
        ...
        >
          <CustomInput
            ...
            placeholder="请输入 任务标题"
            @clear="onSearchClear"
          ></CustomInput>
        </el-form-item>
      </template>
      
      <template v-if="(showBuildIn || builtInSearch?.process_inst_id == null)">
        <el-form-item
          label="流程实例"
          prop="process_inst_id"
        >
          <CustomSelect
            v-model="process_inst_id_search"
      ...
      <template v-if="(showBuildIn || builtInSearch?.assignee_usr_id == null)">
        <el-form-item
          label="处理人"
          prop="assignee_usr_id"
        >
          <CustomSelect
            v-model="assignee_usr_id_search"
            :method="getListUsr"
      ...
      <template v-if="(showBuildIn || builtInSearch?.status == null)">
        ...
        >
          <DictSelect
            ...
            code="bpm_task_status"
            placeholder="请选择 任务状态"
            multiple
            @change="onSearch(false)"
          ></DictSelect>
        </el-form-item>
    ...
    >
      ...
      >
        ...
        >
          ...
          <!-- 流程实例 -->
          <template v-else-if="'process_inst_id_lbl' === col.prop && (showBuildIn || builtInSearch?.process_inst_id == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
          ...
          <!-- 处理人 -->
          <template v-else-if="'assignee_usr_id_lbl' === col.prop && (showBuildIn || builtInSearch?.assignee_usr_id == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
          ...
          <!-- 审批动作 -->
          <template v-else-if="'action_lbl' === col.prop">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
...
import {
  ...
  revertByIdsTask,
  deleteByIdsTask,
  forceDeleteByIdsTask,
  useExportExcelTask,
  importModelsTask,
  useDownloadImportTemplateTask,
} from "./Api.ts";

import {
  getListProcessInst, // 流程实例
  getListUsr, // 处理人
} from "./Api.ts";
...
const pagePath = getPagePathTask();
// @ts-ignore
...
const props = defineProps<{
  ...
  ids?: string[]; //ids
  selectedIds?: TaskId[]; //已选择行的id列表
  isMultiple?: string; //是否多选
  id?: TaskId; // ID
  lbl?: string; // 任务标题
  lbl_like?: string; // 任务标题
  process_inst_id?: string|string[]; // 流程实例
  process_inst_id_lbl?: string; // 流程实例
  node_inst_id?: string|string[]; // 节点实例
  node_inst_id_lbl?: string; // 节点实例
  assignee_usr_id?: string|string[]; // 处理人
  assignee_usr_id_lbl?: string; // 处理人
...
const builtInSearchType: { [key: string]: string } = {
  ...
  node_inst_id: "string[]",
  node_inst_id_lbl: "string[]",
  assignee_usr_id: "string[]",
...
/** 内置查询条件 */
const builtInSearch: TaskSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: TaskModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
let search = $ref<TaskSearch>(initSearch());

// 流程实例
const process_inst_id_search = $computed({
  get() {
    return search.process_inst_id || [ ];
  },
  set(val) {
    if (!val || val.length === 0) {
      search.process_inst_id = undefined;
    } else {
      search.process_inst_id = val;
    }
  },
});

// 处理人
const assignee_usr_id_search = $computed({
  get() {
    return search.assignee_usr_id || [ ];
  },
  set(val) {
    if (!val || val.length === 0) {
      search.assignee_usr_id = undefined;
    } else {
      search.assignee_usr_id = val;
    }
  },
});
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: TaskSearch) {
  if (!searchStaging) {
    return;
  }
...
let selectedIds = $(tableSelected.selectedIds as unknown as TaskId[]);
...
function getTableColumns(): ColumnType[] {
  return [
    ...
    {
      label: "流程实例",
      prop: "process_inst_id_lbl",
      sortBy: "process_inst_id_lbl",
      width: 250,
      align: "left",
      headerAlign: "center",
      showOverflowTooltip: true,
    ...
    {
      label: "处理人",
      prop: "assignee_usr_id_lbl",
      sortBy: "assignee_usr_id_lbl",
      width: 200,
      align: "center",
      headerAlign: "center",
      showOverflowTooltip: true,
...
async function useFindCount(
  ...
  page.total = await findCountTask(
    search,
    opt,
  );
}
...
async function onSortChange(
  { prop, order, column }: { column: TableColumnCtx<TaskModel> } & Sort,
) {
  if (!order) {
    sort = {
      ..._defaultSort,
...
const exportExcel = $ref(useExportExcelTask());
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 复制 的 审批任务");
    return;
  }
...
const downloadImportTemplate = $ref(useDownloadImportTemplateTask());
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<TaskInput>(
      file,
      header,
      {
        key_types: {
    ...
    const res = await importModelsTask(
      models,
      $$(importPercentage),
      $$(isStopImport),
    );
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要编辑的 审批任务");
    return;
  }
...
async function onRowDblclick(
  row: TaskModel,
  column: TableColumnCtx<TaskModel>,
) {
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要查看的 审批任务");
    return;
  }
  const search = getDataSearch();
  const is_deleted = search.is_deleted;
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要删除的 审批任务");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定删除已选择的 ${ selectedIds.length } 审批任务?`, {
  ...
  const num = await deleteByIdsTask(selectedIds);
  tableData = tableData.filter((item) => !selectedIds.includes(item.id));
  selectedIds = [ ];
  dirtyStore.fireDirty(pageName);
  await dataGrid(true);
  ElMessage.success(`删除 ${ num } 审批任务 成功`);
  emit("remove", num);
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 彻底删除 的 审批任务");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定 彻底删除 已选择的 ${ selectedIds.length } 审批任务?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await forceDeleteByIdsTask(selectedIds);
  if (num) {
    selectedIds = [ ];
    ElMessage.success(`彻底删除 ${ num } 审批任务 成功`);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要还原的 审批任务");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定还原已选择的 ${ selectedIds.length } 审批任务?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await revertByIdsTask(selectedIds);
  if (num) {
    search.is_deleted = 0;
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(`还原 ${ num } 审批任务 成功`);
    emit("revert", num);
  }
}
