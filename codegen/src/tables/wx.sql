...
CREATE TABLE if not exists `wx_wx_usr` (
  ...
  `gender` int(11) NOT NULL DEFAULT 0 COMMENT '性别,dictbiz:wx_usr_gender',
...
CREATE TABLE if not exists `wx_pay_transactions_jsapi` (
  ...
  `trade_state` varchar(32) NOT NULL DEFAULT 'NOTPAY' COMMENT '交易状态,dictbiz:wx_pay_notice_trade_state',
  ...
  `currency` varchar(16) NOT NULL DEFAULT '' COMMENT '货币类型,dictbiz:wx_pay_notice_currency',
...
CREATE TABLE if not exists `wx_wx_pay_notice` (
  ...
  `out_trade_no` varchar(32) NOT NULL DEFAULT '' COMMENT '商户订单号',
  `transaction_id` varchar(32) NOT NULL DEFAULT '' COMMENT '微信支付订单号',
  `trade_type` varchar(16) NOT NULL DEFAULT '' COMMENT '交易类型,dictbiz:wx_unified_order_trade_type',
  `trade_state` varchar(32) NOT NULL DEFAULT 'NOTPAY' COMMENT '交易状态,dictbiz:wx_pay_notice_trade_state',
  `trade_state_desc` varchar(256) NOT NULL DEFAULT '未支付' COMMENT '交易状态描述',
  ...
  `currency` varchar(16) NOT NULL DEFAULT 'CNY' COMMENT '货币类型,dictbiz:wx_pay_notice_currency',
  `payer_currency` varchar(16) NOT NULL DEFAULT 'CNY' COMMENT '用户支付币种,dictbiz:wx_pay_notice_currency',
