...
CREATE TABLE if not exists `wx_wx_pay_notice` (
  ...
  `attach` varchar(128) NOT NULL DEFAULT '' COMMENT '附加数据',
  `success_time` datetime DEFAULT NULL COMMENT '支付完成时间',
  `total` int unsigned NOT NULL DEFAULT 0 COMMENT '总金额(分)',
  `payer_total` int unsigned NOT NULL DEFAULT 0 COMMENT '用户支付金额(分)',
  `currency` ENUM('CNY') NOT NULL DEFAULT 'CNY' COMMENT '货币类型,dict:wx_pay_notice_currency',
...
CREATE TABLE if not exists `wx_wx_refund` (
  ...
  `status` ENUM('NO_REFUND', 'SUCCESS', 'CLOSED', 'PROCESSING', 'ABNORMAL') NOT NULL DEFAULT 'PROCESSING' COMMENT '退款状态,dict:wx_refund_status',
...
CREATE TABLE if not exists `wx_wx_refund_notice` (
  ...
  `out_refund_no` varchar(64) NOT NULL DEFAULT '' COMMENT '商户退款单号',
  `refund_id` varchar(32) NOT NULL DEFAULT '' COMMENT '微信退款单号',
  `refund_status` ENUM('NO_REFUND', 'SUCCESS', 'CLOSED', 'PROCESSING', 'ABNORMAL') NOT NULL DEFAULT 'PROCESSING' COMMENT '退款状态,dict:wx_refund_status',
  `success_time` datetime DEFAULT NULL COMMENT '退款成功时间',
  `user_received_account` varchar(64) NOT NULL DEFAULT '' COMMENT '退款入账账户',
  `amount_total` int unsigned NOT NULL DEFAULT 0 COMMENT '订单金额(分)',
  `amount_refund` int unsigned NOT NULL DEFAULT 0 COMMENT '退款金额(分)',
  ...
  INDEX (`refund_id`),
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_as_cs COMMENT='微信退款通知';
