...
CREATE TABLE if not exists `nuxt_seo` (
  ...
  `ico` varchar(22) NOT NULL DEFAULT '' COMMENT '图标',
  `lbl` varchar(100) NOT NULL DEFAULT '' COMMENT '标题',
...
------------------------------------------------------------------------ SEO优化域名
drop table if exists `nuxt_seo_domain`;
CREATE TABLE if not exists `nuxt_seo_domain` (
  `id` varchar(22) NOT NULL COMMENT 'ID',
  `seo_id` varchar(22) NOT NULL DEFAULT '' COMMENT 'SEO优化',
  `domain_id` varchar(22) NOT NULL DEFAULT '' COMMENT '域名',
  `order_by` int unsigned NOT NULL DEFAULT 1 COMMENT '排序',
  `tenant_id` varchar(22) NOT NULL DEFAULT '' COMMENT '租户',
  `create_usr_id` varchar(22) NOT NULL DEFAULT '' COMMENT '创建人',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  ...
  `is_deleted` tinyint unsigned NOT NULL DEFAULT 0 COMMENT '删除,dict:is_deleted',
  `delete_time` datetime DEFAULT NULL COMMENT '删除时间',
  INDEX (`tenant_id`, `is_deleted`, `seo_id`, `domain_id`),
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_as_cs COMMENT='SEO优化域名';
