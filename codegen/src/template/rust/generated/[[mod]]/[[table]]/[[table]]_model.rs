...
use crate::<#=auditMod#>::<#=auditTable#>::<#=auditTable#>_model::<#=auditTable_Up#>Model;<#
}
#><#
if (hasEncrypt) {
#>
...
impl From<<#=tableUP#>Model> for crate::<#=mod#>::<#=historyTable#>::<#=historyTable#>_model::<#=historyTableUp#>Input {
  fn from(model: <#=tableUP#>Model) -> Self {
    Self {<#
      if (hasTenantId) {
      #>
...
pub struct <#=Table_Up#>Id([u8; 22]);

impl Serialize for <#=Table_Up#>Id {
...
impl<'de> Deserialize<'de> for <#=Table_Up#>Id {
...
impl fmt::Debug for <#=Table_Up#>Id {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "<#=Table_Up#>Id({s})"),
      Err(_) => write!(f, "<#=Table_Up#>Id()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("<#=Table_Up#>Id must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl From<[u8; 22]> for <#=Table_Up#>Id {
...
impl From<<#=Table_Up#>Id> for [u8; 22] {
  fn from(id: <#=Table_Up#>Id) -> Self {
    id.0
  }
}
...
impl<'r> sqlx::Decode<'r, MySql> for <#=Table_Up#>Id {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("<#=Table_Up#>Id must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}
