...
export function intoInput<#=Table_Up#>(
  ...
  // <#=column_comment#>
  input.<#=column_name#> = new Decimal(input.<#=column_name#> ?? "0");<#
  }
  #><#
  for (const inlineForeignTab of inlineForeignTabs) {
    const inlineForeignSchema = optTables[inlineForeignTab.mod + "_" + inlineForeignTab.table];
  ...
  input?.<#=inline_column_name#>?.map(intoInput<#=Table_Up#>);<#
    } else if (inline_foreign_type === "one2one") {
  #>
  
  // <#=inlineForeignTab.label#>
  intoInput<#=Table_Up#>(input?.<#=inline_column_name#>);<#
    }
  #><#
  }
  #><#
  ...
  input?.<#=column_name#>_<#=table#>_models?.map(intoInput<#=Table_Up#>);<#
  }
  #>
}
