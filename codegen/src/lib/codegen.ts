...
import type { TableColumn, TablesConfigItem } from "../config.ts";
...
export async function codegen(context: Context, schema: TablesConfigItem, table_names: string[]) {
  ...
  if (!mod_keys.includes(mod)) {
    if (mod_keys.length === 0) {
      console.log();
    }
    console.log("生成模块: " + mod);
    mod_keys.push(mod);
  }
  ...
  async function treeDir(dir: string, writeFnArr: Function[]) {
    ...
    if (dir === "/rust/generated/[[mod]]/[[table]]/usr_sync_dao.rs") {
...
export async function gitDiffOut() {
  ...
  const diffStr = `git diff --patience --full-index ./* > ${projectPh}/${ diffFile }`;
