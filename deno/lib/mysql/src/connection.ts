...
export class Connection {
  ...
  async threadId() {
    ...
    const reslut = (await this.query("select connection_id() as threadId") as any[]);
    this._threadId = reslut[0].threadId;
    return this._threadId;
  }
  ...
  private buildIterator(fields: FieldInfo[]): any {
    const next = async () => {
      ...
      const value = parseRow(receive.body, fields, this.config);
