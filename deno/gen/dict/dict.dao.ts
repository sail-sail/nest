...
/**
 * 获得表的唯一字段名列表
 * @return {{ uniqueKeys: (keyof DictModel)[]; uniqueComments: { [key: string]: string }; }}
 */
...
async function findOne(
  ...
  const model = result[0] as DictModel | undefined;
  return model;
}
...
async function updateById(
  ...
  const args = new QueryArgs();
  let sql = /*sql*/ `
    update dict set update_time = ${ args.push(reqDate()) }
  `;
  let updateFldNum = 0;
  if (model.code !== undefined) {
