...
async function _creates(
  ...
  const args = new QueryArgs();
  let sql = `insert into wshop_card_consume(id,create_time,tenant_id,org_id,create_usr_id,transaction_id,card_id,usr_id,amt,give_amt,integral,balance,give_balance,rem)values`;
...
export async function updateOrgById(
  ...
  const args = new QueryArgs();
  const sql = `update wshop_card_consume set org_id=${ args.push(org_id) } where id=${ args.push(id) }
  `;
  const result = await execute(sql, args);
  const num = result.affectedRows;
  return num;
...
export async function deleteByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    const args = new QueryArgs();
    const sql = `update wshop_card_consume set is_deleted=1,delete_time=${ args.push(reqDate()) } where id=${ args.push(id) } limit 1`;
    const result = await execute(sql, args);
...
export async function forceDeleteByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    {
      const args = new QueryArgs();
      const sql = `select * from wshop_card_consume where id = ${ args.push(id) }`;
