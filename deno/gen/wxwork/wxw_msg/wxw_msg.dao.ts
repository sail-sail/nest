...
// MARK: syncUsrLblByUsrIdWxwMsg
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdWxwMsg(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameWxwMsg();
  const method = "syncUsrLblByUsrIdWxwMsg";
  ...
  const sql = `update wxwork_wxw_msg set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
