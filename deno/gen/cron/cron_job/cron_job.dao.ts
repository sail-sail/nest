...
async function getWhereQuery(
  ...
  if (search?.job_id && search?.job_id.length > 0) {
...
export async function findAll(
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    // 时区
    let timezone_lbl = model.timezone;
...
export async function setIdByLbl(
  ...
  // 任务
  if (isNotEmpty(input.job_id_lbl) && input.job_id === undefined) {
  ...
  // 时区
  if (isNotEmpty(input.timezone_lbl) && input.timezone === undefined) {
