...
export async function getFieldCommentsCronJob(): Promise<CronJobFieldComment> {
  const field_comments: CronJobFieldComment = {
    id: "ID",
    lbl: "名称",
    job_id: "任务",
    job_id_lbl: "任务",
...
// MARK: syncUsrLblByUsrIdCronJob
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdCronJob(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameCronJob();
  const method = "syncUsrLblByUsrIdCronJob";
  ...
  const sql = `update cron_cron_job set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function updateByIdReturnCronJob(
  id: CronJobId,
  input: CronJobInput,
  options?: {
    is_debug?: boolean;
...
export async function findLastOrderByCronJob(
  ...
  let sql = `select t.order_by from cron_cron_job t`;
  const args = new QueryArgs();
