...
export async function checkByUniqueCronJobLog(
  ...
  if (isEquals) {
    if (uniqueType === UniqueType.Throw) {
      throw new UniqueException("定时任务日志 重复");
    }
...
// MARK: syncUsrLblByUsrIdCronJobLog
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdCronJobLog(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameCronJobLog();
  const method = "syncUsrLblByUsrIdCronJobLog";
  ...
  const sql = `update cron_cron_job_log set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
