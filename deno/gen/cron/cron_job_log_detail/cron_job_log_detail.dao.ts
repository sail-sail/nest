...
export async function getFieldCommentsCronJobLogDetail(): Promise<CronJobLogDetailFieldComment> {
  const field_comments: CronJobLogDetailFieldComment = {
    id: "ID",
...
export async function checkByUniqueCronJobLogDetail(
  ...
  if (isEquals) {
    if (uniqueType === UniqueType.Throw) {
      throw new UniqueException("定时任务日志明细 重复");
    }
...
// MARK: syncUsrLblByUsrIdCronJobLogDetail
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdCronJobLogDetail(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameCronJobLogDetail();
  const method = "syncUsrLblByUsrIdCronJobLogDetail";
  ...
  const sql = `update cron_cron_job_log_detail set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function updateByIdReturnCronJobLogDetail(
  id: CronJobLogDetailId,
  input: CronJobLogDetailInput,
  options?: {
    is_debug?: boolean;
  ...
  await updateByIdCronJobLogDetail(
    id,
    input,
    options,
  );
  
  const model = await findByIdCronJobLogDetail(
    id,
    options,
  );
  
  if (!model) {
    throw new Error(`定时任务日志明细 不存在`);
  }
...
export async function deleteByIdsCronJobLogDetail(
  ...
  const oldModels = await findByIdsOkCronJobLogDetail(ids, options);
  let affectedRows = 0;
