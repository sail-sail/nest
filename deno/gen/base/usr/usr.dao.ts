...
async function getWhereQuery(
  ...
  if (isNotEmpty(search?.role_ids_lbl_like)) {
    whereQuery += ` and base_org.lbl like ${ args.push("%" + sqlLike(search?.role_ids_lbl_like) + "%") }`;
  }
...
// MARK: syncUsrLblByUsrIdUsr
/** 根据 usr_id 同步创建人/更新人标签 */
export async function syncUsrLblByUsrIdUsr(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameUsr();
  const method = "syncUsrLblByUsrIdUsr";
  ...
  const sql = `update base_usr set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function deleteByIdsUsr(
  ...
  const oldModels = await findByIdsOkUsr(ids, options);
  let affectedRows = 0;
