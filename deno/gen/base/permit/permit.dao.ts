...
export async function findAll(
  ...
  if (search && search.menu_id != null) {
    ...
    if (len > FIND_ALL_IDS_LIMIT) {
      throw new Error(`search.menu_id.length > ${ FIND_ALL_IDS_LIMIT }`);
    }
  }
  // 创建人
...
export async function existById(
  ...
  const args = new QueryArgs();
  const sql = `select 1 e from base_permit t where t.id = ${ args.push(id) } and t.is_deleted = 0 limit 1`;
...
async function _creates(
  ...
  const args = new QueryArgs();
  let sql = `insert into base_permit(id,create_time,create_usr_id,menu_id,code,lbl,rem,is_sys)values`;
...
export async function deleteByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    const args = new QueryArgs();
    const sql = `update base_permit set is_deleted=1,delete_time=${ args.push(reqDate()) } where id = ${ args.push(id) } limit 1`;
...
export async function forceDeleteByIds(
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    {
      const args = new QueryArgs();
      const sql = `select * from base_permit where id = ${ args.push(id) }`;
