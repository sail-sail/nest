...
export async function checkByUniqueDictDetail(
  ...
  if (isEquals) {
    if (uniqueType === UniqueType.Throw) {
      throw new UniqueException("系统字典明细 重复");
    }
...
// MARK: syncUsrLblByUsrIdDictDetail
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdDictDetail(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameDictDetail();
  const method = "syncUsrLblByUsrIdDictDetail";
  ...
  const sql = `update base_dict_detail set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
