...
// MARK: syncUsrLblByUsrIdI18n
/** 根据 usr_id 同步创建人/更新人标签 */
export async function syncUsrLblByUsrIdI18n(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameI18n();
  const method = "syncUsrLblByUsrIdI18n";
  ...
  const sql = `update base_i18n set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
