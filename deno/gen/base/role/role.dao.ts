...
// MARK: syncUsrLblByUsrIdRole
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdRole(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameRole();
  const method = "syncUsrLblByUsrIdRole";
  ...
  const sql = `update base_role set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function deleteByIdsRole(
  ...
  const oldModels = await findByIdsOkRole(ids, options);
  let affectedRows = 0;
