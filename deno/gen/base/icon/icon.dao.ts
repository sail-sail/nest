...
// MARK: syncUsrLblByUsrIdIcon
/** 根据 usr_id 同步创建人/更新人标签 */
export async function syncUsrLblByUsrIdIcon(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameIcon();
  const method = "syncUsrLblByUsrIdIcon";
  ...
  const sql = `update base_icon set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
