...
// MARK: syncUsrLblByUsrIdDynPageVal
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdDynPageVal(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameDynPageVal();
  const method = "syncUsrLblByUsrIdDynPageVal";
  ...
  const sql = `update base_dyn_page_val set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function updateByIdReturnDynPageVal(
  id: DynPageValId,
  input: DynPageValInput,
  options?: {
    is_debug?: boolean;
  ...
  const model = await findByIdDynPageVal(
    id,
    options,
  );
  
  if (!model) {
    throw new Error(`动态页面值 不存在`);
  }
...
export async function deleteByIdsDynPageVal(
  ...
  const oldModels = await findByIdsOkDynPageVal(ids, options);
  let affectedRows = 0;
