...
// MARK: syncUsrLblByUsrIdDynPageData
/** 根据 usr_id 同步创建人/更新人标签 */
export async function syncUsrLblByUsrIdDynPageData(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameDynPageData();
  const method = "syncUsrLblByUsrIdDynPageData";
  ...
  const sql = `update base_dyn_page_data set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
