...
/**
 * 通过唯一约束获得数据列表
 * @param {OptbizSearch | PartialNull<OptbizModel>} search0
 */
export async function findByUnique(
  search0: OptbizSearch | PartialNull<OptbizModel>,
  options?: {
  },
): Promise<OptbizModel[]> {
  if (search0.id) {
 ...
 * @param {PartialNull<OptbizModel>} model
 * @return {boolean}
 */
export function equalsByUnique(
  oldModel: OptbizModel,
  model: PartialNull<OptbizModel>,
): boolean {
  if (!oldModel || !model) {
    return false;
  }
