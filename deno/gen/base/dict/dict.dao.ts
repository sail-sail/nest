...
// MARK: syncUsrLblByUsrIdDict
/** 根据 usr_id 同步创建人/更新人标签 */
export async function syncUsrLblByUsrIdDict(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameDict();
  const method = "syncUsrLblByUsrIdDict";
  ...
  const sql = `update base_dict set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function deleteByIdsDict(
  ...
  const oldModels = await findByIdsOkDict(ids, options);
  let affectedRows = 0;
