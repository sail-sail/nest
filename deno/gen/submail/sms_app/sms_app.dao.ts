...
// MARK: syncUsrLblByUsrIdSmsApp
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdSmsApp(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameSmsApp();
  const method = "syncUsrLblByUsrIdSmsApp";
  ...
  const sql = `update submail_sms_app set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
