...
// MARK: syncUsrLblByUsrIdBaiduApp
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdBaiduApp(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameBaiduApp();
  const method = "syncUsrLblByUsrIdBaiduApp";
  ...
  const sql = `update baidu_baidu_app set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
