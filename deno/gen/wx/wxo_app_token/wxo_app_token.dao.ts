...
// MARK: syncUsrLblByUsrIdWxoAppToken
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdWxoAppToken(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameWxoAppToken();
  const method = "syncUsrLblByUsrIdWxoAppToken";
  ...
  const sql = `update wx_wxo_app_token set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function deleteByIdsWxoAppToken(
  ...
  const oldModels = await findByIdsOkWxoAppToken(ids, options);
  let affectedRows = 0;
