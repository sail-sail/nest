...
// MARK: syncUsrLblByUsrIdWxoUsr
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdWxoUsr(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameWxoUsr();
  const method = "syncUsrLblByUsrIdWxoUsr";
  ...
  const sql = `update wx_wxo_usr set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
