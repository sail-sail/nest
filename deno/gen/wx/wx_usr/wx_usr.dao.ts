...
export async function getFieldCommentsWxUsr(): Promise<WxUsrFieldComment> {
  const field_comments: WxUsrFieldComment = {
    id: "ID",
    lbl: "名称",
    usr_id: "用户",
    usr_id_lbl: "用户",
...
// MARK: syncUsrLblByUsrIdWxUsr
/** 根据 usr_id 同步创建人/更新人/删除人标签 */
export async function syncUsrLblByUsrIdWxUsr(
  usr_id: UsrId,
  options?: {
    is_debug?: boolean;
  },
  ...
  const table = getTableNameWxUsr();
  const method = "syncUsrLblByUsrIdWxUsr";
  ...
  const sql = `update wx_wx_usr set ${ sqlFields.join(",") } where ${ whereQuerys.join(" or ") }`;
  const res = await execute(sql, args);
  const affectedRows = res.affectedRows;
...
export async function updateByIdWxUsr(
  ...
  // 头像
  if (input.avatar_img != null && input.avatar_img !== oldModel?.avatar_img) {
    await deleteObject(
      oldModel?.avatar_img,
    );
  }
...
export async function deleteByIdsWxUsr(
  ...
  const oldModels = await findByIdsOkWxUsr(ids, options);
  let affectedRows = 0;
