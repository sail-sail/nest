...
let search = $ref<OptbizSearch>(initSearch());
...
let selectedIds = $(tableSelected.selectedIds as unknown as OptbizId[]);
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 复制 的 业务选项");
    return;
  }
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要编辑的 业务选项");
    return;
  }
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要查看的 业务选项");
    return;
  }
  const search = getDataSearch();
  const is_deleted = search.is_deleted;
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要删除的 业务选项");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定删除已选择的 ${ selectedIds.length } 业务选项?`, {
  ...
  dirtyStore.fireDirty(pageName);
  await dataGrid(true);
  ElMessage.success(`删除 ${ num } 业务选项 成功`);
  emit("remove", num);
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 彻底删除 的 业务选项");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定 彻底删除 已选择的 ${ selectedIds.length } 业务选项?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要还原的 业务选项");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定还原已选择的 ${ selectedIds.length } 业务选项?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  if (num) {
    ...
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(`还原 ${ num } 业务选项 成功`);
    emit("revert", num);
  }
}
