  ...
  >
    ...
    >
      ...
      >
        ...
        <template v-if="field_permit('ky') && (showBuildIn || builtInModel?.ky == null)">
          <el-form-item
            :label="n('键')"
            prop="ky"
          >
...
/** 订阅编辑消息回调 */
async function subscribeEditCallback(id?: OptbizId) {
  if (!id) {
    return;
  }
  ...
  try {
    await ElMessageBox.confirm(
      await nsAsync("此 {0} 已被其他用户编辑，是否刷新?", await nsAsync("业务选项")),
      {
