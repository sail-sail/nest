...
const builtInSearch = $computed(() => {
  ...
  return Object.fromEntries(entries) as unknown as DictbizSearch;
});
...
const builtInModel = $computed(() => {
  ...
  return Object.fromEntries(entries) as unknown as DictbizModel;
});
...
let {
  ...
  rowClkCtrl,
  rowClkShift,
} = $(useSelect<DictbizModel>($$(tableRef)));
...
async function importExcelClk() {
  ...
  const models = await getExcelData<DictbizInput>(file, header);
