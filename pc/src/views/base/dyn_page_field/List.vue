  ...
  >
    ...
    >
      ...
      <template v-if="(showBuildIn || builtInSearch?.dyn_page_id == null)">
        ...
        >
          <CustomInput
            v-model="search.dyn_page_id_lbl_like"
            placeholder="请输入 动态页面"
...
import {
  ...
  useExportExcelDynPageField,
  updateByIdDynPageField,
  importModelsDynPageField,
  useDownloadImportTemplateDynPageField,
} from "./Api.ts";
...
/** 内置查询条件 */
const builtInSearch: DynPageFieldSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: DynPageFieldModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: DynPageFieldSearch) {
  if (!searchStaging) {
    return;
  }
...
/** 表格选择功能 */
const tableSelected = useSelect<DynPageFieldModel, DynPageFieldId>(
  $$(tableRef),
  {
    multiple: $$(multiple),
    isListSelectDialog,
...
/** 表格数据 */
let tableData = $ref<DynPageFieldModel[]>([ ]);
...
const {
  ...
  storeColumns,
  initColumns,
} = $(useTableColumns<DynPageFieldModel>(
  $$(tableColumns),
  {
...
async function useFindAll(
  ...
  if (isPagination) {
    ...
    tableData = await findAllDynPageField(
      search,
      {
        pgSize,
        pgOffset,
...
const exportExcel = $ref(useExportExcelDynPageField());
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 复制 的 动态页面字段");
    return;
  }
...
const downloadImportTemplate = $ref(useDownloadImportTemplateDynPageField());
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<DynPageFieldInput>(
      file,
      header,
      {
        key_types: {
    ...
    const res = await importModelsDynPageField(
      models,
      $$(importPercentage),
      $$(isStopImport),
    );
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要编辑的 动态页面字段");
    return;
  }
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要查看的 动态页面字段");
    return;
  }
  const search = getDataSearch();
  const is_deleted = search.is_deleted;
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要删除的 动态页面字段");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定删除已选择的 ${ selectedIds.length } 动态页面字段?`, {
  ...
  const num = await deleteByIdsDynPageField(selectedIds);
  tableData = tableData.filter((item) => !selectedIds.includes(item.id));
  selectedIds = [ ];
  dirtyStore.fireDirty(pageName);
  await dataGrid(true);
  ElMessage.success(`删除 ${ num } 动态页面字段 成功`);
  emit("remove", num);
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 彻底删除 的 动态页面字段");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定 彻底删除 已选择的 ${ selectedIds.length } 动态页面字段?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await forceDeleteByIdsDynPageField(selectedIds);
  if (num) {
    selectedIds = [ ];
    ElMessage.success(`彻底删除 ${ num } 动态页面字段 成功`);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onEnableByIds(is_enabled: 0 | 1) {
  ...
  if (selectedIds.length === 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = "请选择需要 启用 的 动态页面字段";
    } else {
      msg = "请选择需要 禁用 的 动态页面字段";
    }
    ElMessage.warning(msg);
    return;
  }
  const num = await enableByIdsDynPageField(selectedIds, is_enabled);
  if (num > 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = `启用 ${ num } 动态页面字段 成功`;
    } else {
      msg = `禁用 ${ num } 动态页面字段 成功`;
    }
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要还原的 动态页面字段");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定还原已选择的 ${ selectedIds.length } 动态页面字段?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await revertByIdsDynPageField(selectedIds);
  if (num) {
    search.is_deleted = 0;
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(`还原 ${ num } 动态页面字段 成功`);
    emit("revert", num);
  }
}
