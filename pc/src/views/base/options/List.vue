...
/** 表格选择功能 */
const tableSelected = useSelect<OptionsModel, OptionsId>(
  $$(tableRef),
  {
    multiple: $$(multiple),
    isListSelectDialog,
...
async function onDeleteByIds() {
  ...
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定删除已选择的 {0} {1}", selectedIds.length, await nsAsync("系统选项")) }?`, {
  ...
  if (num) {
    ...
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(await nsAsync("删除 {0} {1} 成功", num, await nsAsync("系统选项")));
    emit("remove", num);
  }
}
...
async function onForceDeleteByIds() {
  ...
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定 彻底删除 已选择的 {0} {1}", selectedIds.length, await nsAsync("系统选项")) }?`, {
      confirmButtonText: await nsAsync("确定"),
      cancelButtonText: await nsAsync("取消"),
      type: "warning",
    });
...
async function onEnableByIds(is_enabled: 0 | 1) {
  ...
  if (num > 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = await nsAsync("启用 {0} {1} 成功", num, await nsAsync("系统选项"));
    } else {
      msg = await nsAsync("禁用 {0} {1} 成功", num, await nsAsync("系统选项"));
    }
...
async function onLockByIds(is_locked: 0 | 1) {
  ...
  if (num > 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = await nsAsync("锁定 {0} {1} 成功", num, await nsAsync("系统选项"));
    } else {
      msg = await nsAsync("解锁 {0} {1} 成功", num, await nsAsync("系统选项"));
    }
    ElMessage.success(msg);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
...
async function onRevertByIds() {
  ...
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定还原已选择的 {0} {1}", selectedIds.length, await nsAsync("系统选项")) }?`, {
      confirmButtonText: await nsAsync("确定"),
      cancelButtonText: await nsAsync("取消"),
      type: "warning",
    });
  ...
  if (num) {
    ...
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(await nsAsync("还原 {0} {1} 成功", num, await nsAsync("系统选项")));
    emit("revert", num);
  }
}
