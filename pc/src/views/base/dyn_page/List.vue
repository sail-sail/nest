...
import {
  ...
  enableByIdsDynPage,
  useExportExcelDynPage,
  updateByIdDynPage,
  importModelsDynPage,
  useDownloadImportTemplateDynPage,
} from "./Api.ts";
...
/** 内置查询条件 */
const builtInSearch: DynPageSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: DynPageModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: DynPageSearch) {
  if (!searchStaging) {
    return;
  }
...
/** 表格数据 */
let tableData = $ref<DynPageModel[]>([ ]);
...
/** 当前表格数据对应的搜索条件 */
let currentSearch = $ref<DynPageSearch>({ });
...
async function useFindAll(
  ...
  if (isPagination) {
    ...
    tableData = await findAllDynPage(
      search,
      {
        pgSize,
        pgOffset,
...
async function useFindCount(
  ...
  page.total = await findCountDynPage(
    search2,
    opt,
  );
}
...
const exportExcel = $ref(useExportExcelDynPage());
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 复制 的 动态页面");
    return;
  }
...
const downloadImportTemplate = $ref(useDownloadImportTemplateDynPage());
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<DynPageInput>(
      file,
      header,
      {
        key_types: {
    ...
    const res = await importModelsDynPage(
      models,
      $$(importPercentage),
      $$(isStopImport),
    );
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要编辑的 动态页面");
    return;
  }
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要查看的 动态页面");
    return;
  }
  const search = getDataSearch();
  const is_deleted = search.is_deleted;
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要删除的 动态页面");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定删除已选择的 ${ selectedIds.length } 动态页面?`, {
  ...
  const num = await deleteByIdsDynPage(selectedIds);
  tableData = tableData.filter((item) => !selectedIds.includes(item.id));
  selectedIds = [ ];
  dirtyStore.fireDirty(pageName);
  await dataGrid(true);
  ElMessage.success(`删除 ${ num } 动态页面 成功`);
  emit("remove", num);
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 彻底删除 的 动态页面");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定 彻底删除 已选择的 ${ selectedIds.length } 动态页面?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await forceDeleteByIdsDynPage(selectedIds);
  if (num) {
    selectedIds = [ ];
    ElMessage.success(`彻底删除 ${ num } 动态页面 成功`);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onEnableByIds(is_enabled: 0 | 1) {
  ...
  const num = await enableByIdsDynPage(selectedIds, is_enabled);
  if (num > 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = `启用 ${ num } 动态页面 成功`;
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要还原的 动态页面");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定还原已选择的 ${ selectedIds.length } 动态页面?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await revertByIdsDynPage(selectedIds);
  if (num) {
    search.is_deleted = 0;
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(`还原 ${ num } 动态页面 成功`);
    emit("revert", num);
  }
}
