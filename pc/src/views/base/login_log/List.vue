...
async function useFindAll(
  ...
  if (isPagination) {
    ...
    tableData = await findAllLoginLog(
      search,
      {
        pgSize,
        pgOffset,
...
async function onDeleteByIds() {
  ...
  const num = await deleteByIdsLoginLog(selectedIds);
...
async function onForceDeleteByIds() {
  ...
  const num = await forceDeleteByIdsLoginLog(selectedIds);
  if (num) {
    selectedIds = [ ];
...
async function onRevertByIds() {
  ...
  const num = await revertByIdsLoginLog(selectedIds);
  if (num) {
    search.is_deleted = 0;
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
