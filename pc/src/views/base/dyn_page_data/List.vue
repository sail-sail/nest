...
const props = defineProps<{
  ...
  ids?: string[]; //ids
  selectedIds?: DynPageDataId[]; //已选择行的id列表
  isMultiple?: string; //是否多选
  id?: DynPageDataId; // ID
  ref_code?: string; // 关联页面路由
  ref_code_like?: string; // 关联页面路由
}>();
...
/** 内置查询条件 */
const builtInSearch: DynPageDataSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: DynPageDataModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: DynPageDataSearch) {
  if (!searchStaging) {
    return;
  }
...
/** 表格数据 */
let tableData = $ref<DynPageDataModel[]>([ ]);
...
} = $(useTableColumns<DynPageDataModel>(
  $$(tableColumns),
  {
...
async function useFindAll(
  ...
  if (isPagination) {
    ...
    tableData = await findAllDynPageData(
      search,
      {
        pgSize,
        pgOffset,
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 复制 的 动态页面数据");
    return;
  }
...
const downloadImportTemplate = $ref(useDownloadImportTemplateDynPageData());
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<DynPageDataInput>(
      file,
      header,
      {
        key_types: {
    ...
    const res = await importModelsDynPageData(
      models,
      $$(importPercentage),
      $$(isStopImport),
    );
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要编辑的 动态页面数据");
    return;
  }
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要查看的 动态页面数据");
    return;
  }
  const search = getDataSearch();
  const is_deleted = search.is_deleted;
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要删除的 动态页面数据");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定删除已选择的 ${ selectedIds.length } 动态页面数据?`, {
  ...
  const num = await deleteByIdsDynPageData(selectedIds);
  tableData = tableData.filter((item) => !selectedIds.includes(item.id));
  selectedIds = [ ];
  dirtyStore.fireDirty(pageName);
  await dataGrid(true);
  ElMessage.success(`删除 ${ num } 动态页面数据 成功`);
  emit("remove", num);
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 彻底删除 的 动态页面数据");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定 彻底删除 已选择的 ${ selectedIds.length } 动态页面数据?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await forceDeleteByIdsDynPageData(selectedIds);
  if (num) {
    selectedIds = [ ];
    ElMessage.success(`彻底删除 ${ num } 动态页面数据 成功`);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要还原的 动态页面数据");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定还原已选择的 ${ selectedIds.length } 动态页面数据?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await revertByIdsDynPageData(selectedIds);
  if (num) {
    search.is_deleted = 0;
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(`还原 ${ num } 动态页面数据 成功`);
    emit("revert", num);
  }
}
