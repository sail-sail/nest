...
let dialogModel: DictDetailInput = $ref({
} as DictDetailInput);

let dict_detail_model = $ref<DictDetailModel>();

let ids = $ref<DictDetailId[]>([ ]);
...
async function showDialog(
  arg?: {
    ...
    findOne?: typeof findOneDictDetail;
    action: DialogAction;
  },
) {
  inited = false;
  ...
  if (arg?.findOne) {
    ...
    findOneModel = findOneDictDetail;
  }
...
async function onRefresh() {
  ...
  if (data) {
    dialogModel = intoInputDictDetail({
      ...data,
    });
    dialogTitle = `${ oldDialogTitle } - ${ dialogModel.lbl }`;
  }
  dict_detail_model = data;
}
...
async function save() {
  ...
  if (dialogAction === "add" || dialogAction === "copy") {
    ...
    id = await createDictDetail(dialogModel2);
    dialogModel.id = id;
    msg = "新增成功";
