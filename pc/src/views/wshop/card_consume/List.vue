...
/** 内置查询条件 */
const builtInSearch: CardConsumeSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: CardConsumeModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
// 卡号
const card_id_search = $computed({
  get() {
    return search.card_id || [ ];
  },
  set(val) {
    if (!val || val.length === 0) {
      search.card_id = undefined;
    } else {
      search.card_id = val;
    }
  },
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: CardConsumeSearch) {
  if (!searchStaging) {
    return;
  }
...
/** 表格选择功能 */
const tableSelected = useSelect<CardConsumeModel, CardConsumeId>(
  $$(tableRef),
  {
    multiple: $$(multiple),
    isListSelectDialog,
...
/** 表格数据 */
let tableData = $ref<CardConsumeModel[]>([ ]);
...
/** 当前表格数据对应的搜索条件 */
let currentSearch = $ref<CardConsumeSearch>({ });
...
async function onSortChange(
  { prop, order, column }: { column: TableColumnCtx<CardConsumeModel> } & Sort,
) {
  if (!order) {
    sort = {
      ..._defaultSort,
...
async function onRowDblclick(
  ...
  column: TableColumnCtx<CardConsumeModel>,
) {
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要查看的 会员卡消费记录");
    return;
  }
  const search = getDataSearch();
  const is_deleted = search.is_deleted;
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要删除的 会员卡消费记录");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定删除已选择的 ${ selectedIds.length } 会员卡消费记录?`, {
  ...
  dirtyStore.fireDirty(pageName);
  await dataGrid(true);
  ElMessage.success(`删除 ${ num } 会员卡消费记录 成功`);
  emit("remove", num);
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 彻底删除 的 会员卡消费记录");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定 彻底删除 已选择的 ${ selectedIds.length } 会员卡消费记录?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要还原的 会员卡消费记录");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定还原已选择的 ${ selectedIds.length } 会员卡消费记录?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  if (num) {
    ...
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(`还原 ${ num } 会员卡消费记录 成功`);
    emit("revert", num);
  }
}
