...
const pagePath = "/wshop/wxapp_config";
...
/** 内置查询条件 */
const builtInSearch: WxappConfigSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: WxappConfigModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: WxappConfigSearch) {
  if (!searchStaging) {
    return;
  }
...
let {
  ...
  onRowEnd,
  tableFocus,
} = $(useSelect<WxappConfigModel, WxappConfigId>(
  $$(tableRef),
  {
    multiple: $$(multiple),
    isListSelectDialog,
...
/** 表格数据 */
let tableData = $ref<WxappConfigModel[]>([ ]);
...
let {
  ...
  resetColumns,
  storeColumns,
} = $(useTableColumns<WxappConfigModel>(
  $$(tableColumns),
  {
...
async function onSortChange(
  { prop, order, column }: { column: TableColumnCtx<WxappConfigModel> } & Sort,
) {
  if (!order) {
    sort = {
      ..._defaultSort,
...
async function openAdd() {
  ...
  const {
    ...
    title: await nsAsync("新增") + await nsAsync("小程序配置"),
    action: "add",
    builtInModel,
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要 复制 的 {0}", await nsAsync("小程序配置")));
    return;
  }
  const {
    changedIds,
  } = await detailRef.showDialog({
    title: await nsAsync("复制") + await nsAsync("小程序配置"),
    action: "copy",
    builtInModel,
    showBuildIn: $$(showBuildIn),
    model: {
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<WxappConfigInput>(
      file,
      header,
      {
        key_types: {
...
/** 锁定 */
async function onIs_locked(id: WxappConfigId, is_locked: 0 | 1) {
  if (isLocked) {
    return;
  }
...
/** 启用 */
async function onIs_enabled(id: WxappConfigId, is_enabled: 0 | 1) {
  if (isLocked) {
    return;
  }
  const notLoading = true;
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要编辑的 {0}", await nsAsync("小程序配置")));
    return;
  }
  const {
    changedIds,
  } = await detailRef.showDialog({
    title: await nsAsync("编辑") + await nsAsync("小程序配置"),
    action: "edit",
    builtInModel,
    showBuildIn: $$(showBuildIn),
    isReadonly: $$(isLocked),
...
async function onRowDblclick(
  row: WxappConfigModel,
  column: TableColumnCtx<WxappConfigModel>,
) {
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要查看的 {0}", await nsAsync("小程序配置")));
    return;
  }
  ...
  const {
    ...
    title: await nsAsync("查看") + await nsAsync("小程序配置"),
    action: "view",
    builtInModel,
    showBuildIn: $$(showBuildIn),
    isLocked: $$(isLocked),
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要删除的 {0}", await nsAsync("小程序配置")));
    return;
  }
  try {
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要 彻底删除 的 {0}", await nsAsync("小程序配置")));
    return;
  }
  try {
...
async function onEnableByIds(is_enabled: 0 | 1) {
  ...
  if (selectedIds.length === 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = await nsAsync("请选择需要 启用 的 {0}", await nsAsync("小程序配置"));
    } else {
      msg = await nsAsync("请选择需要 禁用 的 {0}", await nsAsync("小程序配置"));
    }
...
async function onLockByIds(is_locked: 0 | 1) {
  ...
  if (selectedIds.length === 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = await nsAsync("请选择需要 锁定 的 {0}", await nsAsync("小程序配置"));
    } else {
      msg = await nsAsync("请选择需要 解锁 的 {0}", await nsAsync("小程序配置"));
    }
    ElMessage.warning(msg);
    return;
  }
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要还原的 {0}", await nsAsync("小程序配置")));
    return;
  }
  try {
