    ...
    >
      ...
      >
        ...
        <template v-if="(showBuildIn || builtInModel?.original_price == null)">
          ...
          >
            <CustomInputNumber
              v-model="dialogModel.original_price"
              :max="99999999999.99"
              :precision="2"
              :placeholder="`${ ns('请输入') } ${ n('原价') }`"
              :readonly="isLocked || isReadonly"
            ></CustomInputNumber>
          </el-form-item>
        </template>
        ...
        <template v-if="(showBuildIn || builtInModel?.introduct == null)">
          ...
          >
            <CustomInput
              v-model="dialogModel.introduct"
              :placeholder="`${ ns('请输入') } ${ n('简介') }`"
              :readonly="isLocked || isReadonly"
            ></CustomInput>
          </el-form-item>
        </template>
...
async function onPageUp(e?: KeyboardEvent) {
  ...
  if (!isSucc) {
    ElMessage.warning(await nsAsync("已经是第一个 {0} 了", await nsAsync("产品")));
  }
}
...
watch(
  ...
  () => {
    ...
    if (!dialogModel.pt_type_ids || dialogModel.pt_type_ids.length === 0) {
      dialogModel.pt_type_ids_lbl = [ ];
    }
    if (!dialogModel.is_new) {
      dialogModel.is_new_lbl = "";
    }
  },
);
