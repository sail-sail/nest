            ...
            >
              <template #default="{ row }">
                <CustomSwitch
                  ...
                  v-model="row.is_new"
                  @change="onIs_new(row.id, row.is_new)"
                ></CustomSwitch>
              </template>
            </el-table-column>
          </template>
...
/** 内置查询条件 */
const builtInSearch: PtSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: PtModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: PtSearch) {
  if (!searchStaging) {
    return;
  }
...
/** 表格选择功能 */
const tableSelected = useSelect<PtModel, PtId>(
  $$(tableRef),
  {
    multiple: $$(multiple),
    isListSelectDialog,
...
async function onSortChange(
  { prop, order, column }: { column: TableColumnCtx<PtModel> } & Sort,
) {
  if (!order) {
    sort = {
      ..._defaultSort,
...
async function openAdd() {
  ...
  const {
    ...
    title: await nsAsync("新增") + " " + await nsAsync("产品"),
    action: "add",
    builtInModel,
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要 复制 的 {0}", await nsAsync("产品")));
    return;
  }
  const {
    changedIds,
  } = await detailRef.showDialog({
    title: await nsAsync("复制") + " " + await nsAsync("产品"),
    action: "copy",
    builtInModel,
    showBuildIn: $$(showBuildIn),
    model: {
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<PtInput>(
      file,
      header,
      {
        key_types: {
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要编辑的 {0}", await nsAsync("产品")));
    return;
  }
  const {
    changedIds,
  } = await detailRef.showDialog({
    title: await nsAsync("编辑") + " " + await nsAsync("产品"),
    action: "edit",
    builtInModel,
    showBuildIn: $$(showBuildIn),
    isReadonly: $$(isLocked),
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要查看的 {0}", await nsAsync("产品")));
    return;
  }
  ...
  const {
    ...
    title: await nsAsync("查看") + " " + await nsAsync("产品"),
    action: "view",
    builtInModel,
    showBuildIn: $$(showBuildIn),
    isLocked: $$(isLocked),
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要删除的 {0}", await nsAsync("产品")));
    return;
  }
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定删除已选择的 {0} {1}", selectedIds.length, await nsAsync("产品")) }?`, {
  ...
  if (num) {
    ...
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(await nsAsync("删除 {0} {1} 成功", num, await nsAsync("产品")));
    emit("remove", num);
  }
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要 彻底删除 的 {0}", await nsAsync("产品")));
    return;
  }
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定 彻底删除 已选择的 {0} {1}", selectedIds.length, await nsAsync("产品")) }?`, {
      confirmButtonText: await nsAsync("确定"),
      cancelButtonText: await nsAsync("取消"),
      type: "warning",
    });
  ...
  if (num) {
    selectedIds = [ ];
    ElMessage.success(await nsAsync("彻底删除 {0} {1} 成功", num, await nsAsync("产品")));
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onEnableByIds(is_enabled: 0 | 1) {
  ...
  if (selectedIds.length === 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = await nsAsync("请选择需要 启用 的 {0}", await nsAsync("产品"));
    } else {
      msg = await nsAsync("请选择需要 禁用 的 {0}", await nsAsync("产品"));
    }
  ...
  if (num > 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = await nsAsync("启用 {0} {1} 成功", num, await nsAsync("产品"));
    } else {
      msg = await nsAsync("禁用 {0} {1} 成功", num, await nsAsync("产品"));
    }
...
async function onLockByIds(is_locked: 0 | 1) {
  ...
  if (selectedIds.length === 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = await nsAsync("请选择需要 锁定 的 {0}", await nsAsync("产品"));
    } else {
      msg = await nsAsync("请选择需要 解锁 的 {0}", await nsAsync("产品"));
    }
    ElMessage.warning(msg);
    return;
  }
  ...
  if (num > 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = await nsAsync("锁定 {0} {1} 成功", num, await nsAsync("产品"));
    } else {
      msg = await nsAsync("解锁 {0} {1} 成功", num, await nsAsync("产品"));
    }
    ElMessage.success(msg);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要还原的 {0}", await nsAsync("产品")));
    return;
  }
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定还原已选择的 {0} {1}", selectedIds.length, await nsAsync("产品")) }?`, {
      confirmButtonText: await nsAsync("确定"),
      cancelButtonText: await nsAsync("取消"),
      type: "warning",
    });
  ...
  if (num) {
    ...
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(await nsAsync("还原 {0} {1} 成功", num, await nsAsync("产品")));
    emit("revert", num);
  }
}
