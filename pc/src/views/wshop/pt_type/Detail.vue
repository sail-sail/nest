...
let dialogModel: PtTypeInput = $ref({
} as PtTypeInput);

let pt_type_model = $ref<PtTypeModel>();

let ids = $ref<PtTypeId[]>([ ]);
...
/** 内置变量 */
let builtInModel = $ref<PtTypeInput>();
...
let findOneModel = findOnePtType;
...
watch(
  ...
  () => {
    ...
    if (!dialogModel.is_home) {
      dialogModel.is_home_lbl = "";
    }
    if (!dialogModel.is_recommend) {
      dialogModel.is_recommend_lbl = "";
    }
  },
);
...
async function save() {
  ...
  if (dialogAction === "add" || dialogAction === "copy") {
    ...
    id = await createPtType(dialogModel2);
    dialogModel.id = id;
    msg = "新增成功";
  } else if (dialogAction === "edit" || dialogAction === "view") {
    if (!dialogModel.id) {
