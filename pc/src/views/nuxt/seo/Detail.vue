  ...
  >
    ...
    >
      ...
      >
        ...
        <template v-if="(showBuildIn || builtInModel?.keywords == null)">
          <el-form-item
            :label="n('关键词')"
            prop="keywords"
          >
        ...
        <template v-if="(showBuildIn || builtInModel?.og_description == null)">
          <el-form-item
            :label="n('分享描述')"
            prop="og_description"
          >
...
async function onPageUp(e?: KeyboardEvent) {
  ...
  if (!isSucc) {
    ElMessage.warning(await nsAsync("已经是第一个 {0} 了", await nsAsync("SEO优化")));
  }
}
