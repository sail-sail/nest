  ...
  >
    ...
    >
      ...
      >
        ...
        <template v-if="field_permit('keywords') && (showBuildIn || builtInModel?.keywords == null)">
          <el-form-item
            :label="n('关键词')"
            prop="keywords"
          >
            <CustomInput
              v-model="dialogModel.keywords"
              :placeholder="`${ ns('请输入') } ${ n('关键词') }`"
              :readonly="isLocked || isReadonly"
            ></CustomInput>
          </el-form-item>
        </template>
        
        <template v-if="field_permit('og_image') && (showBuildIn || builtInModel?.og_image == null)">
          <el-form-item
            :label="n('分享图片')"
            prop="og_image"
          >
        ...
        <template v-if="field_permit('og_title') && (showBuildIn || builtInModel?.og_title == null)">
          <el-form-item
            :label="n('分享标题')"
            prop="og_title"
          >
            <CustomInput
              v-model="dialogModel.og_title"
              :placeholder="`${ ns('请输入') } ${ n('分享标题') }`"
              :readonly="isLocked || isReadonly"
            ></CustomInput>
          </el-form-item>
        </template>
        
        <template v-if="field_permit('og_description') && (showBuildIn || builtInModel?.og_description == null)">
          <el-form-item
            :label="n('分享描述')"
            prop="og_description"
          >
            <CustomInput
              v-model="dialogModel.og_description"
              :placeholder="`${ ns('请输入') } ${ n('分享描述') }`"
              :readonly="isLocked || isReadonly"
            ></CustomInput>
          </el-form-item>
        </template>
...
watchEffect(async () => {
  ...
  form_rules = {
    ...
    title: [
      {
        required: true,
        message: `${ await nsAsync("请输入") } ${ n("标题") }`,
      },
      {
        type: "string",
        max: 45,
        message: `${ n("标题") } ${ await nsAsync("长度不能超过 {0}", 45) }`,
      },
    ],
    // 描述
    description: [
      {
        required: true,
        message: `${ await nsAsync("请输入") } ${ n("描述") }`,
      },
      {
        type: "string",
        max: 120,
        message: `${ n("描述") } ${ await nsAsync("长度不能超过 {0}", 120) }`,
      },
    ],
    // 排序
    order_by: [
