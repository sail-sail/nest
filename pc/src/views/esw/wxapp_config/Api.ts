...
export async function findAll(
  ...
  const res = data.findAllWxappConfig;
...
export async function findCount(
  ...
  const res = data.findCountWxappConfig;
  return res;
}
...
export async function findById(
  ...
  const res = data.findByIdWxappConfig;
  return res;
}
...
export function useDownloadImportTemplate(routePath: string) {
  ...
  async function workerFn2() {
    ...
    const buffer = await workerFn(
      `${ location.origin }/import_template/esw/wxapp_config.xlsx`,
      {
        data,
      },
    );
...
export function useExportExcel(routePath: string) {
  ...
  async function workerFn2(
    ...
    try {
      const buffer = await workerFn(
        `${ location.origin }/excel_template/esw/wxapp_config.xlsx`,
        {
          data,
        },
      );
