  ...
  >
    ...
    >
      ...
      >
        ...
        <template v-if="(showBuildIn || builtInModel?.node_id == null)">
          <el-form-item
            label="节点ID"
            prop="node_id"
          >
            <CustomInput
              v-model="dialogModel.node_id"
              placeholder="请输入 节点ID"
              :readonly="isLocked || isReadonly"
            ></CustomInput>
          </el-form-item>
        </template>
        
        <template v-if="(showBuildIn || builtInModel?.node_type == null)">
          <el-form-item
            label="节点类型"
            prop="node_type"
          >
            <DictSelect
              v-model="dialogModel.node_type"
              :set="dialogModel.node_type = dialogModel.node_type ?? undefined"
              code="bpm_node_type"
              placeholder="请选择 节点类型"
              :readonly="isLocked || isReadonly"
            ></DictSelect>
          </el-form-item>
        </template>
...
let dialogModel = $ref<NodeInstInput>({
} as NodeInstInput);

let node_inst_model = $ref<NodeInstModel>();

let ids = $ref<NodeInstId[]>([ ]);
let is_deleted = $ref<0 | 1>(0);
let changedIds = $ref<NodeInstId[]>([ ]);
...
type OnCloseResolveType = {
  type: "ok" | "cancel";
  changedIds: NodeInstId[];
};

let onCloseResolve = function(_value: OnCloseResolveType) { };

/** 内置变量 */
let builtInModel = $ref<NodeInstInput>();
...
let findOneModel = findOneNodeInst;
...
async function showDialog(
  arg?: {
    ...
    findOne?: typeof findOneNodeInst;
    action: DialogAction;
  },
) {
  inited = false;
...
watch(
  ...
  () => {
    ...
    if (!dialogModel.node_type) {
      dialogModel.node_type_lbl = "";
    }
...
async function save() {
  ...
  let id: NodeInstId | undefined = undefined;
  let msg = "";
  if (dialogAction === "add" || dialogAction === "copy") {
    ...
    id = await createNodeInst(dialogModel2);
    dialogModel.id = id;
    msg = "新增成功";
