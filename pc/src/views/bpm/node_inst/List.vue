    ...
    >
      ...
      >
        ...
        >
          ...
          <!-- 节点ID -->
          <template v-else-if="'node_id' === col.prop">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
          ...
          <!-- 节点类型 -->
          <template v-else-if="'node_type_lbl' === col.prop">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
...
import {
  ...
  revertByIdsNodeInst,
  deleteByIdsNodeInst,
  forceDeleteByIdsNodeInst,
  useExportExcelNodeInst,
  importModelsNodeInst,
  useDownloadImportTemplateNodeInst,
} from "./Api.ts";
...
const pagePath = getPagePathNodeInst();
// @ts-ignore
...
const props = defineProps<{
  ...
  ids?: string[]; //ids
  selectedIds?: NodeInstId[]; //已选择行的id列表
...
/** 内置查询条件 */
const builtInSearch: NodeInstSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: NodeInstModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
let search = $ref<NodeInstSearch>(initSearch());
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: NodeInstSearch) {
  if (!searchStaging) {
    return;
  }
...
let selectedIds = $(tableSelected.selectedIds as unknown as NodeInstId[]);
...
/** 表格数据 */
let tableData = $ref<NodeInstModel[]>([ ]);
...
async function useFindAll(
  ...
  if (isPagination) {
    ...
    tableData = await findAllNodeInst(
      search,
      {
        pgSize,
        pgOffset,
...
async function useFindCount(
  ...
  page.total = await findCountNodeInst(
    search,
    opt,
  );
}
...
async function onSortChange(
  { prop, order, column }: { column: TableColumnCtx<NodeInstModel> } & Sort,
) {
  if (!order) {
    sort = {
      ..._defaultSort,
...
const exportExcel = $ref(useExportExcelNodeInst());
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 复制 的 节点实例");
    return;
  }
...
const downloadImportTemplate = $ref(useDownloadImportTemplateNodeInst());
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<NodeInstInput>(
      file,
      header,
      {
        key_types: {
    ...
    const res = await importModelsNodeInst(
      models,
      $$(importPercentage),
      $$(isStopImport),
    );
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要编辑的 节点实例");
    return;
  }
...
async function onRowDblclick(
  row: NodeInstModel,
  column: TableColumnCtx<NodeInstModel>,
) {
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要查看的 节点实例");
    return;
  }
  const search = getDataSearch();
  const is_deleted = search.is_deleted;
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要删除的 节点实例");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定删除已选择的 ${ selectedIds.length } 节点实例?`, {
  ...
  const num = await deleteByIdsNodeInst(selectedIds);
  tableData = tableData.filter((item) => !selectedIds.includes(item.id));
  selectedIds = [ ];
  dirtyStore.fireDirty(pageName);
  await dataGrid(true);
  ElMessage.success(`删除 ${ num } 节点实例 成功`);
  emit("remove", num);
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 彻底删除 的 节点实例");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定 彻底删除 已选择的 ${ selectedIds.length } 节点实例?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await forceDeleteByIdsNodeInst(selectedIds);
  if (num) {
    selectedIds = [ ];
    ElMessage.success(`彻底删除 ${ num } 节点实例 成功`);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要还原的 节点实例");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定还原已选择的 ${ selectedIds.length } 节点实例?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await revertByIdsNodeInst(selectedIds);
  if (num) {
    search.is_deleted = 0;
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(`还原 ${ num } 节点实例 成功`);
    emit("revert", num);
  }
}
