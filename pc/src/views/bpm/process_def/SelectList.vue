...
export type OnCloseResolveType = {
  type: "ok" | "cancel";
  selectedIds?: ProcessDefId[];
  selectedModels?: ProcessDefModel[];
};
export type OnBeforeCloseFnType = (value: OnCloseResolveType) => Promise<boolean | undefined>;
export type OnBeforeChangeFnType = (value: ProcessDefModel[]) => Promise<boolean | undefined>;
...
let selectedIds = $ref<ProcessDefId[]>([ ]);
...
function selectedIdsChg(value: ProcessDefId[]) {
  selectedIds = value;
}
...
/** 双击行 */
async function onRowDblclick(row: { id: ProcessDefId }) {
  selectedIds = [ row.id ];
  await onSave();
}
