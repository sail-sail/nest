...
const emit = defineEmits<{
  (e: "update:modelValue", value?: ProcessDefId | ProcessDefId[]): void,
  (e: "update:modelLabel", value?: string): void,
  (e: "change", value?: ProcessDefModel | ProcessDefModel[]): void,
  (e: "clear"): void,
}>();
...
const props = withDefaults(
  defineProps<{
    modelValue?: ProcessDefId | ProcessDefId[] | null;
...
let selectedValue: ProcessDefModel | (ProcessDefModel | undefined)[] | null | undefined = undefined;
...
function getModelValueArr() {
  let modelValueArr: ProcessDefId[] = [ ];
  if (modelValue) {
    if (Array.isArray(modelValue)) {
      modelValueArr = modelValue;
    } else {
      modelValueArr = modelValue.split(",") as unknown as ProcessDefId[];
    }
  }
  return modelValueArr;
}

async function getModelsByIds(ids: ProcessDefId[]) {
  if (ids.length === 0) {
    return [ ];
  }
  const process_def_models = await findByIdsProcessDef(
    ids,
    {
      notLoading: true,
    },
...
async function onSelectList(value?: ProcessDefModel | ProcessDefModel[]) {
  selectedValue = value;
  if (props.multiple) {
