    ...
    >
      ...
      >
        ...
        >
          ...
          <!-- 原任务 -->
          <template v-if="'task_id_lbl' === col.prop && (showBuildIn || builtInSearch?.task_id == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
          ...
          <!-- 转出人 -->
          <template v-else-if="'from_usr_id_lbl' === col.prop && (showBuildIn || builtInSearch?.from_usr_id == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
          ...
          <!-- 接收人 -->
          <template v-else-if="'to_usr_id_lbl' === col.prop && (showBuildIn || builtInSearch?.to_usr_id == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
...
import {
  ...
  revertByIdsTransfer,
  deleteByIdsTransfer,
  forceDeleteByIdsTransfer,
  useExportExcelTransfer,
  importModelsTransfer,
  useDownloadImportTemplateTransfer,
} from "./Api.ts";
...
const pagePath = getPagePathTransfer();
// @ts-ignore
...
const props = defineProps<{
  ...
  ids?: string[]; //ids
  selectedIds?: TransferId[]; //已选择行的id列表
  isMultiple?: string; //是否多选
  id?: TransferId; // ID
  task_id?: string|string[]; // 原任务
  task_id_lbl?: string; // 原任务
  from_usr_id?: string|string[]; // 转出人
  from_usr_id_lbl?: string; // 转出人
  to_usr_id?: string|string[]; // 接收人
  to_usr_id_lbl?: string; // 接收人
}>();
...
const builtInSearchType: { [key: string]: string } = {
  ...
  task_id_lbl: "string[]",
  from_usr_id: "string[]",
  from_usr_id_lbl: "string[]",
  to_usr_id: "string[]",
  to_usr_id_lbl: "string[]",
...
/** 内置查询条件 */
const builtInSearch: TransferSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: TransferModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
let search = $ref<TransferSearch>(initSearch());
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: TransferSearch) {
  if (!searchStaging) {
    return;
  }
...
let selectedIds = $(tableSelected.selectedIds as unknown as TransferId[]);
...
/** 表格数据 */
let tableData = $ref<TransferModel[]>([ ]);
...
function getTableColumns(): ColumnType[] {
  return [
    ...
    {
      label: "转出人",
      prop: "from_usr_id_lbl",
      sortBy: "from_usr_id_lbl",
      width: 200,
      align: "center",
      headerAlign: "center",
      showOverflowTooltip: true,
...
async function useFindAll(
  ...
  if (isPagination) {
    ...
    tableData = await findAllTransfer(
      search,
      {
        pgSize,
        pgOffset,
...
async function useFindCount(
  ...
  page.total = await findCountTransfer(
    search,
    opt,
  );
}
...
async function onSortChange(
  { prop, order, column }: { column: TableColumnCtx<TransferModel> } & Sort,
) {
  if (!order) {
    sort = {
      ..._defaultSort,
...
const exportExcel = $ref(useExportExcelTransfer());
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 复制 的 转交记录");
    return;
  }
...
const downloadImportTemplate = $ref(useDownloadImportTemplateTransfer());
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<TransferInput>(
      file,
      header,
      {
        key_types: {
    ...
    const res = await importModelsTransfer(
      models,
      $$(importPercentage),
      $$(isStopImport),
    );
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要编辑的 转交记录");
    return;
  }
...
async function onRowDblclick(
  row: TransferModel,
  column: TableColumnCtx<TransferModel>,
) {
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要查看的 转交记录");
    return;
  }
  const search = getDataSearch();
  const is_deleted = search.is_deleted;
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要删除的 转交记录");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定删除已选择的 ${ selectedIds.length } 转交记录?`, {
  ...
  const num = await deleteByIdsTransfer(selectedIds);
  tableData = tableData.filter((item) => !selectedIds.includes(item.id));
  selectedIds = [ ];
  dirtyStore.fireDirty(pageName);
  await dataGrid(true);
  ElMessage.success(`删除 ${ num } 转交记录 成功`);
  emit("remove", num);
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 彻底删除 的 转交记录");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定 彻底删除 已选择的 ${ selectedIds.length } 转交记录?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await forceDeleteByIdsTransfer(selectedIds);
  if (num) {
    selectedIds = [ ];
    ElMessage.success(`彻底删除 ${ num } 转交记录 成功`);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要还原的 转交记录");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定还原已选择的 ${ selectedIds.length } 转交记录?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await revertByIdsTransfer(selectedIds);
  if (num) {
    search.is_deleted = 0;
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(`还原 ${ num } 转交记录 成功`);
    emit("revert", num);
  }
}
