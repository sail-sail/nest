...
>
  ...
  >
    ...
    >
      ...
      <template v-if="(showBuildIn || builtInSearch?.action == null)">
        <el-form-item
          label="动作"
          prop="action"
        >
          <DictSelect
            v-model="action_search"
            code="bpm_log_action"
            placeholder="请选择 动作"
            multiple
            @change="onSearch(false)"
    ...
    >
      ...
      >
        ...
        >
          ...
          <!-- 流程实例 -->
          <template v-if="'process_inst_id_lbl' === col.prop && (showBuildIn || builtInSearch?.process_inst_id == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
          ...
          <!-- 动作 -->
          <template v-else-if="'action_lbl' === col.prop && (showBuildIn || builtInSearch?.action == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
          ...
          <!-- 节点名称 -->
          <template v-else-if="'node_label' === col.prop">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
...
const props = defineProps<{
  ...
  task_id?: string|string[]; // 关联任务
  task_id_lbl?: string; // 关联任务
  action?: string|string[]; // 动作
  usr_id?: string|string[]; // 操作人
  usr_id_lbl?: string; // 操作人
}>();
...
/** 内置查询条件 */
const builtInSearch: LogSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: LogModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
let search = $ref<LogSearch>(initSearch());
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: LogSearch) {
  if (!searchStaging) {
    return;
  }
...
let selectedIds = $(tableSelected.selectedIds as unknown as LogId[]);
...
async function onSortChange(
  { prop, order, column }: { column: TableColumnCtx<LogModel> } & Sort,
) {
  if (!order) {
    sort = {
      ..._defaultSort,
...
const exportExcel = $ref(useExportExcelLog());
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 复制 的 流程日志");
    return;
  }
...
const downloadImportTemplate = $ref(useDownloadImportTemplateLog());
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<LogInput>(
      file,
      header,
      {
        key_types: {
    ...
    const res = await importModelsLog(
      models,
      $$(importPercentage),
      $$(isStopImport),
    );
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要编辑的 流程日志");
    return;
  }
...
async function onRowDblclick(
  row: LogModel,
  column: TableColumnCtx<LogModel>,
) {
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要查看的 流程日志");
    return;
  }
  const search = getDataSearch();
  const is_deleted = search.is_deleted;
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要删除的 流程日志");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定删除已选择的 ${ selectedIds.length } 流程日志?`, {
  ...
  const num = await deleteByIdsLog(selectedIds);
  tableData = tableData.filter((item) => !selectedIds.includes(item.id));
  selectedIds = [ ];
  dirtyStore.fireDirty(pageName);
  await dataGrid(true);
  ElMessage.success(`删除 ${ num } 流程日志 成功`);
  emit("remove", num);
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 彻底删除 的 流程日志");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定 彻底删除 已选择的 ${ selectedIds.length } 流程日志?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await forceDeleteByIdsLog(selectedIds);
  if (num) {
    selectedIds = [ ];
    ElMessage.success(`彻底删除 ${ num } 流程日志 成功`);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要还原的 流程日志");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定还原已选择的 ${ selectedIds.length } 流程日志?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await revertByIdsLog(selectedIds);
  if (num) {
    search.is_deleted = 0;
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(`还原 ${ num } 流程日志 成功`);
    emit("revert", num);
  }
}
