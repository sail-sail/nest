  ...
  >
    ...
    >
      ...
      >
        ...
        <template v-if="(showBuildIn || builtInModel?.process_def_id == null)">
          ...
          >
            <CustomSelect
              ...
              :find-by-values="findByIdsProcessDef"
        ...
        <template v-if="(showBuildIn || builtInModel?.process_revision_id == null)">
          <el-form-item
            label="流程版本"
            prop="process_revision_id"
          >
            <CustomSelect
              v-model="dialogModel.process_revision_id"
              v-model:model-label="dialogModel.process_revision_id_lbl"
              :method="getListProcessRevision"
              :find-by-values="findByIdsProcessRevision"
              :options-map="((item: ProcessRevisionModel) => {
                return {
                  label: item.lbl,
                  value: item.id,
                };
        ...
        <template v-if="(showBuildIn || builtInModel?.start_usr_id == null)">
          <el-form-item
            label="发起人"
            prop="start_usr_id"
          >
            <CustomSelect
              v-model="dialogModel.start_usr_id"
              v-model:model-label="dialogModel.start_usr_id_lbl"
              :method="getListUsr"
        ...
        <template v-if="(showBuildIn || builtInModel?.start_dept_id == null)">
          <el-form-item
            label="发起人部门"
            prop="start_dept_id"
          >
        ...
        <template v-if="(showBuildIn || builtInModel?.current_node_lbls == null) && dialogAction !== 'add' && dialogAction !== 'copy' && dialogAction !== 'edit'">
          <el-form-item
            label="当前节点名称"
            prop="current_node_lbls"
          >
            <CustomInput
              v-model="dialogModel.current_node_lbls"
              placeholder="请输入 当前节点名称"
              :readonly="isLocked || isReadonly"
            ></CustomInput>
          </el-form-item>
        </template>
...
import {
  createProcessInst,
  findOneProcessInst,
  updateByIdProcessInst,
  getDefaultInputProcessInst,
  getPagePathProcessInst,
  intoInputProcessInst,
} from "./Api.ts";

import {
  getListProcessDef,
  getListProcessRevision,
  getListUsr,
} from "./Api.ts";

import {
  findByIdsProcessDef,
} from "@/views/bpm/process_def/Api.ts";

import {
  findByIdsProcessRevision,
} from "@/views/bpm/process_revision/Api.ts";
...
let dialogModel = $ref<ProcessInstInput>({
} as ProcessInstInput);

let process_inst_model = $ref<ProcessInstModel>();

let ids = $ref<ProcessInstId[]>([ ]);
let is_deleted = $ref<0 | 1>(0);
let changedIds = $ref<ProcessInstId[]>([ ]);
...
type OnCloseResolveType = {
  type: "ok" | "cancel";
  changedIds: ProcessInstId[];
};

let onCloseResolve = function(_value: OnCloseResolveType) { };

/** 内置变量 */
let builtInModel = $ref<ProcessInstInput>();
...
let findOneModel = findOneProcessInst;
...
async function showDialog(
  arg?: {
    ...
    findOne?: typeof findOneProcessInst;
    action: DialogAction;
  },
) {
  inited = false;
  ...
  if (arg?.findOne) {
    ...
    findOneModel = findOneProcessInst;
  }
...
async function onRefresh() {
  ...
  if (data) {
    dialogModel = intoInputProcessInst({
      ...data,
    });
    dialogTitle = `${ oldDialogTitle } - ${ dialogModel.lbl }`;
  }
  process_inst_model = data;
}
...
watch(
  ...
  () => {
    ...
    if (!dialogModel.process_revision_id) {
      dialogModel.process_revision_id_lbl = "";
    }
    ...
    if (!dialogModel.start_dept_id) {
      dialogModel.start_dept_id_lbl = "";
    }
  },
);
...
async function save() {
  ...
  let id: ProcessInstId | undefined = undefined;
  let msg = "";
  if (dialogAction === "add" || dialogAction === "copy") {
    ...
    id = await createProcessInst(dialogModel2);
    dialogModel.id = id;
    msg = "新增成功";
    ...
    Object.assign(dialogModel2, { is_deleted: undefined });
    id = await updateByIdProcessInst(
      dialogModel.id,
      dialogModel2,
    );
    msg = "编辑成功";
