...
export type OnCloseResolveType = {
  type: "ok" | "cancel";
  selectedIds?: ProcessRevisionId[];
  selectedModels?: ProcessRevisionModel[];
};
export type OnBeforeCloseFnType = (value: OnCloseResolveType) => Promise<boolean | undefined>;
export type OnBeforeChangeFnType = (value: ProcessRevisionModel[]) => Promise<boolean | undefined>;
...
let selectedIds = $ref<ProcessRevisionId[]>([ ]);
...
function selectedIdsChg(value: ProcessRevisionId[]) {
  selectedIds = value;
}
...
/** 双击行 */
async function onRowDblclick(row: { id: ProcessRevisionId }) {
  selectedIds = [ row.id ];
  await onSave();
}
