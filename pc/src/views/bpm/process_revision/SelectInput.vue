...
const emit = defineEmits<{
  (e: "update:modelValue", value?: ProcessRevisionId | ProcessRevisionId[]): void,
  (e: "update:modelLabel", value?: string): void,
  (e: "change", value?: ProcessRevisionModel | ProcessRevisionModel[]): void,
  (e: "clear"): void,
}>();
...
const props = withDefaults(
  defineProps<{
    modelValue?: ProcessRevisionId | ProcessRevisionId[] | null;
...
let selectedValue: ProcessRevisionModel | (ProcessRevisionModel | undefined)[] | null | undefined = undefined;
...
function getModelValueArr() {
  let modelValueArr: ProcessRevisionId[] = [ ];
  if (modelValue) {
    if (Array.isArray(modelValue)) {
      modelValueArr = modelValue;
    } else {
      modelValueArr = modelValue.split(",") as unknown as ProcessRevisionId[];
    }
  }
  return modelValueArr;
}

async function getModelsByIds(ids: ProcessRevisionId[]) {
  if (ids.length === 0) {
    return [ ];
  }
  const process_revision_models = await findByIdsProcessRevision(
    ids,
    {
      notLoading: true,
    },
...
async function refreshInputValue() {
  ...
  let models: ProcessRevisionModel[];
...
async function onInputChange() {
  ...
  modelValue = "" as ProcessRevisionId;
  emit("update:modelValue", modelValue);
}
...
async function onSelectList(value?: ProcessRevisionModel | ProcessRevisionModel[]) {
  selectedValue = value;
  if (props.multiple) {
