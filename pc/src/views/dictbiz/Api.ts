...
export async function importFile(
  ...
  const data: {
    importModelsDictbiz: Mutation["importModelsDictbiz"];
  } = await mutation({
    query: /* GraphQL */ `
      mutation($models: [DictbizInput!]!) {
        importModelsDictbiz(models: $models)
      }
    `,
    variables: {
      models,
  ...
  const res = data.importModelsDictbiz;
  return res;
}
