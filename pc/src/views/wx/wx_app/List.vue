...
import {
  ...
  updateByIdWxApp,
  importModelsWxApp,
  useDownloadImportTemplateWxApp,
} from "./Api.ts";
...
const pagePath = getPagePathWxApp();
// @ts-ignore
...
let search = $ref<WxAppSearch>(initSearch());
...
let selectedIds = $(tableSelected.selectedIds as unknown as WxAppId[]);
...
const exportExcel = $ref(useExportExcelWxApp());
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 复制 的 小程序设置");
    return;
  }
...
const downloadImportTemplate = $ref(useDownloadImportTemplateWxApp());
...
async function onImportExcel() {
  ...
  try {
    ...
    const res = await importModelsWxApp(
      models,
      $$(importPercentage),
      $$(isStopImport),
    );
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要编辑的 小程序设置");
    return;
  }
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要查看的 小程序设置");
    return;
  }
  const search = getDataSearch();
  const is_deleted = search.is_deleted;
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要删除的 小程序设置");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定删除已选择的 ${ selectedIds.length } 小程序设置?`, {
  ...
  const num = await deleteByIdsWxApp(selectedIds);
  tableData = tableData.filter((item) => !selectedIds.includes(item.id));
  selectedIds = [ ];
  dirtyStore.fireDirty(pageName);
  await dataGrid(true);
  ElMessage.success(`删除 ${ num } 小程序设置 成功`);
  emit("remove", num);
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 彻底删除 的 小程序设置");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定 彻底删除 已选择的 ${ selectedIds.length } 小程序设置?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await forceDeleteByIdsWxApp(selectedIds);
  if (num) {
    selectedIds = [ ];
    ElMessage.success(`彻底删除 ${ num } 小程序设置 成功`);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onEnableByIds(is_enabled: 0 | 1) {
  ...
  if (selectedIds.length === 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = "请选择需要 启用 的 小程序设置";
    } else {
      msg = "请选择需要 禁用 的 小程序设置";
    }
    ElMessage.warning(msg);
    return;
  }
  const num = await enableByIdsWxApp(selectedIds, is_enabled);
  if (num > 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = `启用 ${ num } 小程序设置 成功`;
...
async function onLockByIds(is_locked: 0 | 1) {
  ...
  if (selectedIds.length === 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = "请选择需要 锁定 的 小程序设置";
    } else {
      msg = "请选择需要 解锁 的 小程序设置";
    }
    ElMessage.warning(msg);
    return;
  }
  const num = await lockByIdsWxApp(selectedIds, is_locked);
  if (num > 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = `锁定 ${ num } 小程序设置 成功`;
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要还原的 小程序设置");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定还原已选择的 ${ selectedIds.length } 小程序设置?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await revertByIdsWxApp(selectedIds);
  if (num) {
    search.is_deleted = 0;
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(`还原 ${ num } 小程序设置 成功`);
    emit("revert", num);
  }
}
