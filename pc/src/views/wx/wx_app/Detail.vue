        ...
        <template v-if="(showBuildIn || builtInModel?.default_role_ids == null)">
          ...
          >
            <RoleSelectInput
              ...
              :model-label="wx_app_model?.default_role_ids_lbl"
...
let dialogModel: WxAppInput = $ref({
} as WxAppInput);

let wx_app_model = $ref<WxAppModel>();

let ids = $ref<WxAppId[]>([ ]);
let is_deleted = $ref<0 | 1>(0);
let changedIds = $ref<WxAppId[]>([ ]);
...
/** 内置变量 */
let builtInModel = $ref<WxAppInput>();
...
async function save() {
  ...
  if (dialogAction === "add" || dialogAction === "copy") {
    ...
    id = await createWxApp(dialogModel2);
    dialogModel.id = id;
    msg = "新增成功";
  } else if (dialogAction === "edit" || dialogAction === "view") {
    if (!dialogModel.id) {
