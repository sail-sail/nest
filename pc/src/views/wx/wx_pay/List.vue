    ...
    >
      ...
      >
        ...
        >
          ...
          <!-- 支付终端IP -->
          <template v-else-if="'payer_client_ip' === col.prop && (showBuildIn || builtInSearch?.payer_client_ip == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
          ...
          <!-- 通知地址 -->
          <template v-else-if="'notify_url' === col.prop && (showBuildIn || builtInSearch?.notify_url == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
...
/** 内置查询条件 */
const builtInSearch: WxPaySearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: WxPayModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: WxPaySearch) {
  if (!searchStaging) {
    return;
  }
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<WxPayInput>(
      file,
      header,
      {
        key_types: {
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要查看的 {0}", await nsAsync("微信支付设置")));
    return;
  }
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要删除的 {0}", await nsAsync("微信支付设置")));
    return;
  }
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定删除已选择的 {0} 个 {1}", selectedIds.length, await nsAsync("微信支付设置")) }?`, {
  ...
  if (num) {
    ...
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(await nsAsync("删除 {0} 个 {1} 成功", num, await nsAsync("微信支付设置")));
    emit("remove", num);
  }
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要 彻底删除 的 {0}", await nsAsync("微信支付设置")));
    return;
  }
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定 彻底删除 已选择的 {0} 个 {1}", selectedIds.length, await nsAsync("微信支付设置")) }?`, {
      confirmButtonText: await nsAsync("确定"),
      cancelButtonText: await nsAsync("取消"),
      type: "warning",
    });
  ...
  if (num) {
    selectedIds = [ ];
    ElMessage.success(await nsAsync("彻底删除 {0} 个 {1} 成功", num, await nsAsync("微信支付设置")));
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onEnableByIds(is_enabled: 0 | 1) {
  ...
  if (selectedIds.length === 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = await nsAsync("请选择需要 启用 的 {0}", await nsAsync("微信支付设置"));
    } else {
      msg = await nsAsync("请选择需要 禁用 的 {0}", await nsAsync("微信支付设置"));
    }
  ...
  if (num > 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = await nsAsync("启用 {0} 个 {1} 成功", num, await nsAsync("微信支付设置"));
    } else {
      msg = await nsAsync("禁用 {0} 个 {1} 成功", num, await nsAsync("微信支付设置"));
    }
...
async function onLockByIds(is_locked: 0 | 1) {
  ...
  if (selectedIds.length === 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = await nsAsync("请选择需要 锁定 的 {0}", await nsAsync("微信支付设置"));
    } else {
      msg = await nsAsync("请选择需要 解锁 的 {0}", await nsAsync("微信支付设置"));
    }
    ElMessage.warning(msg);
    return;
  }
  ...
  if (num > 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = await nsAsync("锁定 {0} 个 {1} 成功", num, await nsAsync("微信支付设置"));
    } else {
      msg = await nsAsync("解锁 {0} 个 {1} 成功", num, await nsAsync("微信支付设置"));
    }
    ElMessage.success(msg);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要还原的 {0}", await nsAsync("微信支付设置")));
    return;
  }
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定还原已选择的 {0} 个 {1}", selectedIds.length, await nsAsync("微信支付设置")) }?`, {
      confirmButtonText: await nsAsync("确定"),
      cancelButtonText: await nsAsync("取消"),
      type: "warning",
    });
  ...
  if (num) {
    ...
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(await nsAsync("还原 {0} 个 {1} 成功", num, await nsAsync("微信支付设置")));
    emit("revert", num);
  }
}
