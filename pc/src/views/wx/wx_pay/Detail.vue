    ...
    >
      ...
      >
        ...
        <template v-if="(showBuildIn || builtInModel?.payer_client_ip == null)">
          ...
          >
            <CustomInput
              v-model="dialogModel.payer_client_ip"
              :placeholder="`${ ns('请输入') } ${ n('支付终端IP') }`"
              :readonly="isLocked || isReadonly"
            ></CustomInput>
          </el-form-item>
        </template>
...
/** 内置变量 */
let builtInModel = $ref<WxPayInput>();
...
async function onPageUp(e?: KeyboardEvent) {
  ...
  if (!isSucc) {
    ElMessage.warning(await nsAsync("已经是第一个 {0} 了", await nsAsync("微信支付设置")));
  }
}
