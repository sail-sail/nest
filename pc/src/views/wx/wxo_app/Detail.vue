  ...
  >
    ...
    >
      ...
      >
        ...
        <template v-if="(showBuildIn || builtInModel?.appsecret == null)">
          ...
          >
            <CustomInput
              ...
              placeholder="请输入 开发者密码"
              :readonly="isLocked || isReadonly"
            ></CustomInput>
          </el-form-item>
        </template>
        
        <template v-if="(showBuildIn || builtInModel?.token == null)">
          <el-form-item
            label="令牌"
            prop="token"
          >
            <CustomInput
              v-model="dialogModel.token"
              placeholder="请输入 令牌"
              :readonly="isLocked || isReadonly"
            ></CustomInput>
          </el-form-item>
        ...
        <template v-if="(showBuildIn || builtInModel?.encoding_aes_key == null)">
          <el-form-item
            label="消息加解密密钥"
            prop="encoding_aes_key"
          >
            <CustomInput
              v-model="dialogModel.encoding_aes_key"
              placeholder="请输入 消息加解密密钥"
              :readonly="isLocked || isReadonly"
            ></CustomInput>
          </el-form-item>
        </template>
...
let dialogModel = $ref<WxoAppInput>({
} as WxoAppInput);
...
/** 内置变量 */
let builtInModel = $ref<WxoAppInput>();
...
async function save() {
  ...
  if (dialogAction === "add" || dialogAction === "copy") {
    ...
    id = await createWxoApp(dialogModel2);
    dialogModel.id = id;
    msg = "新增成功";
