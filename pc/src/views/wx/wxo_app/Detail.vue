  ...
  >
    ...
    >
      ...
      >
        ...
        <template v-if="field_permit('appsecret') && (showBuildIn || builtInModel?.appsecret == null)">
          <el-form-item
            :label="n('开发者密码')"
            prop="appsecret"
          >
        ...
        <template v-if="field_permit('token') && (showBuildIn || builtInModel?.token == null)">
          <el-form-item
            :label="n('令牌')"
            prop="token"
          >
            <CustomInput
              ...
              :placeholder="`${ ns('请输入') } ${ n('令牌') }`"
              :readonly="isLocked || isReadonly"
            ></CustomInput>
          </el-form-item>
        </template>
        
        <template v-if="field_permit('encoding_aes_key') && (showBuildIn || builtInModel?.encoding_aes_key == null)">
          <el-form-item
            :label="n('消息加解密密钥')"
          ...
          >
            <CustomInput
              ...
              :placeholder="`${ ns('请输入') } ${ n('消息加解密密钥') }`"
              :readonly="isLocked || isReadonly"
            ></CustomInput>
          </el-form-item>
        </template>
        
        <template v-if="field_permit('domain_id') && (showBuildIn || builtInModel?.domain_id == null)">
          <el-form-item
            :label="n('网页授权域名')"
            prop="domain_id"
          >
...
watchEffect(async () => {
  ...
  form_rules = {
    ...
    domain_id: [
      {
        required: true,
        message: `${ await nsAsync("请选择") } ${ n("网页授权域名") }`,
      },
    ],
    // 排序
    order_by: [
