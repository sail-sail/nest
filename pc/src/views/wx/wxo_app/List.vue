...
>
  ...
  >
    ...
    >
      ...
      <template v-if="field_permit('appid') && (builtInSearch?.appid == null && (showBuildIn || builtInSearch?.appid_like == null))">
        <el-form-item
          :label="n('开发者ID')"
          prop="appid_like"
        >
...
const props = defineProps<{
  ...
  ids?: string[]; //ids
  selectedIds?: WxoAppId[]; //已选择行的id列表
  ...
  domain_id?: string|string[]; // 网页授权域名
  domain_id_lbl?: string; // 网页授权域名
...
/** 内置查询条件 */
const builtInSearch: WxoAppSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: WxoAppModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: WxoAppSearch) {
  if (!searchStaging) {
    return;
  }
...
let {
  ...
  onRowEnd,
  tableFocus,
} = $(useSelect<WxoAppModel, WxoAppId>(
  $$(tableRef),
  {
    multiple: $$(multiple),
    isListSelectDialog,
...
async function openAdd() {
  ...
  const {
    ...
    title: await nsAsync("新增") + " " + await nsAsync("公众号设置"),
    action: "add",
    builtInModel,
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要 复制 的 {0}", await nsAsync("公众号设置")));
    return;
  }
  const {
    changedIds,
  } = await detailRef.showDialog({
    title: await nsAsync("复制") + " " + await nsAsync("公众号设置"),
    action: "copy",
    builtInModel,
    showBuildIn: $$(showBuildIn),
    model: {
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<WxoAppInput>(
      file,
      header,
      {
        key_types: {
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要编辑的 {0}", await nsAsync("公众号设置")));
    return;
  }
  const {
    changedIds,
  } = await detailRef.showDialog({
    title: await nsAsync("编辑") + " " + await nsAsync("公众号设置"),
    action: "edit",
    builtInModel,
    showBuildIn: $$(showBuildIn),
    isReadonly: $$(isLocked),
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要查看的 {0}", await nsAsync("公众号设置")));
    return;
  }
  ...
  const {
    ...
    title: await nsAsync("查看") + " " + await nsAsync("公众号设置"),
    action: "view",
    builtInModel,
    showBuildIn: $$(showBuildIn),
    isLocked: $$(isLocked),
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要删除的 {0}", await nsAsync("公众号设置")));
    return;
  }
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定删除已选择的 {0} {1}", selectedIds.length, await nsAsync("公众号设置")) }?`, {
  ...
  if (num) {
    ...
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(await nsAsync("删除 {0} {1} 成功", num, await nsAsync("公众号设置")));
    emit("remove", num);
  }
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要 彻底删除 的 {0}", await nsAsync("公众号设置")));
    return;
  }
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定 彻底删除 已选择的 {0} {1}", selectedIds.length, await nsAsync("公众号设置")) }?`, {
      confirmButtonText: await nsAsync("确定"),
      cancelButtonText: await nsAsync("取消"),
      type: "warning",
    });
  ...
  if (num) {
    selectedIds = [ ];
    ElMessage.success(await nsAsync("彻底删除 {0} {1} 成功", num, await nsAsync("公众号设置")));
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onEnableByIds(is_enabled: 0 | 1) {
  ...
  if (selectedIds.length === 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = await nsAsync("请选择需要 启用 的 {0}", await nsAsync("公众号设置"));
    } else {
      msg = await nsAsync("请选择需要 禁用 的 {0}", await nsAsync("公众号设置"));
    }
  ...
  if (num > 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = await nsAsync("启用 {0} {1} 成功", num, await nsAsync("公众号设置"));
    } else {
      msg = await nsAsync("禁用 {0} {1} 成功", num, await nsAsync("公众号设置"));
    }
...
async function onLockByIds(is_locked: 0 | 1) {
  ...
  if (selectedIds.length === 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = await nsAsync("请选择需要 锁定 的 {0}", await nsAsync("公众号设置"));
    } else {
      msg = await nsAsync("请选择需要 解锁 的 {0}", await nsAsync("公众号设置"));
    }
    ElMessage.warning(msg);
    return;
  }
  ...
  if (num > 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = await nsAsync("锁定 {0} {1} 成功", num, await nsAsync("公众号设置"));
    } else {
      msg = await nsAsync("解锁 {0} {1} 成功", num, await nsAsync("公众号设置"));
    }
    ElMessage.success(msg);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning(await nsAsync("请选择需要还原的 {0}", await nsAsync("公众号设置")));
    return;
  }
  try {
    await ElMessageBox.confirm(`${ await nsAsync("确定还原已选择的 {0} {1}", selectedIds.length, await nsAsync("公众号设置")) }?`, {
      confirmButtonText: await nsAsync("确定"),
      cancelButtonText: await nsAsync("取消"),
      type: "warning",
    });
  ...
  if (num) {
    ...
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(await nsAsync("还原 {0} {1} 成功", num, await nsAsync("公众号设置")));
    emit("revert", num);
  }
}
