...
import {
  ...
  enableByIdsWxoApp,
  lockByIdsWxoApp,
  useExportExcelWxoApp,
  updateByIdWxoApp,
  importModelsWxoApp,
  useDownloadImportTemplateWxoApp,
} from "./Api.ts";
...
const pagePath = getPagePathWxoApp();
...
/** 当前表格数据对应的搜索条件 */
let currentSearch = $ref<WxoAppSearch>({ });
...
async function useFindCount(
  ...
  page.total = await findCountWxoApp(
    search2,
    opt,
  );
}
...
const exportExcel = $ref(useExportExcelWxoApp());
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 复制 的 公众号设置");
    return;
  }
...
const downloadImportTemplate = $ref(useDownloadImportTemplateWxoApp());
...
async function onImportExcel() {
  ...
  try {
    ...
    const res = await importModelsWxoApp(
      models,
      $$(importPercentage),
      $$(isStopImport),
    );
...
async function openEdit() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要编辑的 公众号设置");
    return;
  }
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要查看的 公众号设置");
    return;
  }
  const search = getDataSearch();
  const is_deleted = search.is_deleted;
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要删除的 公众号设置");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定删除已选择的 ${ selectedIds.length } 公众号设置?`, {
  ...
  const num = await deleteByIdsWxoApp(selectedIds);
  tableData = tableData.filter((item) => !selectedIds.includes(item.id));
  selectedIds = [ ];
  dirtyStore.fireDirty(pageName);
  await dataGrid(true);
  ElMessage.success(`删除 ${ num } 公众号设置 成功`);
  emit("remove", num);
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 彻底删除 的 公众号设置");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定 彻底删除 已选择的 ${ selectedIds.length } 公众号设置?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await forceDeleteByIdsWxoApp(selectedIds);
  if (num) {
    selectedIds = [ ];
    ElMessage.success(`彻底删除 ${ num } 公众号设置 成功`);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onEnableByIds(is_enabled: 0 | 1) {
  ...
  if (selectedIds.length === 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = "请选择需要 启用 的 公众号设置";
    } else {
      msg = "请选择需要 禁用 的 公众号设置";
    }
    ElMessage.warning(msg);
    return;
  }
  const num = await enableByIdsWxoApp(selectedIds, is_enabled);
  if (num > 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = `启用 ${ num } 公众号设置 成功`;
...
async function onLockByIds(is_locked: 0 | 1) {
  ...
  if (selectedIds.length === 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = "请选择需要 锁定 的 公众号设置";
    } else {
      msg = "请选择需要 解锁 的 公众号设置";
    }
    ElMessage.warning(msg);
    return;
  }
  const num = await lockByIdsWxoApp(selectedIds, is_locked);
  if (num > 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = `锁定 ${ num } 公众号设置 成功`;
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要还原的 公众号设置");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定还原已选择的 ${ selectedIds.length } 公众号设置?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await revertByIdsWxoApp(selectedIds);
  if (num) {
    search.is_deleted = 0;
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(`还原 ${ num } 公众号设置 成功`);
    emit("revert", num);
  }
}
