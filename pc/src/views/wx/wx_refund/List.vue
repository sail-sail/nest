...
const props = defineProps<{
  ...
  ids?: string[]; //ids
  selectedIds?: WxRefundId[]; //已选择行的id列表
...
/** 内置查询条件 */
const builtInSearch: WxRefundSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: WxRefundModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 暂存查询 */
async function onSearchStaging(searchStaging?: WxRefundSearch) {
  if (!searchStaging) {
    return;
  }
...
let selectedIds = $(tableSelected.selectedIds as unknown as WxRefundId[]);
...
/** 表格数据 */
let tableData = $ref<WxRefundModel[]>([ ]);
...
function getTableColumns(): ColumnType[] {
  return [
    ...
    {
      label: "资金账户",
      prop: "funds_account_lbl",
      sortBy: "funds_account",
      width: 140,
      align: "center",
      headerAlign: "center",
      showOverflowTooltip: true,
    ...
    {
      label: "退款币种",
      prop: "amount_currency_lbl",
      sortBy: "amount_currency",
      width: 100,
      align: "center",
      headerAlign: "center",
      showOverflowTooltip: true,
...
async function useFindAll(
  ...
  if (isPagination) {
    ...
    tableData = await findAllWxRefund(
      search,
      {
        pgSize,
        pgOffset,
...
async function useFindCount(
  ...
  page.total = await findCountWxRefund(
    search,
    opt,
  );
}
...
const exportExcel = $ref(useExportExcelWxRefund());
...
async function openView() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要查看的 微信退款申请");
    return;
  }
  const search = getDataSearch();
