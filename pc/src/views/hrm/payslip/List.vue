    ...
    >
      ...
      >
        ...
        >
          ...
          <!-- 工号 -->
          <template v-else-if="'job_num' === col.prop && (showBuildIn || builtInSearch?.job_num == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
          ...
          <!-- 已发送 -->
          <template v-else-if="'is_send_lbl' === col.prop && (showBuildIn || builtInSearch?.is_send == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
          ...
          <!-- 已确认 -->
          <template v-else-if="'is_confirm_lbl' === col.prop && (showBuildIn || builtInSearch?.is_confirm == null)">
            <el-table-column
              v-if="col.hide !== true"
              v-bind="col"
            >
...
/** 内置搜索条件 */
const builtInSearch: PayslipSearch = $(initBuiltInSearch(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 内置变量 */
const builtInModel: PayslipModel = $(initBuiltInModel(
  props,
  builtInSearchType,
  propsNotInSearch,
));
...
/** 表格数据 */
let tableData = $ref<PayslipModel[]>([ ]);
...
async function onSortChange(
  { prop, order, column }: { column: TableColumnCtx<PayslipModel> } & Sort,
) {
  let sortBy = "";
...
let exportExcel = $ref(useExportExcel("/hrm/payslip"));
...
const downloadImportTemplate = $ref(useDownloadImportTemplate("/hrm/payslip"));
...
async function onImportExcel() {
  ...
  try {
    ...
    const models = await getExcelData<PayslipInput>(
      file,
      header,
      {
        date_keys: [
