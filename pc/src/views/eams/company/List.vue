...
import {
  ...
  enableByIdsCompany,
  lockByIdsCompany,
  useExportExcelCompany,
  updateByIdCompany,
  importModelsCompany,
  useDownloadImportTemplateCompany,
} from "./Api.ts";
...
/** 当前表格数据对应的搜索条件 */
let currentSearch = $ref<CompanySearch>({ });
...
async function useFindAll(
  ...
  if (isPagination) {
    ...
    tableData = await findAllCompany(
      search,
      {
        pgSize,
        pgOffset,
...
async function useFindCount(
  ...
  page.total = await findCountCompany(
    search2,
    opt,
  );
}
...
const exportExcel = $ref(useExportExcelCompany());
...
async function openCopy() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 复制 的 单位");
    return;
  }
...
const downloadImportTemplate = $ref(useDownloadImportTemplateCompany());
...
async function onImportExcel() {
  ...
  try {
    ...
    const res = await importModelsCompany(
      models,
      $$(importPercentage),
      $$(isStopImport),
    );
...
async function onDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要删除的 单位");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定删除已选择的 ${ selectedIds.length } 单位?`, {
  ...
  const num = await deleteByIdsCompany(selectedIds);
  tableData = tableData.filter((item) => !selectedIds.includes(item.id));
  selectedIds = [ ];
  dirtyStore.fireDirty(pageName);
  await dataGrid(true);
  ElMessage.success(`删除 ${ num } 单位 成功`);
  emit("remove", num);
}
...
async function onForceDeleteByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要 彻底删除 的 单位");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定 彻底删除 已选择的 ${ selectedIds.length } 单位?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await forceDeleteByIdsCompany(selectedIds);
  if (num) {
    selectedIds = [ ];
    ElMessage.success(`彻底删除 ${ num } 单位 成功`);
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
  }
}
...
async function onEnableByIds(is_enabled: 0 | 1) {
  ...
  const num = await enableByIdsCompany(selectedIds, is_enabled);
  if (num > 0) {
    let msg = "";
    if (is_enabled === 1) {
      msg = `启用 ${ num } 单位 成功`;
...
async function onLockByIds(is_locked: 0 | 1) {
  ...
  const num = await lockByIdsCompany(selectedIds, is_locked);
  if (num > 0) {
    let msg = "";
    if (is_locked === 1) {
      msg = `锁定 ${ num } 单位 成功`;
...
async function onRevertByIds() {
  ...
  if (selectedIds.length === 0) {
    ElMessage.warning("请选择需要还原的 单位");
    return;
  }
  try {
    await ElMessageBox.confirm(`确定还原已选择的 ${ selectedIds.length } 单位?`, {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
  ...
  const num = await revertByIdsCompany(selectedIds);
  if (num) {
    search.is_deleted = 0;
    dirtyStore.fireDirty(pageName);
    await dataGrid(true);
    ElMessage.success(`还原 ${ num } 单位 成功`);
    emit("revert", num);
  }
}
