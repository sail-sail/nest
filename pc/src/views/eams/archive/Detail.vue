  ...
  >
    ...
    >
      ...
      >
        ...
        <template v-if="(showBuildIn || builtInModel?.company_id == null)">
          <el-form-item
            :label="n('关联单位')"
            prop="company_id"
          >
...
watchEffect(async () => {
  ...
  form_rules = {
    ...
    code: [
      {
        required: true,
        message: `${ await nsAsync("请输入") } ${ n("编号") }`,
      },
      {
        type: "string",
        max: 22,
        message: `${ n("编号") } ${ await nsAsync("长度不能超过 {0}", 22) }`,
      },
    ],
    // 名称
    lbl: [
...
type OnCloseResolveType = {
  type: "ok" | "cancel";
  changedIds: ArchiveId[];
};

let onCloseResolve = function(_value: OnCloseResolveType) { };

/** 内置变量 */
let builtInModel = $ref<ArchiveInput>();
...
async function onPageUp(e?: KeyboardEvent) {
  ...
  if (!isSucc) {
    ElMessage.warning(await nsAsync("已经是第一个 {0} 了", await nsAsync("全宗设置")));
  }
}
...
watch(
  () => [
    inited,
    dialogModel.company_id,
  ],
  () => {
    if (!inited) {
      return;
    }
    if (!dialogModel.company_id) {
      dialogModel.company_id_lbl = "";
    }
  },
);
