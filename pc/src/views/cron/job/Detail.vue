  ...
  form_rules = {
    ...
    code: [
      ...
      },
      {
        type: "string",
        max: 100,
        message: `${ n("编码") } ${ await nsAsync("长度不能超过 {0}", 100) }`,
      },
    ],
    // 名称
    lbl: [
...
async function onPageUp(e?: KeyboardEvent) {
  ...
  if (!isSucc) {
    ElMessage.warning(await nsAsync("已经是第一个 {0} 了", await nsAsync("任务")));
  }
}
