...
async fn get_where_query(
  ...
  {
    let ids: Option<Vec<WxPayId>> = match search {
  ...
  // 公钥
  {
    let public_key = match search {
      Some(item) => item.public_key.clone(),
      None => None,
    };
    if let Some(public_key) = public_key {
      where_query.push_str(" and t.public_key=?");
      args.push(public_key.into());
    }
    let public_key_like = match search {
  ...
  // 私钥
  {
    let private_key = match search {
      Some(item) => item.private_key.clone(),
      None => None,
    };
    if let Some(private_key) = private_key {
      where_query.push_str(" and t.private_key=?");
      args.push(private_key.into());
    }
    let private_key_like = match search {
  ...
  // APIv3密钥
  {
    let v3_key = match search {
      Some(item) => item.v3_key.clone(),
      None => None,
    };
    if let Some(v3_key) = v3_key {
      where_query.push_str(" and t.v3_key=?");
      args.push(v3_key.into());
    }
    let v3_key_like = match search {
  ...
  // 支付终端IP
  {
    let payer_client_ip = match search {
      Some(item) => item.payer_client_ip.clone(),
      None => None,
    };
    if let Some(payer_client_ip) = payer_client_ip {
      where_query.push_str(" and t.payer_client_ip=?");
      args.push(payer_client_ip.into());
    }
    let payer_client_ip_like = match search {
...
async fn get_from_query(
  ...
  let from_query = r#"wx_wx_pay t"#.to_owned();
  Ok(from_query)
}
...
pub async fn find_all_wx_pay(
  ...
  let mut res: Vec<WxPayModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
// MARK: exists_by_id_wx_pay
/// 根据 id 判断微信支付设置是否存在
#[allow(dead_code)]
pub async fn exists_by_id_wx_pay(
  id: WxPayId,
  options: Option<Options>,
) -> Result<bool> {
...
// MARK: find_by_unique_wx_pay
/// 通过唯一约束获得数据列表
#[allow(unused_variables)]
pub async fn find_by_unique_wx_pay(
  search: WxPaySearch,
  sort: Option<Vec<SortInput>>,
  options: Option<Options>,
) -> Result<Vec<WxPayModel>> {
...
// MARK: check_by_unique_wx_pay
/// 通过唯一约束检查数据是否已经存在
#[allow(unused_variables)]
pub async fn check_by_unique_wx_pay(
  input: WxPayInput,
  model: WxPayModel,
  options: Option<Options>,
...
// MARK: set_id_by_lbl_wx_pay
/// 根据lbl翻译业务字典, 外键关联id, 日期
#[allow(unused_variables, dead_code)]
pub async fn set_id_by_lbl_wx_pay(
  input: WxPayInput,
) -> Result<WxPayInput> {
...
// MARK: creates_return_wx_pay
/// 批量创建微信支付设置并返回
#[allow(dead_code)]
pub async fn creates_return_wx_pay(
...
async fn _creates(
  ...
  let mut ids2: Vec<WxPayId> = vec![];
  let mut inputs2: Vec<WxPayInput> = vec![];
    ...
    .enumerate()
  {
    ...
    let id: WxPayId = get_short_uuid().into();
    ids2.push(id);
...
// MARK: create_return_wx_pay
/// 创建微信支付设置并返回
#[allow(dead_code)]
pub async fn create_return_wx_pay(
  #[allow(unused_mut)]
  mut input: WxPayInput,
  options: Option<Options>,
) -> Result<WxPayModel> {
...
// MARK: update_tenant_by_id_wx_pay
/// 微信支付设置根据id修改租户id
...
// MARK: sync_usr_lbl_by_usr_id_wx_pay
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_wx_pay(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_wx_pay();
  let method = "sync_usr_lbl_by_usr_id_wx_pay";
...
// MARK: del_cache_wx_pay
/// 清空缓存
#[allow(dead_code)]
pub async fn del_cache_wx_pay() -> Result<()> {
...
// MARK: get_is_enabled_by_id_wx_pay
/// 根据 id 查找微信支付设置是否已启用
/// 记录不存在则返回 false
...
// MARK: get_is_locked_by_id_wx_pay
/// 根据 id 查找微信支付设置是否已锁定
/// 已锁定的记录不能修改和删除
/// 记录不存在则返回 false
...
pub async fn revert_by_ids_wx_pay(
  ...
  for id in ids.clone() {
    ...
    let mut old_model = find_one_wx_pay(
      WxPaySearch {
        id: Some(id),
        is_deleted: Some(1),
        ..Default::default()
...
// MARK: force_delete_by_ids_wx_pay
/// 根据 ids 彻底删除微信支付设置
#[allow(unused_variables)]
...
// MARK: find_last_order_by_wx_pay
...
// MARK: validate_is_enabled_wx_pay
/// 校验微信支付设置是否启用
#[allow(dead_code)]
pub async fn validate_is_enabled_wx_pay(
  model: &WxPayModel,
) -> Result<()> {
