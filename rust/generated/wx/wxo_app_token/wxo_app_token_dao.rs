...
pub async fn find_all_wxo_app_token(
  ...
  // 小程序设置
  if let Some(search) = &search && let Some(wxo_app_id) = &search.wxo_app_id {
    let len = wxo_app_id.len();
    if len == 0 {
      return Ok(vec![]);
    }
    if len > ids_limit {
  ...
  let res = {
    ...
    if let Some(str) = str {
      let res2: Vec<WxoAppTokenModel>;
      let res = serde_json::from_str::<Vec<WxoAppTokenModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
  ...
  let mut res: Vec<WxoAppTokenModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
pub async fn find_count_wxo_app_token(
  ...
  // 小程序设置
  if let Some(search) = &search && search.wxo_app_id.is_some() {
...
pub async fn get_field_comments_wxo_app_token(
  ...
  let mut field_comments = WxoAppTokenFieldComment {
    id: "ID".into(),
...
// MARK: find_by_ids_ok_wxo_app_token
...
pub async fn find_by_ids_ok_wxo_app_token(
  ...
  let table = get_table_name_wxo_app_token();
  let method = "find_by_ids_ok_wxo_app_token";
  ...
  let wxo_app_token_models = find_by_ids_wxo_app_token(
    ids.clone(),
    options,
  ).await?;
  ...
  let wxo_app_token_models = ids
    .into_iter()
    .map(|id| {
      let model = wxo_app_token_models
        .iter()
...
pub async fn find_by_ids_wxo_app_token(
  ...
  let wxo_app_token_models = find_all_wxo_app_token(
    search,
    None,
    None,
    options,
...
// MARK: sync_usr_lbl_by_usr_id_wxo_app_token
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_wxo_app_token(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_wxo_app_token();
  let method = "sync_usr_lbl_by_usr_id_wxo_app_token";
...
pub async fn delete_by_ids_wxo_app_token(
  ...
  let old_models = find_by_ids_ok_wxo_app_token(
    ids.clone(),
    options,
  ).await?;
...
pub async fn force_delete_by_ids_wxo_app_token(
  ...
  for id in ids.clone() {
    ...
    let old_model = find_one_wxo_app_token(
      Some(WxoAppTokenSearch {
        id: Some(id),
        is_deleted: Some(1),
        ..Default::default()
      }),
