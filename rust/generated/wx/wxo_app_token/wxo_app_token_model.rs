...
impl std::fmt::Debug for WxoAppTokenInput {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let mut item = &mut f.debug_struct("WxoAppTokenInput");
    if let Some(ref id) = self.id {
      item = item.field("id", id);
    }
    if let Some(ref is_deleted) = self.is_deleted {
  ...
  for item in sort {
    ...
    if !get_can_sort_in_api_wxo_app_token.contains(&prop) {
      return Err(eyre!(ServiceException {
        message: format!("check_sort_wxo_app_token: {}", serde_json::to_string(item)?).into(),
        trace: true,
        ..Default::default()
      }));
    }
...
// MARK: get_page_path_wxo_app_token
pub fn get_page_path_wxo_app_token() -> &'static str {
  "/wx/wxo_app_token"
}

// MARK: get_table_name_wxo_app_token
pub fn get_table_name_wxo_app_token() -> &'static str {
  "wx_wxo_app_token"
}
