...
// MARK: sync_usr_lbl_by_usr_id_wx_app
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_wx_app(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_wx_app();
  let method = "sync_usr_lbl_by_usr_id_wx_app";
...
// MARK: update_by_id_return_wx_app
/// 根据 id 更新小程序设置, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_wx_app(
  id: WxAppId,
  input: WxAppInput,
  options: Option<Options>,
) -> Result<WxAppModel> {
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "小程序设置 update_by_id_return_wx_app id: {id}",
    )),
  }
}
