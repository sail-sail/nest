...
async fn get_where_query(
  ...
  // 昵称
  {
    ...
    if let Some(nick_name_like) = nick_name_like && !nick_name_like.is_empty() {
  ...
  // 头像
  {
    ...
    if let Some(avatar_img_like) = avatar_img_like && !avatar_img_like.is_empty() {
  ...
  // 语言
  {
    ...
    if let Some(language_like) = language_like && !language_like.is_empty() {
...
pub async fn find_all_wx_usr(
  ...
  let table = get_table_name_wx_usr();
  ...
  // 性别
  if let Some(search) = &search && let Some(gender) = &search.gender {
    let len = gender.len();
    if len == 0 {
      return Ok(vec![]);
    }
  ...
  let res = {
    ...
    if let Some(str) = str {
      let res2: Vec<WxUsrModel>;
      let res = serde_json::from_str::<Vec<WxUsrModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
  ...
  let mut res: Vec<WxUsrModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
pub async fn find_count_wx_usr(
  ...
  // 性别
  if let Some(search) = &search && search.gender.is_some() {
...
pub async fn get_field_comments_wx_usr(
  ...
  let mut field_comments = WxUsrFieldComment {
    id: "ID".into(),
    lbl: "名称".into(),
    usr_id: "用户".into(),
    usr_id_lbl: "用户".into(),
...
pub async fn find_by_id_ok_wx_usr(
  ...
  let Some(wx_usr_model) = wx_usr_model else {
    let err_msg = SmolStr::new("此 小程序用户 已被删除");
    error!(
...
pub async fn find_by_ids_ok_wx_usr(
  ...
  let wx_usr_models = ids
    ...
    .map(|id| {
      ...
      let err_msg = SmolStr::new("此 小程序用户 已经被删除");
      Err(eyre!(err_msg))
    })
...
// MARK: sync_usr_lbl_by_usr_id_wx_usr
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_wx_usr(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_wx_usr();
  let method = "sync_usr_lbl_by_usr_id_wx_usr";
...
pub async fn validate_option_wx_usr(
  ...
  let model = match model {
    Some(model) => model,
    None => {
      let err_msg = SmolStr::new("小程序用户不存在");
      error!(
        "{req_id} {err_msg}",
        req_id = get_req_id(),
      );
