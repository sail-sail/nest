...
pub struct WxUsrId([u8; 22]);

impl Serialize for WxUsrId {
...
impl<'de> Deserialize<'de> for WxUsrId {
...
impl fmt::Debug for WxUsrId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "WxUsrId({s})"),
      Err(_) => write!(f, "WxUsrId()")
    }
  }
}

impl fmt::Display for WxUsrId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "{s}"),
      Err(_) => write!(f, "")
...
#[async_graphql::Scalar(name = "WxUsrId")]
impl async_graphql::ScalarType for WxUsrId {
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("WxUsrId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl From<WxUsrId> for ArgType {
  fn from(value: WxUsrId) -> Self {
...
impl From<WxUsrId> for SmolStr {
  fn from(id: WxUsrId) -> Self {
    std::str::from_utf8(&id.0).unwrap_or("").into()
  }
}

impl From<SmolStr> for WxUsrId {
...
impl From<String> for WxUsrId {
  fn from(s: String) -> Self {
    s.as_str().into()
  }
}

impl From<[u8; 22]> for WxUsrId {
...
impl From<WxUsrId> for [u8; 22] {
  fn from(id: WxUsrId) -> Self {
    id.0
  }
}

impl From<&str> for WxUsrId {
  fn from(s: &str) -> Self {
    let bytes = s.as_bytes();
    let mut arr = [0u8; 22];
    if bytes.len() == 22 {
...
impl Encode<'_, MySql> for WxUsrId {
...
impl sqlx::Type<MySql> for WxUsrId {
...
impl<'r> sqlx::Decode<'r, MySql> for WxUsrId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("WxUsrId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}

impl PartialEq<str> for WxUsrId {
