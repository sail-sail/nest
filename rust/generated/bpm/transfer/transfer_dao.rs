...
async fn _creates(
  ...
  for input in inputs {
    ...
    // 转出人
    if (input.from_usr_id_lbl.is_none() || input.from_usr_id_lbl.as_ref().unwrap().is_empty())
      && input.from_usr_id.is_some()
      && !input.from_usr_id.as_ref().unwrap().is_empty()
    {
      let usr_model = crate::base::usr::usr_dao::find_by_id_usr(
        input.from_usr_id.clone().unwrap(),
        Some(Options::new().set_is_debug(Some(false))),
      ).await?;
      if let Some(usr_model) = usr_model {
        input.from_usr_id_lbl = usr_model.lbl.into();
    ...
    // 接收人
    if (input.to_usr_id_lbl.is_none() || input.to_usr_id_lbl.as_ref().unwrap().is_empty())
      && input.to_usr_id.is_some()
      && !input.to_usr_id.as_ref().unwrap().is_empty()
    {
      let usr_model = crate::base::usr::usr_dao::find_by_id_usr(
        input.to_usr_id.clone().unwrap(),
        Some(Options::new().set_is_debug(Some(false))),
      ).await?;
      if let Some(usr_model) = usr_model {
        input.to_usr_id_lbl = usr_model.lbl.into();
...
// MARK: sync_usr_lbl_by_usr_id_transfer
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_transfer(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_transfer();
  let method = "sync_usr_lbl_by_usr_id_transfer";
