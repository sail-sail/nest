...
static CAN_SORT_IN_API_PROCESS_DEF: [&str; 3] = [
  "order_by",
  "create_time",
  "update_time",
];
...
/// 流程定义 前端允许排序的字段
fn get_can_sort_in_api_process_def() -> &'static [&'static str; 3] {
  &CAN_SORT_IN_API_PROCESS_DEF
}

#[derive(SimpleObject, Default, Serialize, Deserialize, Clone, Debug)]
#[graphql(rename_fields = "snake_case", name = "ProcessDefModel")]
#[allow(dead_code)]
pub struct ProcessDefModel {
  /// 租户ID
  #[graphql(skip)]
  ...
  /// 当前生效版本
  #[graphql(name = "current_revision_id_lbl")]
  pub current_revision_id_lbl: SmolStr,
  /// 启用
...
impl FromRow<'_, MySqlRow> for ProcessDefModel {
  fn from_row(row: &MySqlRow) -> sqlx::Result<Self> {
    // 租户ID
    let tenant_id = row.try_get("tenant_id")?;
    // ID
    let id: ProcessDefId = row.try_get("id")?;
    // 编码-序列号
    ...
    let current_revision_id_lbl = SmolStr::new(current_revision_id_lbl.unwrap_or_default());
    // 启用
...
#[graphql(rename_fields = "snake_case", name = "ProcessDefFieldComment")]
#[allow(dead_code)]
pub struct ProcessDefFieldComment {
  /// ID
  #[graphql(name = "id")]
  pub id: SmolStr,
  /// 编码
...
#[graphql(rename_fields = "snake_case", name = "ProcessDefSearch")]
#[allow(dead_code)]
pub struct ProcessDefSearch {
  /// ID
  pub id: Option<ProcessDefId>,
  /// ID列表
  pub ids: Option<Vec<ProcessDefId>>,
  #[graphql(skip)]
  ...
  /// 当前生效版本
  #[graphql(name = "current_revision_id_save_null")]
  pub current_revision_id_is_null: Option<bool>,
  /// 当前生效版本
  #[graphql(name = "current_revision_id_lbl")]
  pub current_revision_id_lbl: Option<Vec<SmolStr>>,
  /// 当前生效版本
  #[graphql(name = "current_revision_id_lbl_like")]
  pub current_revision_id_lbl_like: Option<SmolStr>,
  /// 启用
  #[graphql(name = "is_enabled")]
...
impl std::fmt::Debug for ProcessDefSearch {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let mut item = &mut f.debug_struct("ProcessDefSearch");
    ...
    if let Some(ref current_revision_id_lbl) = self.current_revision_id_lbl {
      item = item.field("current_revision_id_lbl", current_revision_id_lbl);
    }
    if let Some(ref current_revision_id_lbl_like) = self.current_revision_id_lbl_like {
      item = item.field("current_revision_id_lbl_like", current_revision_id_lbl_like);
    }
    if let Some(ref current_revision_id_is_null) = self.current_revision_id_is_null {
      item = item.field("current_revision_id_is_null", current_revision_id_is_null);
    }
    // 启用
...
#[derive(InputObject, Serialize, Deserialize, Default, Clone)]
#[graphql(rename_fields = "snake_case", name = "ProcessDefInput")]
#[allow(dead_code)]
pub struct ProcessDefInput {
  /// ID
  pub id: Option<ProcessDefId>,
  /// 已删除
  #[graphql(skip)]
  ...
  /// 当前生效版本
  #[graphql(name = "current_revision_id_lbl")]
  pub current_revision_id_lbl: Option<SmolStr>,
  /// 启用
  #[graphql(name = "is_enabled")]
...
impl std::fmt::Debug for ProcessDefInput {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let mut item = &mut f.debug_struct("ProcessDefInput");
    if let Some(ref id) = self.id {
      item = item.field("id", id);
    }
    if let Some(ref is_deleted) = self.is_deleted {
...
impl From<ProcessDefModel> for ProcessDefInput {
  fn from(model: ProcessDefModel) -> Self {
    Self {
      id: model.id.into(),
...
impl From<ProcessDefInput> for ProcessDefSearch {
  fn from(input: ProcessDefInput) -> Self {
    Self {
      id: input.id,
      ids: None,
      // 租户ID
      ...
      // 当前生效版本
      current_revision_id_lbl: input.current_revision_id_lbl.map(|x| vec![x]),
      // 启用
...
pub fn check_sort_process_def(
  ...
  let get_can_sort_in_api_process_def = get_can_sort_in_api_process_def();
  ...
  for item in sort {
    ...
    if !get_can_sort_in_api_process_def.contains(&prop) {
      return Err(eyre!(ServiceException {
        message: format!("check_sort_process_def: {}", serde_json::to_string(item)?).into(),
        trace: true,
        ..Default::default()
      }));
    }
...
// MARK: get_page_path_process_def
pub fn get_page_path_process_def() -> &'static str {
  "/bpm/process_def"
}

// MARK: get_table_name_process_def
pub fn get_table_name_process_def() -> &'static str {
  "bpm_process_def"
}
