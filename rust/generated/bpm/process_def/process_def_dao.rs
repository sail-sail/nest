...
async fn _creates(
  ...
  for input in inputs {
    ...
    // 当前生效版本
    if (input.current_revision_id_lbl.is_none() || input.current_revision_id_lbl.as_ref().unwrap().is_empty())
      && input.current_revision_id.is_some()
      && !input.current_revision_id.as_ref().unwrap().is_empty()
    {
      let process_revision_model = crate::bpm::process_revision::process_revision_dao::find_by_id_process_revision(
        input.current_revision_id.clone().unwrap(),
        Some(Options::new().set_is_debug(Some(false))),
      ).await?;
      if let Some(process_revision_model) = process_revision_model {
        input.current_revision_id_lbl = process_revision_model.lbl.into();
...
// MARK: sync_usr_lbl_by_usr_id_process_def
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_process_def(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_process_def();
  let method = "sync_usr_lbl_by_usr_id_process_def";
