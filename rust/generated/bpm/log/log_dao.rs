...
async fn _creates(
  ...
  for input in inputs {
    ...
    // 流程实例
    if (input.process_inst_id_lbl.is_none() || input.process_inst_id_lbl.as_ref().unwrap().is_empty())
      && input.process_inst_id.is_some()
      && !input.process_inst_id.as_ref().unwrap().is_empty()
    {
      let process_inst_model = crate::bpm::process_inst::process_inst_dao::find_by_id_process_inst(
        input.process_inst_id.clone().unwrap(),
        Some(Options::new().set_is_debug(Some(false))),
      ).await?;
      if let Some(process_inst_model) = process_inst_model {
        input.process_inst_id_lbl = process_inst_model.lbl.into();
    ...
    // 节点实例
    if (input.node_inst_id_lbl.is_none() || input.node_inst_id_lbl.as_ref().unwrap().is_empty())
      && input.node_inst_id.is_some()
      && !input.node_inst_id.as_ref().unwrap().is_empty()
    {
      let node_inst_model = crate::bpm::node_inst::node_inst_dao::find_by_id_node_inst(
        input.node_inst_id.clone().unwrap(),
        Some(Options::new().set_is_debug(Some(false))),
      ).await?;
      if let Some(node_inst_model) = node_inst_model {
        input.node_inst_id_lbl = node_inst_model.lbl.into();
    ...
    // 关联任务
    if (input.task_id_lbl.is_none() || input.task_id_lbl.as_ref().unwrap().is_empty())
      && input.task_id.is_some()
      && !input.task_id.as_ref().unwrap().is_empty()
    {
      let task_model = crate::bpm::task::task_dao::find_by_id_task(
        input.task_id.clone().unwrap(),
        Some(Options::new().set_is_debug(Some(false))),
      ).await?;
      if let Some(task_model) = task_model {
        input.task_id_lbl = task_model.lbl.into();
    ...
    // 操作人
    if (input.usr_id_lbl.is_none() || input.usr_id_lbl.as_ref().unwrap().is_empty())
      && input.usr_id.is_some()
      && !input.usr_id.as_ref().unwrap().is_empty()
    {
      let usr_model = crate::base::usr::usr_dao::find_by_id_usr(
        input.usr_id.clone().unwrap(),
        Some(Options::new().set_is_debug(Some(false))),
      ).await?;
      if let Some(usr_model) = usr_model {
        input.usr_id_lbl = usr_model.lbl.into();
...
// MARK: sync_usr_lbl_by_usr_id_log
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_log(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_log();
  let method = "sync_usr_lbl_by_usr_id_log";
