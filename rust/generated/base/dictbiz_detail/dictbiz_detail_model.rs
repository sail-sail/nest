...
static CAN_SORT_IN_API_DICTBIZ_DETAIL: OnceLock<[&'static str; 3]> = OnceLock::new();

/// 业务字典明细 前端允许排序的字段
fn get_can_sort_in_api_dictbiz_detail() -> &'static [&'static str; 3] {
  CAN_SORT_IN_API_DICTBIZ_DETAIL.get_or_init(|| [
    "order_by",
    "create_time",
    "update_time",
  ])
...
#[graphql(rename_fields = "snake_case", name = "DictbizDetailFieldComment")]
#[allow(dead_code)]
...
#[graphql(rename_fields = "snake_case", name = "DictbizDetailSearch")]
#[allow(dead_code)]
...
impl std::fmt::Debug for DictbizDetailSearch {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    ...
    if let Some(ref dictbiz_id_lbl) = self.dictbiz_id_lbl {
      item = item.field("dictbiz_id_lbl", dictbiz_id_lbl);
    }
    if let Some(ref dictbiz_id_lbl_like) = self.dictbiz_id_lbl_like {
      item = item.field("dictbiz_id_lbl_like", dictbiz_id_lbl_like);
    }
...
pub struct DictbizDetailId([u8; 22]);

impl Serialize for DictbizDetailId {
...
impl<'de> Deserialize<'de> for DictbizDetailId {
...
impl fmt::Debug for DictbizDetailId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "DictbizDetailId({s})"),
      Err(_) => write!(f, "DictbizDetailId()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("DictbizDetailId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl From<[u8; 22]> for DictbizDetailId {
...
impl From<DictbizDetailId> for [u8; 22] {
  fn from(id: DictbizDetailId) -> Self {
    id.0
  }
}
...
impl<'r> sqlx::Decode<'r, MySql> for DictbizDetailId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("DictbizDetailId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}

impl PartialEq<str> for DictbizDetailId {
...
pub fn check_sort_dictbiz_detail(
  ...
  let get_can_sort_in_api_dictbiz_detail = get_can_sort_in_api_dictbiz_detail();
  ...
  for item in sort {
    ...
    if !get_can_sort_in_api_dictbiz_detail.contains(&prop) {
      return Err(eyre!("check_sort_dictbiz_detail: {}", serde_json::to_string(item)?));
    }
  }
