...
pub async fn find_all_lang(
  ...
  let res = {
    ...
    if let Some(str) = str {
      ...
      let res = serde_json::from_str::<Vec<LangModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
...
pub async fn find_by_ids_ok_lang(
  ...
  let lang_models = find_by_ids_lang(
    ids.clone(),
    options,
  ).await?;
...
// MARK: sync_usr_lbl_by_usr_id_lang
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_lang(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_lang();
  let method = "sync_usr_lbl_by_usr_id_lang";
...
// MARK: update_by_id_return_lang
/// 根据 id 更新语言, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_lang(
  id: LangId,
  input: LangInput,
  options: Option<Options>,
) -> Result<LangModel> {
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "语言 update_by_id_return_lang id: {id}",
    )),
  }
}
