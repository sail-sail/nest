...
#[graphql(rename_fields = "snake_case", name = "BackgroundTaskFieldComment")]
#[allow(dead_code)]
...
pub struct BackgroundTaskId([u8; 22]);
...
impl fmt::Debug for BackgroundTaskId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "BackgroundTaskId({s})"),
      Err(_) => write!(f, "BackgroundTaskId()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("BackgroundTaskId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl From<[u8; 22]> for BackgroundTaskId {
...
impl From<BackgroundTaskId> for [u8; 22] {
  fn from(id: BackgroundTaskId) -> Self {
    id.0
  }
}
...
impl<'r> sqlx::Decode<'r, MySql> for BackgroundTaskId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("BackgroundTaskId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}
