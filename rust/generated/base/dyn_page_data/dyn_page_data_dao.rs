    ...
    for field in dyn_page_field_models.iter() {
      ...
      if let Some(val_model) = val_model_opt {
        ...
        if [
          ...
          // 数字类型：解析为数字
          let num_val = val_model.lbl.parse::<i64>().unwrap_or_default();
          val = Some(serde_json::json!(num_val));
        } else {
          // 字符串类型：直接使用字符串值
...
async fn _creates(
  ...
  // 更新动态字段
  let ref_code = inputs2
    .first()
    .and_then(|item| item.ref_code.clone());
  let page_path = get_page_path_dyn_page_data(ref_code);
...
// MARK: sync_usr_lbl_by_usr_id_dyn_page_data
/// 根据 usr_id 同步创建人/更新人标签
pub async fn sync_usr_lbl_by_usr_id_dyn_page_data(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_dyn_page_data();
  let method = "sync_usr_lbl_by_usr_id_dyn_page_data";
...
pub async fn update_by_id_dyn_page_data(
  ...
  if let Some(dyn_page_data) = dyn_page_data {
    ...
    let ref_code = input.ref_code.clone();
    let page_path = get_page_path_dyn_page_data(ref_code);
