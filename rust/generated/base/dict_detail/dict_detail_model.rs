...
pub struct DictDetailId([u8; 22]);

impl Serialize for DictDetailId {
...
impl<'de> Deserialize<'de> for DictDetailId {
...
impl fmt::Debug for DictDetailId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "DictDetailId({s})"),
      Err(_) => write!(f, "DictDetailId()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("DictDetailId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl From<[u8; 22]> for DictDetailId {
...
impl From<DictDetailId> for [u8; 22] {
  fn from(id: DictDetailId) -> Self {
    id.0
  }
}
...
impl<'r> sqlx::Decode<'r, MySql> for DictDetailId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("DictDetailId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}
