...
pub async fn find_all_operation_record(
  ...
  let table = get_table_name_operation_record();
...
pub async fn get_field_comments_operation_record(
  ...
  let mut field_comments = OperationRecordFieldComment {
    id: "ID".into(),
    module: "模块".into(),
...
// MARK: find_by_ids_ok_operation_record
...
pub async fn find_by_ids_ok_operation_record(
  ...
  let table = get_table_name_operation_record();
  let method = "find_by_ids_ok_operation_record";
  ...
  let operation_record_models = find_by_ids_operation_record(
    ids.clone(),
    options,
  ).await?;
  ...
  let operation_record_models = ids
    .into_iter()
    .map(|id| {
      let model = operation_record_models
        .iter()
...
pub async fn find_by_ids_operation_record(
  ...
  let operation_record_models = find_all_operation_record(
    search,
    None,
    None,
    options,
...
pub async fn exists_by_id_operation_record(
  ...
  let exists = exists_operation_record(
    search,
    options,
  ).await?;
...
// MARK: sync_usr_lbl_by_usr_id_operation_record
/// 根据 usr_id 同步创建人/更新人标签
pub async fn sync_usr_lbl_by_usr_id_operation_record(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_operation_record();
  let method = "sync_usr_lbl_by_usr_id_operation_record";
...
pub async fn force_delete_by_ids_operation_record(
  ...
  for id in ids.clone() {
    ...
    let old_model = find_one_operation_record(
      Some(OperationRecordSearch {
        id: Some(id),
        is_deleted: Some(1),
        ..Default::default()
      }),
