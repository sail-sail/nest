...
pub async fn find_all_data_permit(
  ...
  let res = {
    ...
    if let Some(str) = str {
      let res2: Vec<DataPermitModel>;
      let res = serde_json::from_str::<Vec<DataPermitModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
  ...
  let mut res: Vec<DataPermitModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
pub async fn create_return_data_permit(
  ...
  let model_data_permit = match model_data_permit {
    Some(model) => model,
    None => {
...
// MARK: sync_usr_lbl_by_usr_id_data_permit
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_data_permit(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_data_permit();
  let method = "sync_usr_lbl_by_usr_id_data_permit";
...
// MARK: update_by_id_return_data_permit
/// 根据 id 更新数据权限, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_data_permit(
  id: DataPermitId,
  input: DataPermitInput,
  options: Option<Options>,
) -> Result<DataPermitModel> {
  
  update_by_id_data_permit(
    id,
    input,
    options,
  ).await?;
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "数据权限 update_by_id_return_data_permit id: {id}",
    )),
  }
}
