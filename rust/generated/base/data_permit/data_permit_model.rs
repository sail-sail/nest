...
pub struct DataPermitId([u8; 22]);

impl Serialize for DataPermitId {
...
impl<'de> Deserialize<'de> for DataPermitId {
...
impl fmt::Debug for DataPermitId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "DataPermitId({s})"),
      Err(_) => write!(f, "DataPermitId()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("DataPermitId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl From<[u8; 22]> for DataPermitId {
...
impl From<DataPermitId> for [u8; 22] {
  fn from(id: DataPermitId) -> Self {
    id.0
  }
}
...
impl<'r> sqlx::Decode<'r, MySql> for DataPermitId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("DataPermitId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}
