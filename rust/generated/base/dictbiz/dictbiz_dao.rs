...
pub async fn find_all_dictbiz(
  ...
  let res = {
    ...
    if let Some(str) = str {
      let res2: Vec<DictbizModel>;
      let res = serde_json::from_str::<Vec<DictbizModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
  ...
  let mut res: Vec<DictbizModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
pub async fn create_return_dictbiz(
  ...
  let model_dictbiz = match model_dictbiz {
    Some(model) => model,
    None => {
...
// MARK: sync_usr_lbl_by_usr_id_dictbiz
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_dictbiz(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_dictbiz();
  let method = "sync_usr_lbl_by_usr_id_dictbiz";
