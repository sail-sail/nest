...
pub struct DictbizId([u8; 22]);

impl Serialize for DictbizId {
...
impl<'de> Deserialize<'de> for DictbizId {
...
impl fmt::Debug for DictbizId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "DictbizId({s})"),
      Err(_) => write!(f, "DictbizId()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("DictbizId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl<'r> sqlx::Decode<'r, MySql> for DictbizId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("DictbizId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}

impl PartialEq<str> for DictbizId {
