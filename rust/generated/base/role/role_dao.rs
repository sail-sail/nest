...
pub async fn find_all_role(
  ...
  let res = {
    ...
    if let Some(str) = str {
      let res2: Vec<RoleModel>;
      let res = serde_json::from_str::<Vec<RoleModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
  ...
  let mut res: Vec<RoleModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
// MARK: sync_usr_lbl_by_usr_id_role
/// 根据 usr_id 同步创建人/更新人标签
pub async fn sync_usr_lbl_by_usr_id_role(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_role();
  let method = "sync_usr_lbl_by_usr_id_role";
...
// MARK: update_by_id_return_role
/// 根据 id 更新角色, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_role(
  id: RoleId,
  input: RoleInput,
  options: Option<Options>,
) -> Result<RoleModel> {
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "角色 update_by_id_return_role id: {id}",
    )),
  }
}
