...
pub async fn find_all_tenant(
  ...
  let res = {
    ...
    if let Some(str) = str {
      let res2: Vec<TenantModel>;
      let res = serde_json::from_str::<Vec<TenantModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
  ...
  let mut res: Vec<TenantModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
pub async fn find_by_ids_ok_tenant(
  ...
  let tenant_models = find_by_ids_tenant(
    ids.clone(),
    options,
  ).await?;
...
// MARK: sync_usr_lbl_by_usr_id_tenant
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_tenant(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_tenant();
  let method = "sync_usr_lbl_by_usr_id_tenant";
...
// MARK: update_by_id_return_tenant
/// 根据 id 更新租户, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_tenant(
  id: TenantId,
  input: TenantInput,
  options: Option<Options>,
) -> Result<TenantModel> {
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "租户 update_by_id_return_tenant id: {id}",
    )),
  }
}
