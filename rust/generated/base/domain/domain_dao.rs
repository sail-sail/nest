...
async fn get_where_query(
  ...
  // 协议
  {
    ...
    if let Some(protocol_like) = protocol_like && !protocol_like.is_empty() {
...
pub async fn find_all_domain(
  ...
  let res = {
    ...
    if let Some(str) = str {
      ...
      let res = serde_json::from_str::<Vec<DomainModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
...
pub async fn find_by_id_ok_domain(
  ...
  let Some(domain_model) = domain_model else {
    let err_msg = SmolStr::new("此 域名 已被删除");
    error!(
...
pub async fn find_by_ids_ok_domain(
  ...
  let domain_models = ids
    ...
    .map(|id| {
      ...
      let err_msg = SmolStr::new("此 域名 已经被删除");
      Err(eyre!(err_msg))
    })
...
// MARK: sync_usr_lbl_by_usr_id_domain
/// 根据 usr_id 同步创建人/更新人标签
pub async fn sync_usr_lbl_by_usr_id_domain(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_domain();
  let method = "sync_usr_lbl_by_usr_id_domain";
