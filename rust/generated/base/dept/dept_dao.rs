...
pub async fn find_all_dept(
  ...
  let res = {
    ...
    if let Some(str) = str {
      let res2: Vec<DeptModel>;
      let res = serde_json::from_str::<Vec<DeptModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
  ...
  let mut res: Vec<DeptModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
// MARK: update_by_id_return_dept
/// 根据 id 更新部门, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_dept(
  id: DeptId,
  input: DeptInput,
  options: Option<Options>,
) -> Result<DeptModel> {
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "部门 update_by_id_return_dept id: {id}",
    )),
  }
}
