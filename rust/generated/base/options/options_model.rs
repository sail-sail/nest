...
static CAN_SORT_IN_API_OPTIONS: [&str; 3] = [
  "order_by",
  "create_time",
  "update_time",
];
...
impl std::fmt::Debug for OptionsInput {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let mut item = &mut f.debug_struct("OptionsInput");
    if let Some(ref id) = self.id {
      item = item.field("id", id);
    }
    if let Some(ref is_deleted) = self.is_deleted {
  ...
  for item in sort {
    ...
    if !get_can_sort_in_api_options.contains(&prop) {
      return Err(eyre!(ServiceException {
        message: format!("check_sort_options: {}", serde_json::to_string(item)?).into(),
        trace: true,
        ..Default::default()
      }));
    }
