...
async fn get_where_query(
  ...
  {
    let ids: Option<Vec<OptionsId>> = match search {
...
pub async fn find_one_options(
  ...
  let model: Option<OptionsModel> = res.into_iter().next();
...
// MARK: exists_by_id_options
/// 根据 id 判断系统选项是否存在
#[allow(dead_code)]
pub async fn exists_by_id_options(
  id: OptionsId,
  options: Option<Options>,
) -> Result<bool> {
...
// MARK: find_by_unique_options
/// 通过唯一约束获得数据列表
#[allow(unused_variables)]
pub async fn find_by_unique_options(
...
// MARK: check_by_unique_options
/// 通过唯一约束检查数据是否已经存在
#[allow(unused_variables)]
pub async fn check_by_unique_options(
  input: OptionsInput,
  model: OptionsModel,
  options: Option<Options>,
...
// MARK: creates_return_options
/// 批量创建系统选项并返回
#[allow(dead_code)]
pub async fn creates_return_options(
...
async fn _creates(
  ...
  let mut ids2: Vec<OptionsId> = vec![];
  let mut inputs2: Vec<OptionsInput> = vec![];
    ...
    .enumerate()
  {
    ...
    let id: OptionsId = get_short_uuid().into();
    ids2.push(id);
...
// MARK: create_return_options
/// 创建系统选项并返回
#[allow(dead_code)]
pub async fn create_return_options(
  #[allow(unused_mut)]
  ...
  let model_options = match model_options {
    Some(model) => model,
    None => {
...
// MARK: sync_usr_lbl_by_usr_id_options
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_options(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_options();
  let method = "sync_usr_lbl_by_usr_id_options";
...
// MARK: get_version_by_id_options
pub async fn get_version_by_id_options(
  id: OptionsId,
  options: Option<Options>,
) -> Result<Option<u32>> {
  ...
  if let Some(options_model) = options_model {
    return Ok(options_model.version.into());
  }
  
  Ok(0.into())
}

// MARK: update_by_id_options
/// 根据 id 修改系统选项
#[allow(unused_mut)]
#[allow(unused_variables)]
...
// MARK: del_cache_options
/// 清空缓存
#[allow(dead_code)]
pub async fn del_cache_options() -> Result<()> {
...
// MARK: get_is_enabled_by_id_options
/// 根据 id 查找系统选项是否已启用
/// 记录不存在则返回 false
...
// MARK: get_is_locked_by_id_options
/// 根据 id 查找系统选项是否已锁定
/// 已锁定的记录不能修改和删除
/// 记录不存在则返回 false
...
pub async fn revert_by_ids_options(
  ...
  for id in ids.clone() {
    ...
    let mut old_model = find_one_options(
      OptionsSearch {
        id: Some(id),
        is_deleted: Some(1),
        ..Default::default()
...
// MARK: force_delete_by_ids_options
/// 根据 ids 彻底删除系统选项
#[allow(unused_variables)]
...
// MARK: find_last_order_by_options
...
// MARK: validate_is_enabled_options
/// 校验系统选项是否启用
#[allow(dead_code)]
pub async fn validate_is_enabled_options(
  model: &OptionsModel,
) -> Result<()> {
