...
pub async fn find_all_optbiz(
  ...
  let mut res: Vec<OptbizModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
pub async fn find_by_ids_ok_optbiz(
  ...
  let optbiz_models = find_by_ids_optbiz(
    ids.clone(),
    options,
  ).await?;
...
pub async fn find_by_ids_optbiz(
  ...
  let optbiz_models = find_all_optbiz(
    search,
    None,
    None,
    options,
...
// MARK: sync_usr_lbl_by_usr_id_optbiz
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_optbiz(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_optbiz();
  let method = "sync_usr_lbl_by_usr_id_optbiz";
