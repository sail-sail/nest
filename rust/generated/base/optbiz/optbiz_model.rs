...
static CAN_SORT_IN_API_OPTBIZ: [&str; 3] = [
  "order_by",
  "create_time",
  "update_time",
];
...
impl FromRow<'_, MySqlRow> for OptbizModel {
  fn from_row(row: &MySqlRow) -> sqlx::Result<Self> {
    ...
    // 键
    let ky: &str = row.try_get("ky")?;
    let ky = SmolStr::new(ky);
    // 值
...
pub struct OptbizSearch {
  ...
  /// 键
  #[graphql(name = "ky")]
  pub ky: Option<SmolStr>,
  /// 键
  #[graphql(name = "ky_like")]
  pub ky_like: Option<SmolStr>,
  /// 值
  #[graphql(name = "val")]
...
impl std::fmt::Debug for OptbizInput {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let mut item = &mut f.debug_struct("OptbizInput");
    if let Some(ref id) = self.id {
      item = item.field("id", id);
    }
    if let Some(ref is_deleted) = self.is_deleted {
  ...
  for item in sort {
    ...
    if !get_can_sort_in_api_optbiz.contains(&prop) {
      return Err(eyre!(ServiceException {
        message: format!("check_sort_optbiz: {}", serde_json::to_string(item)?).into(),
        trace: true,
        ..Default::default()
      }));
    }
