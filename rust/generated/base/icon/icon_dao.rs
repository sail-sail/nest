...
pub async fn find_all_icon(
  ...
  let res = {
    ...
    if let Some(str) = str {
      let res2: Vec<IconModel>;
      let res = serde_json::from_str::<Vec<IconModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
  ...
  let mut res: Vec<IconModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
pub async fn find_by_ids_ok_icon(
  ...
  let icon_models = find_by_ids_icon(
    ids.clone(),
    options,
  ).await?;
...
// MARK: exists_by_id_icon
/// 根据 id 判断图标库是否存在
#[allow(dead_code)]
pub async fn exists_by_id_icon(
  id: IconId,
  options: Option<Options>,
) -> Result<bool> {
...
// MARK: find_by_unique_icon
/// 通过唯一约束获得数据列表
#[allow(unused_variables)]
pub async fn find_by_unique_icon(
  search: IconSearch,
  sort: Option<Vec<SortInput>>,
  options: Option<Options>,
) -> Result<Vec<IconModel>> {
...
// MARK: check_by_unique_icon
/// 通过唯一约束检查数据是否已经存在
#[allow(unused_variables)]
pub async fn check_by_unique_icon(
  input: IconInput,
  model: IconModel,
  options: Option<Options>,
...
// MARK: creates_return_icon
/// 批量创建图标库并返回
#[allow(dead_code)]
pub async fn creates_return_icon(
  ...
  let models_icon = find_by_ids_icon(
    ids,
    options,
  ).await?;
...
async fn _creates(
  ...
  for input in inputs {
    ...
    let old_models = find_by_unique_icon(
      input.clone().into(),
      None,
      options,
    ).await?;
...
// MARK: create_return_icon
/// 创建图标库并返回
#[allow(dead_code)]
pub async fn create_return_icon(
  #[allow(unused_mut)]
  mut input: IconInput,
  options: Option<Options>,
) -> Result<IconModel> {
  ...
  let model_icon = find_by_id_icon(
    id,
    options,
  ).await?;
  
  let model_icon = match model_icon {
    Some(model) => model,
    None => {
      let err_msg = "create_return_icon: model_icon.is_none()";
      return Err(eyre!(
        ServiceException {
...
// MARK: sync_usr_lbl_by_usr_id_icon
/// 根据 usr_id 同步创建人/更新人标签
pub async fn sync_usr_lbl_by_usr_id_icon(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_icon();
  let method = "sync_usr_lbl_by_usr_id_icon";
...
pub async fn update_by_id_icon(
  ...
  
  {
    ...
    let models = find_by_unique_icon(
      input.into(),
      None,
      options,
    ).await?;
...
// MARK: get_is_enabled_by_id_icon
/// 根据 id 查找图标库是否已启用
/// 记录不存在则返回 false
...
pub async fn revert_by_ids_icon(
  ...
  for id in ids.clone() {
    ...
    let mut old_model = find_one_icon(
      IconSearch {
        id: Some(id),
        is_deleted: Some(1),
        ..Default::default()
...
// MARK: validate_is_enabled_icon
/// 校验图标库是否启用
#[allow(dead_code)]
pub async fn validate_is_enabled_icon(
  model: &IconModel,
) -> Result<()> {
