...
static CAN_SORT_IN_API_DYN_PAGE_VAL: OnceLock<[&'static str; 2]> = OnceLock::new();

/// 动态页面值 前端允许排序的字段
fn get_can_sort_in_api_dyn_page_val() -> &'static [&'static str; 2] {
  CAN_SORT_IN_API_DYN_PAGE_VAL.get_or_init(|| [
    "create_time",
    "update_time",
  ])
}
...
#[graphql(rename_fields = "snake_case", name = "DynPageValModel")]
#[allow(dead_code)]
pub struct DynPageValModel {
  /// 租户ID
  #[graphql(skip)]
...
#[graphql(rename_fields = "snake_case", name = "DynPageValFieldComment")]
#[allow(dead_code)]
...
#[graphql(rename_fields = "snake_case", name = "DynPageValSearch")]
#[allow(dead_code)]
pub struct DynPageValSearch {
  /// ID
  pub id: Option<DynPageValId>,
  /// ID列表
  pub ids: Option<Vec<DynPageValId>>,
  #[graphql(skip)]
...
impl std::fmt::Debug for DynPageValSearch {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let mut item = &mut f.debug_struct("DynPageValSearch");
    ...
    if let Some(ref ref_id) = self.ref_id {
      item = item.field("ref_id", ref_id);
    }
    if let Some(ref ref_id_like) = self.ref_id_like {
      item = item.field("ref_id_like", ref_id_like);
    }
    // 字段编码
...
#[derive(InputObject, Serialize, Deserialize, Default, Clone)]
#[graphql(rename_fields = "snake_case", name = "DynPageValInput")]
#[allow(dead_code)]
pub struct DynPageValInput {
  /// ID
  pub id: Option<DynPageValId>,
  /// 已删除
  #[graphql(skip)]
...
impl std::fmt::Debug for DynPageValInput {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let mut item = &mut f.debug_struct("DynPageValInput");
    if let Some(ref id) = self.id {
      item = item.field("id", id);
    }
    if let Some(ref is_deleted) = self.is_deleted {
...
impl From<DynPageValModel> for DynPageValInput {
  fn from(model: DynPageValModel) -> Self {
    Self {
      id: model.id.into(),
...
impl From<DynPageValInput> for DynPageValSearch {
  fn from(input: DynPageValInput) -> Self {
    Self {
      id: input.id,
      ids: None,
      // 租户ID
...
impl_id!(DynPageValId);

/// 动态页面值 检测字段是否允许前端排序
pub fn check_sort_dyn_page_val(
  ...
  let get_can_sort_in_api_dyn_page_val = get_can_sort_in_api_dyn_page_val();
  ...
  for item in sort {
    ...
    if !get_can_sort_in_api_dyn_page_val.contains(&prop) {
      return Err(eyre!("check_sort_dyn_page_val: {}", serde_json::to_string(item)?));
    }
  }
...
// MARK: get_page_path_dyn_page_val
pub fn get_page_path_dyn_page_val() -> &'static str {
  "/base/dyn_page_val"
}

// MARK: get_table_name_dyn_page_val
pub fn get_table_name_dyn_page_val() -> &'static str {
  "base_dyn_page_val"
}
