...
async fn get_where_query(
  ...
  // 关联数据ID
  {
    let ref_id = match search {
      Some(item) => item.ref_id.clone(),
      None => None,
    };
    ...
    let ref_id_like = match search {
      Some(item) => item.ref_id_like.clone(),
      None => None,
    };
    if let Some(ref_id_like) = ref_id_like && !ref_id_like.is_empty() {
      where_query.push_str(" and t.ref_id like ?");
      args.push(format!("%{}%", sql_like(&ref_id_like)).into());
    }
  }
  // 字段编码
  {
...
async fn get_from_query(
  ...
  let from_query = r#"base_dyn_page_val t"#.to_owned();
  Ok(from_query)
}

// MARK: find_all_dyn_page_val
/// 根据搜索条件和分页查找动态页面值列表
...
pub async fn find_all_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "find_all_dyn_page_val";
...
pub async fn find_count_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "find_count_dyn_page_val";
  ...
  // 关联数据ID
  if let Some(search) = &search && search.ref_ids.is_some() {
    let len = search.ref_ids.as_ref().unwrap().len();
    if len == 0 {
      return Ok(0);
    }
    let ids_limit = options
...
// MARK: get_field_comments_dyn_page_val
/// 获取动态页面值字段注释
#[allow(unused_mut)]
pub async fn get_field_comments_dyn_page_val(
  ...
  let mut field_comments = DynPageValFieldComment {
    id: "ID".into(),
...
pub async fn find_one_ok_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "find_one_ok_dyn_page_val";
  ...
  let dyn_page_val_model = find_one_dyn_page_val(
    search,
    sort,
    options,
  ).await?;
...
pub async fn find_one_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "find_one_dyn_page_val";
  ...
  let model: Option<DynPageValModel> = res.into_iter().next();
  
  Ok(model)
}

// MARK: find_by_id_ok_dyn_page_val
...
pub async fn find_by_id_ok_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "find_by_id_ok_dyn_page_val";
  ...
  let dyn_page_val_model = find_by_id_dyn_page_val(
    id,
    options,
  ).await?;
...
pub async fn find_by_id_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "find_by_id_dyn_page_val";
...
// MARK: find_by_ids_ok_dyn_page_val
...
pub async fn find_by_ids_ok_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "find_by_ids_ok_dyn_page_val";
  ...
  let dyn_page_val_models = find_by_ids_dyn_page_val(
    ids.clone(),
    options,
  ).await?;
  ...
  let dyn_page_val_models = ids
    .into_iter()
    .map(|id| {
      let model = dyn_page_val_models
        .iter()
...
pub async fn find_by_ids_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "find_by_ids_dyn_page_val";
  ...
  let dyn_page_val_models = ids
    ...
    .collect::<Vec<DynPageValModel>>();
  
  Ok(dyn_page_val_models)
}

// MARK: exists_dyn_page_val
/// 根据搜索条件判断动态页面值是否存在
#[allow(dead_code)]
pub async fn exists_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "exists_dyn_page_val";
...
// MARK: exists_by_id_dyn_page_val
/// 根据 id 判断动态页面值是否存在
#[allow(dead_code)]
pub async fn exists_by_id_dyn_page_val(
  id: DynPageValId,
  options: Option<Options>,
) -> Result<bool> {
  
  let table = get_table_name_dyn_page_val();
  let method = "exists_by_id_dyn_page_val";
  ...
  let exists = exists_dyn_page_val(
    search,
    options,
  ).await?;
...
// MARK: find_by_unique_dyn_page_val
/// 通过唯一约束获得数据列表
#[allow(unused_variables)]
pub async fn find_by_unique_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "find_by_unique_dyn_page_val";
...
pub fn equals_by_unique(
  ...
  if
    ...
    input.ref_id.as_ref().is_some() && input.ref_id.as_ref().unwrap() == &model.ref_id &&
...
// MARK: check_by_unique_dyn_page_val
/// 通过唯一约束检查数据是否已经存在
#[allow(unused_variables)]
pub async fn check_by_unique_dyn_page_val(
  input: DynPageValInput,
  model: DynPageValModel,
  options: Option<Options>,
) -> Result<Option<DynPageValId>> {
  
  let table = get_table_name_dyn_page_val();
  let method = "check_by_unique_dyn_page_val";
  ...
  if unique_type == UniqueType::Update {
    let id = update_by_id_dyn_page_val(
      model.id,
      input,
      options,
    ).await?;
  ...
  if unique_type == UniqueType::Throw {
    let err_msg = "动态页面值 重复";
    return Err(eyre!(err_msg));
  }
  Ok(None)
}

// MARK: set_id_by_lbl_dyn_page_val
...
// MARK: creates_return_dyn_page_val
/// 批量创建动态页面值并返回
#[allow(dead_code)]
pub async fn creates_return_dyn_page_val(
  inputs: Vec<DynPageValInput>,
  options: Option<Options>,
) -> Result<Vec<DynPageValModel>> {
  
  let table = get_table_name_dyn_page_val();
  let method = "creates_return_dyn_page_val";
  ...
  Ok(models_dyn_page_val)
}

// MARK: creates_dyn_page_val
/// 批量创建动态页面值
pub async fn creates_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "creates_dyn_page_val";
...
async fn _creates(
  ...
  let mut ids2: Vec<DynPageValId> = vec![];
  let mut inputs2: Vec<DynPageValInput> = vec![];
    ...
    .enumerate()
  {
    ...
    let id: DynPageValId = get_short_uuid().into();
    ids2.push(id);
...
// MARK: create_return_dyn_page_val
/// 创建动态页面值并返回
#[allow(dead_code)]
pub async fn create_return_dyn_page_val(
  #[allow(unused_mut)]
  mut input: DynPageValInput,
  options: Option<Options>,
) -> Result<DynPageValModel> {
...
// MARK: create_dyn_page_val
/// 创建动态页面值
#[allow(dead_code)]
pub async fn create_dyn_page_val(
  #[allow(unused_mut)]
  mut input: DynPageValInput,
  options: Option<Options>,
) -> Result<DynPageValId> {
  ...
  let table = get_table_name_dyn_page_val();
  let method = "create_dyn_page_val";
...
// MARK: update_tenant_by_id_dyn_page_val
/// 动态页面值根据id修改租户id
pub async fn update_tenant_by_id_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "update_tenant_by_id_dyn_page_val";
...
// MARK: sync_usr_lbl_by_usr_id_dyn_page_val
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_dyn_page_val(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_dyn_page_val();
  let method = "sync_usr_lbl_by_usr_id_dyn_page_val";
...
// MARK: update_by_id_dyn_page_val
/// 根据 id 修改动态页面值
#[allow(unused_mut)]
#[allow(unused_variables)]
pub async fn update_by_id_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "update_by_id_dyn_page_val";
...
// MARK: delete_by_ids_dyn_page_val
/// 根据 ids 删除动态页面值
#[allow(unused_variables)]
pub async fn delete_by_ids_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "delete_by_ids_dyn_page_val";
...
// MARK: revert_by_ids_dyn_page_val
/// 根据 ids 还原动态页面值
pub async fn revert_by_ids_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "revert_by_ids_dyn_page_val";
  ...
  for id in ids.clone() {
    ...
    let mut old_model = find_one_dyn_page_val(
      DynPageValSearch {
        id: Some(id),
        is_deleted: Some(1),
        ..Default::default()
...
// MARK: force_delete_by_ids_dyn_page_val
/// 根据 ids 彻底删除动态页面值
#[allow(unused_variables)]
pub async fn force_delete_by_ids_dyn_page_val(
  ...
  let table = get_table_name_dyn_page_val();
  let method = "force_delete_by_ids_dyn_page_val";
...
// MARK: validate_option_dyn_page_val
/// 校验动态页面值是否存在
#[allow(dead_code)]
