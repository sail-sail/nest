...
#[graphql(rename_fields = "snake_case", name = "LoginLogFieldComment")]
#[allow(dead_code)]
...
pub struct LoginLogId([u8; 22]);

impl Serialize for LoginLogId {
...
impl<'de> Deserialize<'de> for LoginLogId {
...
impl fmt::Debug for LoginLogId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match std::str::from_utf8(&self.0) {
      Ok(s) => write!(f, "LoginLogId({s})"),
      Err(_) => write!(f, "LoginLogId()")
    }
  }
}
  ...
  fn parse(value: async_graphql::Value) -> async_graphql::InputValueResult<Self> {
    match value {
      async_graphql::Value::String(s) => {
        ...
        if bytes.len() != 22 {
          return Err(async_graphql::InputValueError::custom("LoginLogId must be 22 bytes string or empty"));
        }
        let mut arr = [0u8; 22];
...
impl From<[u8; 22]> for LoginLogId {
...
impl From<LoginLogId> for [u8; 22] {
  fn from(id: LoginLogId) -> Self {
    id.0
  }
}
...
impl<'r> sqlx::Decode<'r, MySql> for LoginLogId {
  fn decode(
    ...
    if bytes.len() == 22 {
      ...
      return Err("LoginLogId must be 22 bytes".into());
    }
    Ok(Self(arr))
  }
}
