...
pub async fn find_all_wxw_usr(
  ...
  let res = {
    ...
    if let Some(str) = str {
      let res2: Vec<WxwUsrModel>;
      let res = serde_json::from_str::<Vec<WxwUsrModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
  ...
  let mut res: Vec<WxwUsrModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
pub async fn create_return_wxw_usr(
  ...
  let model_wxw_usr = match model_wxw_usr {
    Some(model) => model,
    None => {
...
// MARK: sync_usr_lbl_by_usr_id_wxw_usr
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_wxw_usr(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_wxw_usr();
  let method = "sync_usr_lbl_by_usr_id_wxw_usr";
...
// MARK: update_by_id_return_wxw_usr
/// 根据 id 更新企微用户, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_wxw_usr(
  id: WxwUsrId,
  input: WxwUsrInput,
  options: Option<Options>,
) -> Result<WxwUsrModel> {
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "企微用户 update_by_id_return_wxw_usr id: {id}",
    )),
  }
}
