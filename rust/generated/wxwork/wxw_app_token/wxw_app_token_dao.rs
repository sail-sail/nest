...
async fn get_where_query(
  ...
  // 密钥
  {
    ...
    if let Some(corpsecret_like) = corpsecret_like && !corpsecret_like.is_empty() {
  ...
  // 通讯录密钥
  {
    ...
    if let Some(contactsecret_like) = contactsecret_like && !contactsecret_like.is_empty() {
  ...
  // 企业jsapi_ticket
  {
    ...
    if let Some(jsapi_ticket_like) = jsapi_ticket_like && !jsapi_ticket_like.is_empty() {
  ...
  // 应用jsapi_ticket
  {
    ...
    if let Some(jsapi_ticket_agent_config_like) = jsapi_ticket_agent_config_like && !jsapi_ticket_agent_config_like.is_empty() {
...
pub async fn find_all_wxw_app_token(
  ...
  let res = {
    ...
    if let Some(str) = str {
      ...
      let res = serde_json::from_str::<Vec<WxwAppTokenModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
...
pub async fn find_by_id_ok_wxw_app_token(
  ...
  let Some(wxw_app_token_model) = wxw_app_token_model else {
    let err_msg = SmolStr::new("此 企微应用接口凭据 已被删除");
    error!(
...
pub async fn find_by_ids_ok_wxw_app_token(
  ...
  let wxw_app_token_models = ids
    ...
    .map(|id| {
      ...
      let err_msg = SmolStr::new("此 企微应用接口凭据 已经被删除");
      Err(eyre!(err_msg))
    })
...
pub async fn set_id_by_lbl_wxw_app_token(
  ...
  // 企业jsapi_ticket创建时间
  if input.jsapi_ticket_time.is_none() && let Some(jsapi_ticket_time_lbl) = input.jsapi_ticket_time_lbl.as_ref().filter(|s| !s.is_empty()) {
  ...
  // 应用jsapi_ticket创建时间
  if input.jsapi_ticket_agent_config_time.is_none() && let Some(jsapi_ticket_agent_config_time_lbl) = input.jsapi_ticket_agent_config_time_lbl.as_ref().filter(|s| !s.is_empty()) {
...
pub async fn create_return_wxw_app_token(
  ...
  let model_wxw_app_token = match model_wxw_app_token {
    Some(model) => model,
    None => {
...
// MARK: sync_usr_lbl_by_usr_id_wxw_app_token
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_wxw_app_token(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_wxw_app_token();
  let method = "sync_usr_lbl_by_usr_id_wxw_app_token";
...
// MARK: update_by_id_return_wxw_app_token
/// 根据 id 更新企微应用接口凭据, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_wxw_app_token(
  id: WxwAppTokenId,
  input: WxwAppTokenInput,
  options: Option<Options>,
) -> Result<WxwAppTokenModel> {
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "企微应用接口凭据 update_by_id_return_wxw_app_token id: {id}",
    )),
  }
}
...
pub async fn validate_option_wxw_app_token(
  ...
  let model = match model {
    Some(model) => model,
    None => {
      let err_msg = SmolStr::new("企微应用接口凭据不存在");
      error!(
        "{req_id} {err_msg}",
        req_id = get_req_id(),
      );
