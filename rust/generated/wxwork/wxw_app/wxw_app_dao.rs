...
pub async fn find_all_wxw_app(
  ...
  // 可信域名
  if let Some(search) = &search && let Some(domain_id) = &search.domain_id {
    let len = domain_id.len();
    if len == 0 {
      return Ok(vec![]);
    }
    if len > ids_limit {
  ...
  let res = {
    ...
    if let Some(str) = str {
      let res2: Vec<WxwAppModel>;
      let res = serde_json::from_str::<Vec<WxwAppModel>>(&str);
      if let Ok(res) = res {
        res2 = res;
      } else {
        res2 = vec![];
  ...
  let mut res: Vec<WxwAppModel> = if let Some(res) = res {
    res
  } else {
    let res = query(
      sql,
...
pub async fn create_return_wxw_app(
  ...
  let model_wxw_app = match model_wxw_app {
    Some(model) => model,
    None => {
...
// MARK: sync_usr_lbl_by_usr_id_wxw_app
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_wxw_app(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_wxw_app();
  let method = "sync_usr_lbl_by_usr_id_wxw_app";
...
// MARK: update_by_id_return_wxw_app
/// 根据 id 更新企微应用, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_wxw_app(
  id: WxwAppId,
  input: WxwAppInput,
  options: Option<Options>,
) -> Result<WxwAppModel> {
  
  update_by_id_wxw_app(
    id,
    input,
    options,
  ).await?;
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "企微应用 update_by_id_return_wxw_app id: {id}",
    )),
  }
}
