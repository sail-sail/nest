...
async fn get_where_query(
  ...
  {
    let cron_job_id_lbl: Option<Vec<SmolStr>> = match search {
  ...
  // 执行结果
  {
    ...
    if let Some(exec_result_like) = exec_result_like && !exec_result_like.is_empty() {
...
pub async fn find_all_cron_job_log(
  ...
  let table = get_table_name_cron_job_log();
  ...
  // 定时任务
  if let Some(search) = &search && let Some(cron_job_id) = &search.cron_job_id {
    let len = cron_job_id.len();
    if len == 0 {
      return Ok(vec![]);
    }
    if len > ids_limit {
  ...
  // 执行状态
  if let Some(search) = &search && let Some(exec_state) = &search.exec_state {
    let len = exec_state.len();
    if len == 0 {
      return Ok(vec![]);
    }
    if len > ids_limit {
...
pub async fn find_count_cron_job_log(
  ...
  // 定时任务
  if let Some(search) = &search && search.cron_job_id.is_some() {
  ...
  // 执行状态
  if let Some(search) = &search && search.exec_state.is_some() {
...
pub async fn get_field_comments_cron_job_log(
  ...
  let mut field_comments = CronJobLogFieldComment {
    id: "ID".into(),
...
pub async fn find_one_ok_cron_job_log(
  ...
  let Some(cron_job_log_model) = cron_job_log_model else {
...
pub async fn find_by_id_ok_cron_job_log(
  ...
  let cron_job_log_model = find_by_id_cron_job_log(
    id,
    options,
  ).await?;
  
  let Some(cron_job_log_model) = cron_job_log_model else {
    let err_msg = SmolStr::new("此 定时任务日志 已被删除");
    error!(
...
// MARK: find_by_ids_ok_cron_job_log
...
pub async fn find_by_ids_ok_cron_job_log(
  ...
  let table = get_table_name_cron_job_log();
  let method = "find_by_ids_ok_cron_job_log";
  ...
  let cron_job_log_models = find_by_ids_cron_job_log(
    ids.clone(),
    options,
  ).await?;
  
  if cron_job_log_models.len() != len {
    let err_msg = SmolStr::new("此 定时任务日志 已被删除");
    return Err(eyre!(err_msg));
  }
  
  let cron_job_log_models = ids
    .into_iter()
    .map(|id| {
      let model = cron_job_log_models
        .iter()
        .find(|item| item.id == id);
      if let Some(model) = model {
        return Ok(model.clone());
      }
      let err_msg = SmolStr::new("此 定时任务日志 已经被删除");
      Err(eyre!(err_msg))
    })
...
pub async fn find_by_ids_cron_job_log(
  ...
  let cron_job_log_models = find_all_cron_job_log(
    search,
    None,
    None,
    options,
...
pub async fn exists_by_id_cron_job_log(
  ...
  let exists = exists_cron_job_log(
    search,
    options,
  ).await?;
...
pub async fn create_return_cron_job_log(
  ...
  let model_cron_job_log = match model_cron_job_log {
    Some(model) => model,
    None => {
...
// MARK: sync_usr_lbl_by_usr_id_cron_job_log
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_cron_job_log(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_cron_job_log();
  let method = "sync_usr_lbl_by_usr_id_cron_job_log";
...
// MARK: update_by_id_return_cron_job_log
/// 根据 id 更新定时任务日志, 并返回更新后的数据
#[allow(dead_code)]
pub async fn update_by_id_return_cron_job_log(
  id: CronJobLogId,
  input: CronJobLogInput,
  options: Option<Options>,
) -> Result<CronJobLogModel> {
  ...
  match model {
    Some(model) => Ok(model),
    None => Err(eyre!(
      "定时任务日志 update_by_id_return_cron_job_log id: {id}",
    )),
  }
}
...
pub async fn delete_by_ids_cron_job_log(
  ...
  let old_models = find_by_ids_ok_cron_job_log(
    ids.clone(),
    options,
  ).await?;
...
pub async fn force_delete_by_ids_cron_job_log(
  ...
  for id in ids.clone() {
    ...
    let old_model = find_one_cron_job_log(
      Some(CronJobLogSearch {
        id: Some(id),
        is_deleted: Some(1),
        ..Default::default()
      }),
...
pub async fn validate_option_cron_job_log(
  ...
  let model = match model {
    Some(model) => model,
    None => {
      let err_msg = SmolStr::new("定时任务日志不存在");
      error!(
        "{req_id} {err_msg}",
        req_id = get_req_id(),
      );
