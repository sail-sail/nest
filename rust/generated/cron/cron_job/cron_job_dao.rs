...
pub async fn set_id_by_lbl_cron_job(
  ...
  {
    let job_model = crate::cron::job::job_dao::find_one_job(
      crate::cron::job::job_model::JobSearch {
...
// MARK: sync_usr_lbl_by_usr_id_cron_job
/// 根据 usr_id 同步创建人/更新人/删除人标签
pub async fn sync_usr_lbl_by_usr_id_cron_job(
  usr_id: UsrId,
  options: Option<Options>,
) -> Result<u64> {
  let table = get_table_name_cron_job();
  let method = "sync_usr_lbl_by_usr_id_cron_job";
...
pub async fn delete_by_ids_cron_job(
  ...
  let old_models = find_by_ids_ok_cron_job(
    ids.clone(),
    options,
  ).await?;
