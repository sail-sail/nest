...
> 退款不是同步完成。`refund()` 返回成功只代表微信侧已受理退款请求（即退款请求已被接受）, 不代表退款已经完成; 业务表状态更新应以 `wx_refund_notice` 收到 `Success` 通知后的回调为准。
...
```rust
match action {
  WX_REFUND_NOTICE_ACTION_REFUND_XXX => {
    let record_id = payload.get("record_id")
      .and_then(|v| v.as_str())
      .ok_or_else(|| eyre!("record_id not found"))?;
    let record_id = XxxRecordId::from(record_id);
...
如果是部分退款, 把回调后仍要用到的区间、明细 id、退款申请 id 等一起放进 `attach2.payload`, 然后在这里解析。对同一 `transaction_id` 可能发生多次部分退款时, `payload` 里至少要带能唯一标识“本次退款”的 `refund_apply_id` 或业务退款单 id; 回调里按这个 id 做幂等和对账, 再根据已成功退款累计金额回写剩余可退金额、剩余区间或剩余可履约内容, 不要只按交易单直接覆盖。
...
- 全额退款: 更新业务状态为已退款/已关闭
- 部分退款: 先按 `refund_apply_id` 等业务退款标识完成本次回调对账, 再回写剩余金额、剩余区间或剩余可履约内容
