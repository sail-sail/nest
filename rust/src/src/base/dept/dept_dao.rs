...
/// 获取当前登录用户的部门id列表
#[allow(dead_code)]
pub async fn get_auth_dept_ids() -> Result<Vec<ID>> {
...
async fn get_parents_by_id(
  ...
  let ids2: Vec<ID> = dept_models.into_iter()
    .map(|dept_model| {
...
/// 获取当前用户及其所有父部门的id
#[allow(dead_code)]
pub async fn get_auth_and_parents_dept_ids() -> Result<Vec<ID>> {
  
  let dept_ids = get_auth_dept_ids().await?;
  
  let mut parent_ids: Vec<ID> = vec![];
