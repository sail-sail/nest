...
pub async fn find_all(
  ...
  // 默认
  if let Some(search) = &search {
    if search.is_default.is_some() && search.is_default.as_ref().unwrap().is_empty() {
      return Ok(vec![]);
    }
  }
  // 启用
...
pub async fn create(
  ...
  let mut sql_values = String::with_capacity(2 * 13 + 2);
...
pub async fn revert_by_ids(
  ...
  for id in ids.clone() {
    ...
    // 检查数据的唯一索引
    {
      ...
      let mut input: DomainInput = old_model.into();
      input.id = None;
      ...
      let models: Vec<DomainModel> = models.into_iter()
        .filter(|item| 
          item.id != id
        )
        .collect();
