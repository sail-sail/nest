...
use super::permit_history_model::*;

#[allow(unused_variables)]
fn get_where_query<'a>(
  ctx: &mut impl Ctx<'a>,
  args: &mut QueryArgs,
  search: Option<PermitHistorySearch>,
) -> String {
  ...
  {
    let permit_id: Vec<String> = match &search {
      Some(item) => item.permit_id.clone().unwrap_or_default(),
      None => Default::default(),
    };
    if !permit_id.is_empty() {
      let arg = {
        let mut items = Vec::with_capacity(permit_id.len());
        for item in permit_id {
          args.push(item.into());
          items.push("?");
        }
        items.join(",")
      };
      where_query += &format!(" and permit_id_lbl.id in ({})", arg);
    }
  }
  {
    let permit_id_is_null: bool = match &search {
      Some(item) => item.permit_id_is_null.unwrap_or(false),
      None => false,
    };
    if permit_id_is_null {
      where_query += &format!(" and permit_id_lbl.id is null");
    }
  }
  {
...
fn get_from_query() -> &'static str {
  let from_query = r#"base_permit_history t
...
pub async fn find_all<'a>(
...
) -> Result<Vec<PermitHistoryModel>> {
  
  #[allow(unused_variables)]
  let table = "base_permit_history";
  ...
  let mut res: Vec<PermitHistoryModel> = ctx.query(
    sql,
    args,
    options,
  ).await?;
...
) -> Result<PermitHistoryFieldComment> {
  
  let n_route = i18n_dao::NRoute {
    route_path: "/base/permit_history".to_owned().into(),
  };
  
  let field_comments = PermitHistoryFieldComment {
    permit_id: n_route.n(ctx, "权限".to_owned(), None).await?,
    permit_id_lbl: n_route.n(ctx, "权限".to_owned(), None).await?,
...
pub async fn find_one<'a>(
  ...
  sort: Option<Vec<SortInput>>,
  options: Option<Options>,
) -> Result<Option<PermitHistoryModel>> {
  ...
  let model: Option<PermitHistoryModel> = res.into_iter().next();
...
pub async fn find_by_id<'a>(
  ...
  let search = PermitHistorySearch {
    id: Some(id),
    ..Default::default()
  }.into();
...
pub async fn set_id_by_lbl<'a>(
  ...
  // 权限
  if input.permit_id.is_none() {
    if is_not_empty_opt(&input.permit_id_lbl) && input.permit_id.is_none() {
      input.permit_id_lbl = input.permit_id_lbl.map(|item| 
        item.trim().to_owned()
      );
      let model = crate::gen::base::permit::permit_dao::find_one(
        ctx,
        crate::gen::base::permit::permit_model::PermitSearch {
          lbl: input.permit_id_lbl.clone(),
          ..Default::default()
        }.into(),
        None,
        None,
...
pub async fn create<'a>(
  ...
  if let Some(permit_id) = input.permit_id {
    sql_fields += ",permit_id";
    sql_values += ",?";
    args.push(permit_id.into());
  }
  // 角色
