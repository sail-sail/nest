...
pub struct BackgroundTaskSearch {
  ...
  /// 开始时间
  pub begin_time: Option<Vec<Option<chrono::NaiveDateTime>>>,
  /// 结束时间
  pub end_time: Option<Vec<Option<chrono::NaiveDateTime>>>,
  /// 备注
...
impl From<BackgroundTaskInput> for BackgroundTaskSearch {
  fn from(input: BackgroundTaskInput) -> Self {
    Self {
      ...
      // 开始时间
      begin_time: input.begin_time.map(|x| vec![Some(x), Some(x)]),
      // 结束时间
      end_time: input.end_time.map(|x| vec![Some(x), Some(x)]),
      // 备注
      rem: input.rem,
      // 创建人
