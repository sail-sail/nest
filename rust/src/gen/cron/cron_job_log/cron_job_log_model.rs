...
lazy_static! {
  /// 定时任务日志 前端允许排序的字段
  static ref CAN_SORT_IN_API_CRON_JOB_LOG: [&'static str; 2] = [
    "create_time",
    "update_time",
  ];
}
...
impl FromStr for CronJobLogExecState {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    match s {
      ...
      "fail" => Ok(Self::Fail),
      _ => Err(anyhow::anyhow!("CronJobLogExecState can't convert from {s}")),
    }
  }
}
...
pub fn check_sort_cron_job_log(
  ...
  for item in sort {
    ...
    if !CAN_SORT_IN_API_CRON_JOB_LOG.contains(&prop) {
      return Err(anyhow!("check_sort_cron_job_log: {}", serde_json::to_string(item)?));
    }
  }
