...
lazy_static! {
  /// 定时任务日志明细 前端允许排序的字段
  static ref CAN_SORT_IN_API_CRON_JOB_LOG_DETAIL: [&'static str; 2] = [
    "create_time",
    "update_time",
  ];
}
...
pub fn check_sort_cron_job_log_detail(
  ...
  for item in sort {
    ...
    if !CAN_SORT_IN_API_CRON_JOB_LOG_DETAIL.contains(&prop) {
      return Err(anyhow!("check_sort_cron_job_log_detail: {}", serde_json::to_string(item)?));
    }
  }
