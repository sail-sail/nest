...
pub async fn find_all(
  ...
  if let Some(search) = &search {
    if search.job_id.is_some() {
      ...
      if len > ids_limit {
        return Err(anyhow!("search.job_id.length > {ids_limit}"));
      }
    }
  }
  // 时区
  if let Some(search) = &search {
    if search.timezone.is_some() {
      ...
      let ids_limit = options
        ...
        .unwrap_or(FIND_ALL_IDS_LIMIT);
      if len > ids_limit {
        return Err(anyhow!("search.timezone.length > {ids_limit}"));
      }
    }
  }
  // 锁定
...
pub fn get_n_route() -> i18n_dao::NRoute {
  i18n_dao::NRoute {
    route_path: get_route_path_cron_job().into(),
  }
}
...
pub async fn set_id_by_lbl(
    ...
    && input.job_id.is_none()
  {
    ...
    let model = crate::r#gen::cron::job::job_dao::find_one(
  ...
  {
    let job_model = crate::r#gen::cron::job::job_dao::find_one(
      crate::r#gen::cron::job::job_model::JobSearch {
