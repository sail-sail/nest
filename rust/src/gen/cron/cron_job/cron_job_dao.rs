...
async fn get_where_query(
  ...
  {
    let ids: Option<Vec<CronJobId>> = match search {
  ...
  // 任务
  {
    let job_id: Option<Vec<JobId>> = match search {
  ...
  {
    let job_id_is_null: bool = match search {
      Some(item) => item.job_id_is_null.unwrap_or(false),
      None => false,
    };
    if job_id_is_null {
      where_query.push_str(" and t.job_id is null");
    }
  }
  {
  ...
  {
    let job_id_lbl_like = match search {
      Some(item) => item.job_id_lbl_like.clone(),
      None => None,
    };
    if let Some(job_id_lbl_like) = job_id_lbl_like {
      where_query.push_str(" and job_id_lbl.lbl like ?");
      args.push(format!("%{}%", sql_like(&job_id_lbl_like)).into());
    }
  }
  // Cron表达式
  {
    let cron = match search {
      Some(item) => item.cron.clone(),
      None => None,
    };
    if let Some(cron) = cron {
      where_query.push_str(" and t.cron=?");
      args.push(cron.into());
    }
    let cron_like = match search {
      Some(item) => item.cron_like.clone(),
      None => None,
    };
    if let Some(cron_like) = cron_like {
...
async fn get_from_query(
  ...
  let from_query = r#"cron_cron_job t
  left join cron_job job_id_lbl on job_id_lbl.id=t.job_id"#.to_owned();
  Ok(from_query)
}
...
pub async fn find_one_cron_job(
  ...
  let model: Option<CronJobModel> = res.into_iter().next();
  
  Ok(model)
}

// MARK: find_by_id_cron_job
/// 根据 id 查找定时任务
...
/// 根据 id 判断定时任务是否存在
#[allow(dead_code)]
pub async fn exists_by_id_cron_job(
  id: CronJobId,
  options: Option<Options>,
) -> Result<bool> {
...
// MARK: find_by_unique_cron_job
/// 通过唯一约束获得数据列表
#[allow(unused_variables)]
pub async fn find_by_unique_cron_job(
...
pub fn equals_by_unique(
  ...
  if
    ...
    input.cron.as_ref().is_some() && input.cron.as_ref().unwrap() == &model.cron
  {
    return true;
  }
  false
...
// MARK: check_by_unique_cron_job
/// 通过唯一约束检查数据是否已经存在
#[allow(unused_variables)]
pub async fn check_by_unique_cron_job(
  input: CronJobInput,
  model: CronJobModel,
  options: Option<Options>,
...
pub async fn set_id_by_lbl_cron_job(
  ...
  if input.timezone.is_none() {
    let timezone_dict = &dict_vec[0];
    if let Some(timezone_lbl) = input.timezone_lbl.clone() {
      input.timezone = timezone_dict
        .iter()
        .find(|item| {
          item.lbl == timezone_lbl
        })
        .map(|item| {
  ...
  // 任务
  if input.job_id_lbl.is_some()
    && !input.job_id_lbl.as_ref().unwrap().is_empty()
    && input.job_id.is_none()
  {
    input.job_id_lbl = input.job_id_lbl.map(|item| 
      item.trim().to_owned()
    );
    let model = crate::r#gen::cron::job::job_dao::find_one_job(
      crate::r#gen::cron::job::job_model::JobSearch {
        lbl: input.job_id_lbl.clone(),
        ..Default::default()
      }.into(),
      None,
    ...
    if let Some(model) = model {
      input.job_id = model.id.into();
    }
  } else if
    (input.job_id_lbl.is_none() || input.job_id_lbl.as_ref().unwrap().is_empty())
    && input.job_id.is_some()
  {
    let job_model = crate::r#gen::cron::job::job_dao::find_one_job(
      crate::r#gen::cron::job::job_model::JobSearch {
    ...
    if let Some(job_model) = job_model {
      input.job_id_lbl = job_model.lbl.into();
    }
  }
    ...
    && input.timezone.is_none()
  {
    ...
    (input.timezone_lbl.is_none() || input.timezone_lbl.as_ref().unwrap().is_empty())
...
// MARK: creates_return_cron_job
/// 批量创建定时任务并返回
#[allow(dead_code)]
pub async fn creates_return_cron_job(
...
async fn _creates(
    ...
    .enumerate()
  {
    ...
    let id: CronJobId = get_short_uuid().into();
    ids2.push(id.clone());
...
// MARK: create_return_cron_job
/// 创建定时任务并返回
#[allow(dead_code)]
pub async fn create_return_cron_job(
  #[allow(unused_mut)]
...
// MARK: update_tenant_by_id_cron_job
/// 定时任务根据id修改租户id
...
// MARK: get_is_enabled_by_id_cron_job
/// 根据 id 查找定时任务是否已启用
/// 记录不存在则返回 false
...
// MARK: get_is_locked_by_id_cron_job
/// 根据 id 查找定时任务是否已锁定
/// 已锁定的记录不能修改和删除
/// 记录不存在则返回 false
...
pub async fn revert_by_ids_cron_job(
  ...
  for id in ids.clone() {
    ...
    let mut old_model = find_one_cron_job(
      CronJobSearch {
        id: Some(id.clone()),
        is_deleted: Some(1),
        ..Default::default()
...
// MARK: force_delete_by_ids_cron_job
/// 根据 ids 彻底删除定时任务
#[allow(unused_variables)]
...
// MARK: validate_is_enabled_cron_job
/// 校验定时任务是否启用
#[allow(dead_code)]
pub async fn validate_is_enabled_cron_job(
  model: &CronJobModel,
) -> Result<()> {
