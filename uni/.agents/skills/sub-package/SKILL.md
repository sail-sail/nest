...
3. 对所有移入 `subPackages` 的页面, 都需要把页面路径改成相对 `root` 的路径：去掉目录前缀，只保留文件名
...
1. 子包配置字段：`root` 为子包根目录，即 `pages/{目录名}`；`name` 取目录名；`path` 为相对于 `root` 的页面路径（即只保留文件名部分）
2. 页面归组方式：同一目录下的多个页面归入同一个子包
3. 必须留在主包的页面：tabBar 页面**不能**放入子包；首页（`pages` 数组第一项）**不能**放入子包
