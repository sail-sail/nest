...
>
  ...
  >
    ...
    >
      <tm-radio
        ...
        :size="30"
        :label-font-size="28"
        :label="`${ dyn_page_ids_selected.length ? dyn_page_ids_selected.length : '' }`"
      ></tm-radio>
    </view>
    ...
    >
      ...
      >
        ...
        <text
          un-whitespace="nowrap"
          un-cursor="pointer"
          @click="isEditing = false;dyn_page_ids_selected = [ ]"
        >
          取消
        </text>
  ...
  >
    ...
    <view
      v-if="!inited && dyn_page_models.length === 0"
      un-m="x-2 t-2"
      un-p="x-4 y-4"
      un-box-border
      un-flex="~ [1_0_0]"
    ...
    >
      ...
      >
        ...
        >
          <tm-radio
            :model-value="dyn_page_ids_selected.includes(dyn_page_model.id)"
            :size="30"
            :show-label="false"
            @change="onRadio($event, dyn_page_model.id)"
          >
          </tm-radio>
        </view>
        ...
        >
          ...
          >
            ...
            <view
              ...
              un-m="x-2"
            >
              {{ dyn_page_model.create_time_lbl }}
            </view>
...
const dyn_page_models_computed = computed<DynPageModelComputed[]>(() => {
  return dyn_page_models.map((dyn_page_model, i) => {
    let create_time_lbl = "";
    if (dyn_page_model.create_time) {
      create_time_lbl = dayjs(dyn_page_model.create_time).format("YYYY-MM-DD");
    }
    return {
...
function onRadio(
  ...
  if (checked) {
    if (!dyn_page_ids_selected.includes(dyn_page_id)) {
      dyn_page_ids_selected.push(dyn_page_id);
    }
  } else {
    dyn_page_ids_selected = dyn_page_ids_selected.filter((item) => item !== dyn_page_id);
  }
}
...
uni.$on("/pages/dyn_page/List:refresh", async function() {
  pgOffset = 0;
  await onRefresh();
});
...
const isSelectAll = $computed({
  get() {
    if (dyn_page_models.length === 0) {
      return false;
    }
    return dyn_page_ids_selected.length === dyn_page_models.length;
  },
  set(value) {
    if (value) {
      dyn_page_ids_selected = dyn_page_models.map((item) => item.id);
    } else {
      dyn_page_ids_selected = [ ];
    }
  },
});
...
async function onDelete() {
  if (dyn_page_ids_selected.length === 0) {
    uni.showToast({
      title: "请选择需要删除的动态页面",
      icon: "none",
    });
  ...
  const len = dyn_page_ids_selected.length;
  const {
    confirm,
  } = await uni.showModal({
    title: "提示",
    content: `确定删除选中的 ${ len } 个动态页面吗？`,
    confirmText: "删除",
    cancelText: "取消",
  });
  if (!confirm) {
  ...
  await deleteByIdsDynPage(dyn_page_ids_selected);
  
  uni.showToast({
    title: `删除 ${ len } 个动态页面成功`,
    icon: "none",
  });
...
async function onRefresh() {
  ...
  try {
    ...
    dyn_page_ids_selected = dyn_page_ids_selected
      .filter((id) => dyn_page_models.some((item2) => item2.id === id));
...
async function onLoadMore() {
  ...
  isLoading = true;
  try {
    const dyn_page_models_new = await findAllDynPage(
      getSearchDynPage(),
      {
        pgSize,
        pgOffset,
    ...
    const len = dyn_page_models_new.length;
    if (len < pgSize) {
      isEnd = true;
    }
    if (len > 0) {
      pgOffset += len;
      dyn_page_models.push(...dyn_page_models_new);
    }
    if (!dyn_page_models.some((item) => item.id === dyn_page_id_selected)) {
      dyn_page_id_selected = undefined;
    }
