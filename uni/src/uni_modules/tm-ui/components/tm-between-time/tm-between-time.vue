 ...
 */
	...
	const _start_date = computed(() => {
		const optimizedStart =
			props.start === "" ? startDate.value : new tmDate(optimizeStartTime(props.start));
		return optimizedStart;
	});
	const _end_date = computed(() => {
		const optimizedEnd = props.end === "" ? endDate.value : new tmDate(optimizeEndTime(props.end));
		return optimizedEnd;
	});
	const _start_date_str = computed(() => _start_date.value.format());
	const _end_date_str = computed(() => _end_date.value.format());
	const _start_date_str_format = computed(() =>
		nowValue.value[0] === ""
			? $i18n.t("tmui32x.tmBetweenTime.startText")
			: new tmDate(nowValue.value[0]).format(props.format),
	);
	const _end_date_str_format = computed(() =>
		nowValue.value[1] === ""
			? $i18n.t("tmui32x.tmBetweenTime.endText")
			: new tmDate(nowValue.value[1]).format(props.format),
	);

	const _backgroundColor = computed(() =>
		config.mode === "dark"
			? getDefaultColor(config.inputDarkColor)
			: getDefaultColor("#f5f5f5"),
	);
	const _borderColor = computed(() =>
		config.mode === "dark"
			? getDefaultColor(config.borderDarkColor)
			: getDefaultColor("#eaeaea"),
	);
	const _activeBorderColor = computed(() => getDefaultColor(config.color));
	const _placeStyle = computed(() =>
		config.mode === "dark" ? "color:#c7c7c7;" : "color:#838383;",
	);
	...
	function inputClick(index: number) {
		...
		if (changeIndex.value === 0) {
			if (strStart === "") {
				...
				strStart = new tmDate()
					.getBetweenDate(_start_date.value, _end_date.value, "max")
					.format();
				nowValue.value = [strStart, strEnd];
			}
		} else if (changeIndex.value === 1) {
	...
	function getQuickDateType(): coverValue[] {
		...
		for (let i = 0; i < list.length; i++) {
			...
			if (!isNaN(parseInt(item))) {
				...
				let start = new tmDate()
					.getBetweenDate(_start_date.value, _end_date.value, "min")
					.format(startFDate);
				let end = new tmDate()
					.getBetweenDate(_start_date.value, _end_date.value, "max")
					.format(endFDate);
				...
				if (item === "w") {
					...
					start = new tmDate(itemqatar.start)
						.getBetweenDate(_start_date.value, _end_date.value, "min")
						.format(startFDate);
					end = new tmDate(itemqatar.end)
						.getBetweenDate(_start_date.value, _end_date.value, "max")
						.format(endFDate);
		...
		<tm-drawer
			...
			<template v-slot:title>
				<view
					...
					<tm-text @click="clearDate" style="opacity: 0.5">{{
						$i18n.t("tmui32x.tmBetweenTime.clearText")
					}}</tm-text>
				</view>
			</template>
			<view v-if="yanchiDuration" class="tmPickerDateWrap">
				...
				<view class="tmPickerDateWrapQuickInput">
					<view @click="inputClick(0)" style="flex: 1; height: 40px">
						<input
							:placeholder-style="_placeStyle"
							:style="{
								color: changeIndex == 0 ? _activeBorderColor : _fontColor,
								border: `2px solid ${changeIndex == 0 ? _activeBorderColor : _borderColor}`,
								backgroundColor: _backgroundColor,
							}"
							class="tmPickerInput"
							:disabled="true"
							:value="_start_date_str_format" />
					</view>
					<tm-text style="width: 64px; text-align: center">{{
						$i18n.t("tmui32x.tmBetweenTime.betweenText")
					}}</tm-text>
					<view @click="inputClick(1)" style="flex: 1; height: 40px">
						<input
							:placeholder-style="_placeStyle"
							:style="{
								color: changeIndex == 1 ? _activeBorderColor : _fontColor,
								border: `2px solid ${changeIndex == 1 ? _activeBorderColor : _borderColor}`,
								backgroundColor: _backgroundColor,
							}"
							class="tmPickerInput"
							:disabled="true"
							:value="_end_date_str_format" />
					</view>
				</view>
				<tm-date-view
					v-if="show"
					@change="dateChangeView"
					:format="format"
					:type="type"
					:cell-units="cellUnits"
					:start="_start_date_str"
					:end="_end_date_str"
					:model-value="nowValue[changeIndex]"></tm-date-view>
			</view>
