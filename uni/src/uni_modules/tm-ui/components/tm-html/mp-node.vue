<template>
	<view :id="attrs.id" :class="'_block _'+name+' '+attrs.class" :style="attrs.style">
		<block v-for="(n, i) in childs" v-bind:key="i">
			<!-- 图片 -->
			<!-- 占位图 -->
			<image v-if="n.name==='img'&&!n.t&&((opts[1]&&!ctrl[i])||ctrl[i]<0)&&typeof (ctrl[i]<0?opts[2]:opts[1]) == 'string'" class="_img" :style="n.attrs.style"
				:src="ctrl[i]<0?opts[2]:opts[1]" mode="widthFix" />
			<!-- 显示图片 -->
			<!-- #ifdef H5 || (APP-PLUS && VUE2) -->
			<img v-if="n.name==='img'&&typeof (n.attrs.src||(ctrl.load?n.attrs['data-src']:'')) == 'string'" :id="n.attrs.id" :class="'_img '+n.attrs.class"
				...
				@tap.stop="imgTap(n.attrs.src||(ctrl.load?n.attrs['data-src']:''))" @longpress="imgLongTap" />
			<!-- #endif -->
			...
			<image v-else-if="n.name==='img'" :id="n.attrs.id" :class="'_img '+n.attrs.class"
				...
				:webp="n.webp" :show-menu-by-longpress="opts[3]&&!n.attrs.ignore"
				:image-menu-prevent="!opts[3]||n.attrs.ignore" :data-i="i" @load="imgLoad" @error="mediaError"
				@tap.stop="imgTap(n.attrs.src)" @longpress="imgLongTap" />
			<!-- #endif -->
			<!-- #ifdef MP-KUAISHOU -->
			<image v-else-if="n.name==='img'" :id="n.attrs.id" :class="'_img '+n.attrs.class"
				:style="(ctrl[i]===-1?'display:none;':'')+n.attrs.style" :src="n.attrs.src" :lazy-load="opts[0]"
				:data-i="i" @load="imgLoad" @error="mediaError" @tap.stop="imgTap(n.attrs.src)"></image>
			<!-- #endif -->
			<!-- #ifdef APP-PLUS && VUE3 -->
			<image v-else-if="n.name==='img'" :id="n.attrs.id" :class="'_img '+n.attrs.class"
		...
		methods: {
			...
			imgTap(e) {
				...
				if (this.opts[5]) {
					uni.previewImage({
						// #ifdef MP-WEIXIN
						showmenu: this.opts[3],
						// #endif
						// #ifdef MP-ALIPAY
						enablesavephoto: this.opts[3],
						enableShowPhotoDownload:this.opts[3],
						// #endif
						current:0,
						urls: [e]
					})
