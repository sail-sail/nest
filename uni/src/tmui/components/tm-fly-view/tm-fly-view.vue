<template>
	<view @click="onClick" id="flyViewBody" class="">
		...
		<slot></slot>
		<!-- #endif -->
		<view  id="flyView" ref="flyView" class="flex flex-row flex-row-center-center" :eventPenetrationEnabled="true" :style="{
...
<script lang="ts" setup>
	...
	function onClick(ev : TouchEvent | MouseEvent) {
		...
		uni.$tm.u.queryDom(proxy, '#flyViewBody').then(r => {
			console.log(ev)
			elTop = r.top;
			elLeft = r.left;
			elWidth = r.width
	...
	function play() {
		...
		tween = new TWEEN.Tween({ progress: 0 })
			...
			.onUpdate((e : any) => {
				...
				const x = findXOnQuadraticBezierCurve(
					s_x,
					s_y,
					co_x,
					co_y,
